...
namespace BP.WF.Template.Frm
{
    ...
    public class MapDtlExt : EntityNoName
    {
        ...
        public string InitAttrsOfSelf()
        {
            ...
            GroupField gf = (GroupField)gfs[0];
            //处理属性问题.
            MapAttrs attrs = new MapAttrs();
            attrs.Delete(MapAttrAttr.FK_MapData, this.No);
            ...
            mapDtls.Delete(MapAttrAttr.FK_MapData, this.No);
            mapDtls.Retrieve(MapAttrAttr.FK_MapData, refDtl);
            foreach (MapDtl dtl in mapDtls.ToJavaList())
            {
                dtl.No=dtl.No + "_" + this.NodeID;
                dtl.FrmID=this.No;
                dtl.Insert();
                attrs = new MapAttrs();
                attrs.Delete(MapAttrAttr.FK_MapData, dtl.No + "_" + this.NodeID);
                attrs.Retrieve(MapAttrAttr.FK_MapData, dtl.No, null);
                foreach (MapAttr attr in attrs)
                {
                    String oldMyPK = attr.MyPK;
                    attr.setMyPK(dtl.No + "_" + this.NodeID + "_" + attr.KeyOfEn);
                    attr.setFrmID(dtl.No + "_" + this.NodeID);
                    attr.setGroupID(gf.OID);
                    attr.Insert();
                    //存在字段附件
                    if (attr.UIContralType == UIContralType.AthShow)
                    {
                        ...
                        athDesc.setMyPK(attr.MyPK);
                        athDesc.FrmID=dtl.No + "_" + this.NodeID;
                        athDesc.NodeID=this.NodeID;
                        athDesc.DirectInsert();
                    }
                }
                ...
                exts1.Delete(MapAttrAttr.FK_MapData, dtl.No + "_" + this.NodeID); //先删除，后查询.
                exts1.Retrieve(MapAttrAttr.FK_MapData, dtl.No, null);
                MapExt mapExt1 = null;
                foreach (MapExt ext in exts1)
                {
                    mapExt1 = new MapExt();
                    mapExt1 = ext;
                    mapExt1.setMyPK(ext.ExtType + "_" + dtl.No + "_" + this.NodeID + "_" + ext.AttrOfOper);
                    mapExt1.FrmID=dtl.No + "_" + this.NodeID;
                    mapExt1.Insert();
                }
            }
            //处理mapExt 的问题.
