    ...
    public class FlowExt : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                //map.SetHelperUrl(FlowAttr.TimelineRole, "http://doc.ccbpm.cn/?v=5404&t=17036");
               ...
               true, true, FlowAttr.Draft, "@0=无(不设草稿)@1=保存到待办@2=保存到草稿箱");
                ...
                // rm.Warning = "您确定要回滚它吗？";
                rm.HisAttrs.AddTBInt("workid", 0, "请输入要回滚的WorkID", true, false);
        ...
        public string DoDelFlows(string dtFrom, string dtTo, Boolean isDelCurrFlow)
        {
            ...
            if (isDelCurrFlow == true)
                sql = "SELECT WorkID, FK_Flow FROM WF_GenerWorkFlow  WHERE RDT >= '" + dtFrom + "' AND RDT <= '" + dtTo + "' AND FK_Node IN (SELECT NodeID From WF_Node WHERE FK_Flow='" + this.No + "') ";
            else
            ...
            sql = "SELECT WorkID, FK_Flow FROM WF_GenerWorkerlist  WHERE RDT >= '" + dtFrom + "' AND RDT <= '" + dtTo + "' AND FK_Node IN (SELECT NodeID From WF_Node WHERE FK_Flow='" + this.No + "')";
        ...
        public void DoDelFlowByWorkID(Int64 workid, string fk_flow)
        {
            ...
            //删除表单信息
            if (DataType.IsNullOrEmpty(this.PTable) == false && DBAccess.IsExitsObject(this.PTable) == true)
                DBAccess.RunSQL("DELETE FROM " + this.PTable + " Where OID=" + workid);
            //删除节点信息
        ...
        public string DoRebackFlowData(Int64 workid, int backToNodeID, string note)
        {
            ...
            try
            {
                ...
                string actionType = (int)ActionType.Forward + "," + (int)ActionType.FlowOver + "," + (int)ActionType.ForwardFL + "," + (int)ActionType.ForwardHL + "," + (int)ActionType.Skip + "," + (int)ActionType.TeampUp;
        ...
        protected override bool beforeUpdate()
        {
            ...
            if (fl.ItIsStartInMobile != this.ItIsStartInMobile 
                || fl.ItIsCanStart != this.ItIsCanStart 
                || fl.Name.Equals(this.Name) == false 
                || fl.GetValByKey("Icon").Equals(this.GetValByKey("Icon")))
            {
