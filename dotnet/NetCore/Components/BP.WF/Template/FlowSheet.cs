    ...
    public class FlowSheet : EntityNoName
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.SetHelperUrl(FlowAttr.No, "http://doc.ccbpm.cn/?v=5404&t=17023"); //使用alert的方式显示帮助信息.

                map.AddDDLEntities(FlowAttr.FK_FlowSort, "01", "流程类别", new FlowSorts(), true);

                map.SetHelperUrl(FlowAttr.FK_FlowSort, "http://doc.ccbpm.cn/?v=5404&t=17024");
                ...
                map.SetHelperUrl(FlowAttr.FlowMark, "http://doc.ccbpm.cn/?v=5404&t=16847");
                map.SetHelperUrl(FlowAttr.FlowEventEntity, "http://doc.ccbpm.cn/?v=5404&t=17026");

                // add 2013-02-05.
                map.AddTBString(FlowAttr.TitleRole, null, "标题生成规则", true, false, 0, 150, 10, true);
                map.SetHelperUrl(FlowAttr.TitleRole, "http://doc.ccbpm.cn/?v=5404&t=17040");

                //add  2013-08-30.
                map.AddTBString(FlowAttr.BillNoFormat, null, "单据编号格式", true, false, 0, 50, 10, false);
                map.SetHelperUrl(FlowAttr.BillNoFormat, "http://doc.ccbpm.cn/?v=5404&t=17041");
                ...
                map.SetHelperUrl(FlowAttr.IsCanStart, "http://doc.ccbpm.cn/?v=5404&t=17027");


                map.AddBoolean(FlowAttr.IsMD5, false, "是否是数据加密流程(MD5数据加密防篡改)", true, true, true);
                map.SetHelperUrl(FlowAttr.IsMD5, "http://doc.ccbpm.cn/?v=5404&t=17028");
             ...
             //   map.AddBoolean(FlowAttr.IsWaterMark, false, "表单是否启用水印", true, true, true);
              //  map.AddDDLSysEnum(FlowAttr.WaterMarkText, 0, "'水印默认展示'", true, true, "WaterMarkText", "@0=系统默认值@1=登录人员账号@2=登录人员名称@3=当前日期(yyyy年MM月dd日)@4=当前日期(yyyy年MM月dd日HH时mm分)@5=登录人员名称+当前日期(yyyy年MM月dd日)@6=登录人员名称+当前日期(yyyy年MM月dd日HH时mm分)");
                ...
                map.SetHelperUrl(FlowAttr.IsFullSA, "http://doc.ccbpm.cn/?v=5404&t=17034");
                ...
                map.SetHelperUrl(FlowAttr.IsBatchStart, "http://doc.ccbpm.cn/?v=5404&t=17047");


                // 草稿
                map.AddDDLSysEnum(FlowAttr.Draft, (int)DraftRole.None, "草稿规则",
               true, true, FlowAttr.Draft, "@0=无(不设草稿)@1=保存到待办@2=保存到草稿箱");
                map.SetHelperUrl(FlowAttr.Draft, "http://doc.ccbpm.cn/?v=5404&t=17037");
        ...
        /// <returns></returns>
        public string DoRebackFlowData(Int64 workid, int backToNodeID, string note,string empNo="")
        {
            ...
            try
            {
                ...
                string actionType = (int)ActionType.Forward + "," + (int)ActionType.FlowOver + "," + (int)ActionType.ForwardFL + "," + (int)ActionType.ForwardHL + "," + (int)ActionType.TeampUp + "," + (int)ActionType.Skip;
                string sql = "SELECT  * FROM " + ndTrack + " WHERE   ActionType IN (" + actionType + ")  and WorkID=" + workid + " ORDER BY RDT ASC, NDFrom ";
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    string nodeData = dr[TrackAttr.NodeData].ToString();
                    ...
                    if (gwf.NodeID == backToNodeID)
                        gwl.ItIsPass = false;
                    else gwl.ItIsPass = true;
                    ...
                    string deptNo = emp.DeptNo;
                    string deptName = emp.DeptText;
                    if (DataType.IsNullOrEmpty(nodeData) == false)
                    {
                        ...
                        if (DataType.IsNullOrEmpty(deptName))
                            deptName = emp.DeptText;
                    }
                  
                    gwl.DeptNo = deptNo;
                    gwl.DeptName = deptName;
            ...
            catch (Exception ex)
            {
                ...
                gwf.WFState = WFState.CompleteEnd;
                gwf.Update();
                string sqls = "";
                sqls += "@UPDATE " + fl.PTable + " SET WFState=" + (int)WFState.CompleteEnd + " WHERE OID=" + workid;
                DBAccess.RunSQLs(sqls);
                return "<font color=red>回滚期间出现错误</font><hr>" + ex.Message;
            }
        }
        /// <summary>
        /// 重新产生标题，根据新的规则.
