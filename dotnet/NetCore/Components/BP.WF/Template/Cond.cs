    ...
    public class Cond : EntityMyPK
    {
        ...
        public string SpecOper
        {
            get
            {
                ...
                if (specOperWay == (int)SpecOperWay.CurrOper)
                    return BP.Web.WebUser.No;


                if (specOperWay == (int)SpecOperWay.SpecNodeOper)
                {
                ...
                if (specOperWay == (int)SpecOperWay.SpecSheetField)
                {
                ...
                if (specOperWay == (int)SpecOperWay.SpenEmpNo)
                {
        ...
        {
            Log.DebugWriteInfo("用户[" + WebUser.No + "]，执行删除方向条件，数据=" + BP.Tools.Json.ToJson(this.ToDataTableField()));
            Flow flow = new Flow(this.FlowNo);
            flow.ClearAutoNumCache(true);
            base.afterDelete();
        }
        ...
        public virtual bool IsPassed
        {
            get
            {
                ...
                if (this.HisDataFrom == ConnDataFrom.CondEmp)
                {
                    #region 按人员控制
                ...
                if (this.HisDataFrom == ConnDataFrom.Depts)
                {
                    ...
                    foreach (BP.Port.DeptEmp st in sts)
                    {
                        ...
                        {
                            this.MsgOfCond = "@以部门判断方向，条件为true：部门集合" + strs + "，操作员(" + BP.Web.WebUser.No + ")部门:" + st.DeptNo;
                ...
                //按照部门的领导计算.
                if (this.HisDataFrom == ConnDataFrom.CondLeaderOfDept)
                {
                    #region 按部门控制
                    string strs = "," + this.OperatorValue.ToString() + ",";
                    strs = strs.Replace("@", ",");
                    ...
                    foreach (DataRow dr in dtLeader.Rows)
                        empNosOfLeader += dr[0] + ",";
                    ...
                    foreach (BP.Port.DeptEmp st in sts)
                    {
                        if (empNosOfLeader.Contains(","+st.EmpNo+",")==true)
                        {
                            this.MsgOfCond = "@判断部门领导，条件为true：部门集合" + strs + "，操作员(" + BP.Web.WebUser.No + ")部门:" + st.DeptNo;
                    ...
                    this.MsgOfCond = "@判断部门领导，条件为false：部门集合" + strs + "，操作员(" + BP.Web.WebUser.No + ")部门:";
        ...
        private DeptEmps GetCondShengFenDepts()
        {
            ...
            switch (specOperWay)
            {
                ...
                case 3: //发送人使用部门的父级
                    ...
                    if (i == 0)
                        throw new Exception("err@不可能出现的错误:GetCondShengFenDepts");
                    if (specOperWay == 2)
        ...
        private DeptEmpStations GetCondShengFenStations()
        {
            ...
            switch (specOperWay)
            {
                ...
                case 2: //发送人选择部门+岗位
                    ...
                    i = gwl.Retrieve(GenerWorkerListAttr.WorkID, this.WorkID, GenerWorkerListAttr.FK_Node, nodeID, GenerWorkerListAttr.FK_Emp, this.SpecOper);
                    if (i == 0)
                        sts.Retrieve("FK_Emp", userNo, "FK_Dept", WebUser.DeptNo); 
                                                                                   //    throw new Exception("err@不可能出现的错误:DeptEmpStations");
                    if (specOperWay == 0)
        ...
        private string GenerDeptNosString(string deptNo, string deptNos)
        {
            ...
            {
                deptNos += "," + en.No;
                deptNos = GenerDeptNosString(en.No, deptNos);
            }
            return deptNos;
        }
    ...
    public class Conds : Entities
    {
        ...
        public bool GenerResult(GERpt en, WebUserCopy webUser, Node nd)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("err@条件设置有错误,节点[" + nd.Name + "]，估计是条件表达式不符合规范，请在设计后做检查，技术信息:" + sql);
                }
                return true;
                #endregion 处理混合计算。
            }
