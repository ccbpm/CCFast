    ...
    public class TemplateNode : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBInt(BtnAttr.ThreadIsCanShift, 0, "是否可以移交子线程(当前节点已经发送出去的线程，并且当前节点是分流，或者分合流有效，在子线程退回后在子线程退回后的操作)？", false, false);
                map.AddTBInt(NodeAttr.AutoJumpRole3, 0, "未来节点处理人已经出现过(只针对计算未来处理人的节点使用)", false, false);
                map.AddTBInt("CutEnderEmp", 0, "去掉最后一个接受人", false, false);
                map.AddTBString("AiLab", "Ai助理", "Ai助理", true, false, 0, 50, 10);
                map.AddBoolean("AiAssistant", false, "是否启用", true, true);
                if (SystemConfig.CustomerNo == "TianYu")
                {
