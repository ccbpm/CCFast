    ...
    public class Selector : Entity
    {
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AttrsOfOneVSM.AddBranches(new BP.WF.Template.NodeDepts(), new BP.Port.Depts(),
                   ...
                   BP.WF.Template.NodeDeptAttr.FK_Dept, "绑定部门", BP.Port.EmpAttr.Name, BP.Port.EmpAttr.No, "@WebUser.DeptNo");
        ...
        private DataSet BySQL(int nodeID, Entity en)
        {
            ...
            if (this.SelectorP4 != "")
            {
                ...
                sqlDB = sqlDB.Replace("@WebUser.DeptNo", WebUser.DeptNo);
        ...
        private DataSet AccepterOfDeptStationOfCurrentOper(int nodeID, Entity en)
        {
            ...
            //人员.
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.GBASE8CByOracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR3 || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
            ...
            else //如果没人，就查询父级
            {
                ...
                sql = "SELECT DISTINCT b." + BP.Sys.Base.Glo.UserNo + ",b.Name,b.FK_Dept   FROM Port_DeptEmpStation a, Port_Emp b  WHERE FK_Station IN "
                   ...
                   + " AND a.FK_Emp = b." + BP.Sys.Base.Glo.UserNoWhitOutAS + " ";
                sql += " ORDER BY b.No ";
        ...
        private DataSet DeptAndStation(int nodeID)
        {
            ...
            //@zkr.
            sql = "SELECT distinct a." + BP.Sys.Base.Glo.UserNo + ",a.Name, a.FK_Dept,a.Idx FROM Port_Emp A,  WF_NodeStation b, Port_DeptEmpStation c,WF_NodeDept D WHERE a." + BP.Sys.Base.Glo.UserNoWhitOutAS + "=c.FK_Emp AND B.FK_Station=C.FK_Station AND b.FK_Node=" + nodeID + " AND D.FK_Dept=A.FK_Dept AND D.FK_Node=" + nodeID + "  ORDER BY A.Idx ";
            DataTable dtEmp = DBAccess.RunSQLReturnTable(sql);
            dtEmp.TableName = "Emps";
            ds.Tables.Add(dtEmp);
            return ds;
        ...
        private DataSet ByGroupOnly(int nodeID, Entity en)
        {
            ...
            //人员.
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.GBASE8CByOracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR3 || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR6 || BP.Difference.SystemConfig.AppCenterDBType == DBType.PostgreSQL || BP.Difference.SystemConfig.AppCenterDBType == DBType.HGDB || DBAccess.AppCenterDBType == DBType.UX || DBAccess.AppCenterDBType == DBType.DM)
            {
        ...
        private DataSet ByStationAI(int nodeID, Entity en)
        {
            ...
            //如果按照上一个节点的操作员身份计算.
            if (ShenFenModel == 0)
                return ByStationAI(en, BP.Web.WebUser.DeptNo, WebUser.No, nd);
            ...
            if (ShenFenModel == 2)
            {
                ...
                return ByStationAI(en, emp.DeptNo, emp.No, nd);
            }

            throw new Exception("err@没有判断的身份模式." + ShenFenModel);
        }

        private DataSet ByStationAI(Entity en, string deptNo, string userID,Node nd)
        {
            DataSet ds = null;
            if (nd.DeliveryStationReqEmpsWay == 1 || nd.DeliveryStationReqEmpsWay == 2)
            {
                ds = new DataSet();
                ...
                DataTable staDT = DBAccess.RunSQLReturnTable("SELECT FK_Station From WF_NodeStation WHERE FK_Node=" + nd.NodeID);
                String stas = "";
                ...
                foreach (string str in temps)
                {
                    ...
                    //如果是严谨模式.
                    if (nd.DeliveryStationReqEmpsWay == 1 && mydt1.Rows.Count == 0)
                    {
            ...
            //第一次计算.
            ds = ByStationAI_Ext(en, deptNo, userID);
        ...
        private DataTable FindWorker_GetEmpsByDeptAI(String stas, String deptNo, DataTable dtt)
        {
            ...
            if (dt.Rows.Count == 0)
		    {
                ...
                dt = FindWorker_GetEmpsByStationsAndDepts(stas, deptMy.ParentNo);

			    //本级部门的祖父级,不在向上判断了.
			    if (dt.Rows.Count == 0 && deptMy.ParentNo.Equals("0") == false)
			    {
				    Dept deptParent = new Dept(deptMy.ParentNo);
                    dt = FindWorker_GetEmpsByStationsAndDepts(stas, deptParent.ParentNo);
				    if (dt.Rows.Count != 0){
					    dr = dtt.NewRow();
                        dept = new Dept(deptParent.ParentNo);
                        dr["No"]=dept.No;
					    dr["Name"]=dept.Name;
					    dtt.Rows.Add(dr);
				    }
	    ...
	    private DataTable FindWorker_GetEmpsByStationsAndDepts(String stas, String depts)
        {
            ...
            if (BP.Difference.SystemConfig.CCBPMRunModel != CCBPMRunModel.Single)
                sqlEnd = " AND OrgNo='" + WebUser.OrgNo + "'";
        ...
        private DataSet ByStation(int nodeID, Entity en)
        {
            ...
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle
                ...
                || BP.Difference.SystemConfig.AppCenterDBType == DBType.HGDB || DBAccess.AppCenterDBType == DBType.UX)
            {
                if (DBAccess.IsExitsTableCol("Port_Emp", "Idx") == true)
            ...
            else
            {
                if (SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
                    sql = "SELECT distinct a.UserID as No,a.Name, a.FK_Dept,a.Idx FROM Port_Emp a,WF_NodeStation b, Port_DeptEmpStation c WHERE a.FK_Emp=c.FK_Emp AND B.FK_Station=C.FK_Station AND b.FK_Node=" + nodeID + "  ORDER BY A.Idx";
                else
                    sql = "SELECT distinct a." + BP.Sys.Base.Glo.UserNo + ",a.Name, a.FK_Dept,a.Idx FROM Port_Emp a,WF_NodeStation b, Port_DeptEmpStation c WHERE a.No=c.FK_Emp AND B.FK_Station=C.FK_Station AND b.FK_Node=" + nodeID + "  ORDER BY A.Idx";
            }
