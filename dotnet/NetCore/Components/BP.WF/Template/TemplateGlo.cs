    ...
    public class TemplateGlo
    {
                        ...
                        if (DataType.IsNullOrEmpty(val) == false)
                        {
                            ...
                            if (DataType.IsNullOrEmpty(item) == false)
                            {
                                ...
                                foreach (String nodeID in nodeIDs)
                                {
                                    ...
                                    if (nodeID.Count() < iOldFlowLength)
                                ...
                                if (nodeIDStr.EndsWith(","))
                                    nodeIDStr = nodeIDStr.Substring(0, nodeIDStr.Count() - 1);
            ...
            foreach (DataTable dt in ds.Tables)
            {
                ...
                switch (dt.TableName)
                {
                    ...
                    case "WF_SyncDataField":
                        continue;
                    case "WF_SyncData":
                        DataTable fieldDt = ds.Tables["WF_SyncDataField"]; 
                        ...
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            foreach (DataColumn dc in dt.Columns)
                            {
                                ...
                                if (val == null)
                                    continue;
                                if (dc.ColumnName.ToLower().Equals("flowno"))
                                    val = fl.No;
                                syncData.SetValByKey(dc.ColumnName, val);
                            }
                            String mypk = syncData.MyPK;
                            String newMyPK = DBAccess.GenerGUID(0, null, null);
                            //判断是否有SyncDataField
                            foreach (DataRow drr in fieldDt.Rows)
                            {
                                String refPKVal = drr["RefPKVal"].ToString();
                                if (refPKVal.Equals(mypk) == false)
                                    continue;
                                ...
                                foreach (DataColumn dc in dt.Columns)
                                {
                                    ...
                                    if (val == null)
                                        continue;
                                    if (dc.ColumnName.ToLower().Equals("refpkval"))
                                        val = mypk;
                                    if (dc.ColumnName.ToLower().Equals("flowno"))
                                        val = fl.No;
                                    syncDataField.SetValByKey(dc.ColumnName, val);
                                }
                                syncDataField.setMyPK(syncDataField.RefPKVal + "_" + syncDataField.AttrKey);
                                syncDataField.Insert();
                                continue;
                            }
                            syncData.setMyPK(newMyPK);
                            syncData.Insert();
                        }
                        break;
                    ...
                    case "WF_FrmNode": //Conds.xml。
                        ...
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            // 开始插入。
                            if (DataType.IsNullOrEmpty( fn.FrmID))
                                continue;
                            fn.setMyPK(fn.FrmID + "_" + fn.NodeID+"_" + fl.No);
                            try
                            {
                                fn.Insert();
                            }catch
                    ...
                    case "WF_Cond": //Conds.xml。
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            if(cd.CondType!=CondType.Flow)
                                cd.RefPKVal=fl.No + "_" + cd.NodeID + "_" + cd.ToNodeID;
                            if (cd.CondType == CondType.Flow)
                                cd.RefPKVal = cd.NodeID.ToString();
                            cd.FlowNo = fl.No;
                            int specOperWay = (int)cd.SpecOperWay;
                            if (specOperWay == 10 || specOperWay == 11 || specOperWay == 24)
                            {
                                cd.SetValByKey(CondAttr.SpecOperPara, flowID + cd.SpecOperPara.Substring(iOldFlowLength));
                            }
                    ...
                    case "Sys_MapExt":
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            foreach (DataColumn dc in dt.Columns)
                            {
                                ...
                                {
                                    text = dr[dc.ColumnName] == null ? "" : dr[dc.ColumnName] as string;
                                    continue;
                                }
                                val = val.Replace("ND" + oldFlowID, "ND" + flowID);
                                md.SetValByKey(dc.ColumnName, val);
                            ...
                            //md.InitPK();
                            md.Save(); //执行保存.
                            if (md.ExtType.Equals("HtmlText") == true && DataType.IsNullOrEmpty(text) == false)
                            {
        ...
        public static Node NewNode(string flowNo, int x, int y, string icon = null, int runModel = 0, int nodeType = 0)
        {
            ...
            try
            {
                ...
                nd.FormType = NodeFormType.FoolForm; //设置为经典表单.
                ...
                if (flow.FlowDevModel == FlowDevModel.JiJian)
                {
                    ...
                    FrmNode fn = new FrmNode();
                    fn.FrmID = "ND" + int.Parse(flow.No) + "01";
                    fn.IsEnableFWC = FrmWorkCheckSta.Enable;
                    fn.NodeID = nd.NodeID;
                    fn.FlowNo = flowNo;
                    fn.FrmSln = FrmSln.Readonly;
                    fn.setMyPK(fn.FrmID + "_" + fn.NodeID + "_" + fn.FlowNo);
                    //执行保存.
                    fn.Save();
                }
                ...
                {
                    nd.FormType = NodeFormType.FoolTruck; //设置为经典表单.
                ...
                {
                    nd.FormType = NodeFormType.RefOneFrmTree; //设置为经典表单.
                    nd.NodeFrmID = flow.FrmUrl;
                    nd.FrmWorkCheckSta = FrmWorkCheckSta.Enable;
                    nd.DirectUpdate();
                    FrmNode fn = new FrmNode();
                    fn.FrmID = nd.NodeFrmID;
                    fn.IsEnableFWC = FrmWorkCheckSta.Enable;
                    fn.NodeID = nd.NodeID;
                    fn.FlowNo = flowNo;
                    fn.FrmSln = FrmSln.Readonly;
                ...
                // 如果是绑定EntityTS
                if (flow.FlowDevModel == FlowDevModel.EntityTS)
                {
                    nd.FormType = NodeFormType.EntityTS; //设置为经典表单.
                    nd.FormUrl = flow.FrmUrl;
                    nd.FrmWorkCheckSta = FrmWorkCheckSta.Enable;
                    nd.DirectUpdate();
                    FrmNode fn = new FrmNode();
                    fn.FrmID = flow.FrmUrl;
                    fn.IsEnableFWC = FrmWorkCheckSta.Enable;
                    fn.NodeID = nd.NodeID;
                    fn.FlowNo = flowNo;
                    fn.FrmSln = FrmSln.Readonly;
        ...
        private static void CreatePushMsg(Node nd)
        {
            ...
            pm.Delete(PushMsgAttr.NodeID, nd.NodeID); // 删除已经存在的.

            //设置节点发送成功消息提醒.
            pm.EventNo = EventListNode.SendSuccess;
            pm.NodeID = nd.NodeID;
            pm.FlowNo = nd.FlowNo;
            pm.PushWayNo = "TodoEmps";  // 发送短消息.
            ...
            pm.FlowNo = nd.FlowNo;
            pm.PushWayNo = "TodoEmps";  // 发送短消息.
          //  pm.IsEnableEmail = 1; //不发送邮件消息.
            pm.setMyPK(DBAccess.GenerGUID());
            pm.Insert();

            // 设置撤销消息提醒.
            pm.EventNo = EventListNode.UndoneAfter;
            pm.NodeID = nd.NodeID;
            pm.FlowNo = nd.FlowNo;
            pm.PushWayNo = "TodoEmps";  // 发送短消息.
            pm.setMyPK(DBAccess.GenerGUID());
        ...
        string ptable, string flowMark)
        {
            ...
            try
            {
                ...
                flow.PTable = ptable; // ("PTable", ptable);
