...
namespace BP.WF.Template
{
    ...
    public class PushMsgAttr
    {
        ...
        public const string IsEnableSMS = "IsEnableSMS";
        /// <summary>
        /// 短信字段
        /// </summary>
        ...
        /// 推送方式
        /// </summary>
        public const string PushWayNo = "PushWayNo";
        public const string PushWayName = "PushWayName";
        /// <summary>
        /// 短消息发送设置
        /// </summary>
        ...
        /// 邮件推送方式
        /// </summary>
        public const string IsEnableEmail = "IsEnableEmail";
        /// <summary>
        /// 推送邮件的节点s
        /// </summary>
    ...
    public class PushMsg : EntityMyPK
    {
            ...
            get
            {
                ...
                switch (this.EventNo)
                {
                    ...
                    case EventListFlow.FlowOverAfter: //加签.
                        ...
                        break;
                    case EventListFlow.FlowOnCreateWorkID:
                        str = "流程{{Title}}已创建实例,处理人@WebUser.No,@WebUser.Name";
                        break;
                    default:
                        throw new Exception("@该事件类型没有定义默认的消息模版:" + this.EventNo);
                        break;
            ...
            string jumpToEmps = null)
        {
            ...
            {
                //openWorkURl = hostUrl + "/App/Portal/GuideWeiXin.aspx?DoType=OpenWork&WorkID=" + workid + "&FlowNo=" + currNode.FlowNo + "&GUID=" + WebUser.SID;
                openWorkURl = "";
            }
            else
        ...
        private string SendMessage(string title, Entity en, Node currNode, Int64 workid, string jumpToEmps, string openUrl, SendReturnObjs objs, Row r)
        {
            ...
            string atParas = "@FlowNo=" + currNode.FlowNo + "@WorkID=" + workid + "@NodeID=" + currNode.NodeID + "@FK_Node=" + currNode.NodeID;
            ...
            smsDoc = smsDoc.Replace("{FlowName}", currNode.FlowName);
            smsDoc = smsDoc.Replace("{NodeName}", currNode.Name);
            ...
            if (this.EventNo == BP.Sys.EventListNode.ReturnAfter)
            {
                //获取退回原因
                Paras ps = new Paras();
                if (DataType.IsNullOrEmpty(this.FlowNo))
                    this.FlowNo = r.GetValStrByKey("FlowNo");
            ...
            #region 表单字段作为接受人
            if (this.PushWayNo.Equals("Field") == true)
            {
                /*从字段里取数据. */
                ...
                //发送消息
                BP.WF.Dev2Interface.Port_SendMessage(toEmp, smsDoc, mailTitle, this.EventNo, "WKAlt" + currNode.NodeID + "_" + workid, BP.Web.WebUser.No, openUrl, this.SMSPushModel, workid, null, atParas);
                return "@已向:{" + toEmp + "}发送提醒信息.";
            }
            #endregion 表单字段作为接受人

            #region 如果发送给指定的节点处理人,就计算出来直接退回,任何方式的处理人都是一致的.
            if (this.PushWayNo.Equals("SpecEmpNo") == true)
            {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ThreadPool.QueueUserWorkItem(obj =>
                        {
                            try
                            {
                                ...
                                string paras = "@FlowNo=" + this.FlowNo + "@WorkID=" + workid + "@NodeID=" + this.NodeID + "_" + empNo;

                                //发送消息.
                                BP.WF.Dev2Interface.Port_SendMessage(empNo, smsDocReal, mailTitle, this.EventNo, "WKAlt" + currNode.NodeID + "_" + workid, BP.Web.WebUser.No, openUrl, this.SMSPushModel, workid, null, atParas);
                                //处理短消息.
                                toEmpIDs += empName + ",";
                            }
                            catch (Exception ex)
            ...
            #region 按照SQL计算
            if (this.PushWayNo.Equals("BySQL") == true)
            {
                ...
                bySQL = bySQL.Replace("@WebUser.DeptNo", WebUser.DeptNo);
            ...
            #region 发送给流程发起人
            if (this.PushWayNo.Equals("Starter") == true)
            {
                ...
                //发送消息
                BP.WF.Dev2Interface.Port_SendMessage(gwf.Starter, smsDocReal, mailTitle, this.EventNo, "WKAlt" + currNode.NodeID + "_" + workid, BP.Web.WebUser.No, openUrl, this.SMSPushModel, workid, null, atParas);
                //处理短消息.
            ...
            if ((this.EventNo == BP.Sys.EventListNode.WorkArrive
                || this.EventNo == BP.Sys.EventListNode.ReturnAfter
                || this.EventNo == BP.Sys.EventListNode.ShitAfter
                || this.EventNo == BP.Sys.EventListNode.UndoneAfter)
            ...
            #region 节点发送成功后
            if (this.EventNo.Equals(BP.Sys.EventListNode.SendSuccess) && objs != null && objs.VarAcceptersID != null)
            {
                /*如果向接受人发送消息.*/
                ...
                //    smsDocReal = smsDocReal.Replace("{EmpStr}", empNo);
                //    openUrl = openUrl.Replace("{EmpStr}", empNo);
                //    BP.WF.Dev2Interface.Port_SendMessage(empNo, smsDocReal, mailTitle, this.EventNo, "WKAlt" + objs.VarToNodeID + "_" + workid, BP.Web.WebUser.No, openUrl, this.SMSPushModel, workid, null, atParas);
                //}
                /* 下面代码不可行。 */
                CountdownEvent cdEvent = new CountdownEvent(empList.Count);
                empList.ForEach(emp =>
            ...
            #region 流程结束后、流程删除后
            if (this.EventNo == BP.Sys.EventListFlow.FlowOverAfter
                || this.EventNo == BP.Sys.EventListFlow.AfterFlowDel)
            {
                /*向所有参与人发送消息. */
                ...
                empList.ForEach(emp =>
                {
                    ...
                    ThreadPool.QueueUserWorkItem(o =>
                    {
                        ...
                        try
                        {
                            ...
                            openUrl = openUrl.Replace("{EmpStr}", empNo);
                            //发送消息
                            BP.WF.Dev2Interface.Port_SendMessage(empNo, smsDoccReal, mailTitle, this.EventNo, "WKAlt" + currNode.NodeID + "_" + workid, BP.Web.WebUser.No, openUrl, this.SMSPushModel, workid, null, atParas);
                        }
                        catch (Exception ex)
                        {
                            BP.DA.Log.DebugWriteError("发送消息给[" + empNo + "]失败" + ", 原因：" + ex.Message);
            ...
            #region 节点预警、逾期
            if (this.EventNo == BP.Sys.EventListNode.NodeWarning
                || this.EventNo == BP.Sys.EventListNode.NodeOverDue)
            {
                //获取当前节点的接收人
                GenerWorkFlow gwf = new GenerWorkFlow(workid);
                ...
                empList.ForEach(empNo =>
                {
                    ...
                    ThreadPool.QueueUserWorkItem(o =>
                    {
                        ...
                        try
                        {
                            ...
                            BP.WF.Dev2Interface.Port_SendMessage(empA[0], smsDocReal, mailTitle, this.EventNo, "WKAlt" + currNode.NodeID + "_" + workid, BP.Web.WebUser.No, openUrl, this.SMSPushModel, workid, null, atParas);
                        }
                        catch (Exception ex)
                        {
            ...
            return "";
            //return "err@没有判断的情况...."+title+","+en.ClassID;
        }
        /// <summary>
        /// 发送短信到其它节点的处理人.
        /// </summary>
        ...
        {
            //  this.setMyPK(this.EventNo + "_" + this.NodeID + "_" + this.PushWay;


            //  string sql = "UPDATE WF_PushMsg SET FlowNo=(SELECT FlowNo FROM WF_Node WHERE NodeID= WF_PushMsg.NodeID)";
            // DBAccess.RunSQL(sql);
    ...
    public class PushMsgs : EntitiesMyPK
    {
        ...
        public PushMsgs(string flowNo)
        {
            ...
            this.Retrieve(PushMsgAttr.FlowNo, flowNo);
        }
        /// <summary>
        /// 消息推送
        /// </summary>
        ...
        public PushMsgs(int nodeid)
        {
            ...
            this.Retrieve(PushMsgAttr.NodeID, nodeid);
        }
        /// <summary>
        /// 得到它的 Entity 
        /// </summary>
