...
namespace BP.CCOA.HttpHandler
{
    ...
    public class CCOA_Knowledge : DirectoryPageBase
    {
        public string Knowledge_UploadFile_MinAth()
        {
            ...
            // 获取文件集合
            Microsoft.AspNetCore.Http.IFormFileCollection hfc = HttpContextHelper.RequestFiles();
            // 获取DataUser目录路径
            string basePath = BP.Difference.SystemConfig.PathOfDataUser;

            for (int i = 0; i < hfc.Count; i++)
            {
                Microsoft.AspNetCore.Http.IFormFile file = hfc[i];
                ...
                // 保存文件到指定路径
                HttpContextHelper.UploadFile(file, savePath);
                ...
                dbUpload.SetValByKey(FrmAttachmentDBAttr.FileSize, file.Length / 1024.0);
