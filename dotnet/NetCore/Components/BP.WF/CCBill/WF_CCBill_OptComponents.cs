    ...
    public class WF_CCBill_OptComponents : DirectoryPageBase
    {
        ...
        public string DataVer_AppFieldData()
        {
            try
            {
                ...
                BP.CCBill.Dev2Interface.WriteTrack(this.FrmID, this.WorkID.ToString(), FrmActionType.DataVerReback, msg);
                return msg;
            }
            catch (Exception ex)
            {
        ...
        public string DataVer_Reback()
        {
            ...
            BP.CCBill.Dev2Interface.WriteTrack(this.FrmID, this.WorkID.ToString(), FrmActionType.DataVerReback, "数据回滚");
            return "已经成功还原...";
        }
        ...
        public string DataVer_NewVer()
        {
            ...
            BP.CCBill.Dev2Interface.WriteTrack(this.FrmID, this.WorkID.ToString(), FrmActionType.DataVerReback, "创建数据版本.");
            return "创建成功.";
        ...
        public string QRCode_Init_Vue3()
        {
            string workIDStr = this.GetRequestVal("WorkID");
            string menuNo = this.GetRequestVal("MenuNo");

            //string url = BP.Difference.SystemConfig.HostURL + "/WF/CCBill/OptComponents/QRCodeScan.htm?DoType=MyDict&WorkID=" + workIDStr + "&FrmID=" + this.FrmID + "&MethodNo=" + this.GetRequestVal("MethodNo") + "&IsReadonly=" + GetRequestValInt("IsReadonly");
            String url = BP.Difference.SystemConfig.HostURLVue3 + "/#/CCMobile/ScanGuide?DoType=MyDict&WorkID=" + workIDStr + "&FrmID=" + this.FrmID + "&MethodNo=" + this.GetRequestVal("MethodNo") + "&MenuNo=" + menuNo;
            QRCodeEncoder encoder = new QRCodeEncoder();
            encoder.QRCodeEncodeMode = QRCodeEncoder.ENCODE_MODE.BYTE;//编码方式(注意：BYTE能支持中文，ALPHA_NUMERIC扫描出来的都是数字)
            encoder.QRCodeScale = 4; //大小(值越大生成的二维码图片像素越高).
            encoder.QRCodeVersion = 0; //版本(注意：设置为0主要是防止编码的字符串太长时发生错误)
        ...
        public string FrmBBs_UploadFile()
        {
            ...
            //上传附件
            string filepath = "";
            //IFormFile file = BP.Difference.HttpContextHelper.RequestFiles(0);
