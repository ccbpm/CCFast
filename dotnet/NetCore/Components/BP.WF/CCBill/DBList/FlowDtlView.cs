...
namespace BP.CCBill
{
    ...
    /// 数据源实体
    /// </summary>
    public class FlowDtlView : EntityNoName
    {
        #region 权限控制.
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString("FlowNo", null, "FlowNo", true, true, 0, 10, 20);
        ...
        public string CheckIt()
        {
            ...
            {
                #region 0.变量定义.
                string frmID = "ND" + int.Parse(this.GetValStringByKey("FlowNo")) + "01";  //主表ID.
                string dtlID = this.GetValStringByKey("DictDtlID"); //从表ID.

                MapData md = new MapData(frmID);
                MapAttrs mdAttrs = new MapAttrs("ND" + int.Parse(this.GetValStringByKey("FlowNo")) + "Rpt");
                ...
                string select = "SELECT B.OID,B.OID BillNo, A.Title, A.FlowStarter as Starter,'' StarterName, A.FK_Dept, A.FK_Dept OrgNo,A.FlowStartRDT as RDT, A.WFState as BillState,A.AtPara,A.FID ";
                string from = " FROM " + md.PTable + " A, " + dtl.PTable + " B ";
                string where = " WHERE A.OID=B.RefPK AND A.WFState >1 ";
                #endregion 0.变量定义.
                ...
                foreach (string str in strs)
                {
                    ...
                    if (attr == null)
                    {
                        if (str.Equals("BillState") == true)
                        {
                            MapAttr temp = new MapAttr();
                            temp.Name = "BillState";
                            temp.setKeyOfEn("BillState");
                            temp.MyDataType = DataType.AppInt;
                            temp.SetValByKey("FK_MapData", this.No);
                            temp.setMyPK(temp.FrmID + "_" + temp.KeyOfEn);
                            temp.setUIVisible(false);
                            temp.DirectInsert();
                            continue;
                        }
