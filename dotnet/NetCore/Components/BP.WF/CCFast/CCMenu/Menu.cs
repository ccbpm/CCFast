...
namespace BP.CCFast.CCMenu
{
    ...
    public class Menu : EntityNoName
    {
        ...
        protected override void afterDelete()
        {
            string sql = "";
            #region 删除实体。
            if (this.MenuModel.Equals("Dict") == true || this.MenuModel.Equals("EntityNoName") == true || this.MenuModel.Equals("DBList") == true || this.MenuModel.Equals("Bill") == true)
            {
                ...
                //删除集合方法.
                //删除实集合方法对应的权限功能
                sql = "DELETE FROM GPM_PowerCenter WHERE CtrlPKVal IN(SELECT No FROM Frm_Collection WHERE FrmID='" + frmID + "')";
                DBAccess.RunSQL(sql);

                //删除流程相关的数据
                sql = "SELECT FlowNo FROM Frm_Collection WHERE FrmID='" + frmID + "' AND MethodModel IN('FlowNewEntity','FlowEntityBatchStart')";
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    flow.No = dr[0].ToString();
                    int i = flow.RetrieveFromDBSources();
                    if (i == 1)
                        flow.DoDelete();
                    Menu menu = new Menu();
                    menu.No = frmID + "_" + flow.No;
                    menu.Delete();
                ...
                //删除实体组件.
                //先删除实体组件中方法对应的MapAttr
                sql = "DELETE FROM Sys_MapAttr WHERE FK_MapData IN(SELECT No FROM Frm_Method WHERE FrmID='" + frmID + "' AND MethodModel='Func')";
                DBAccess.RunSQL(sql);

                //删除流程相关的数据
                sql = "SELECT MethodID FROM Frm_Method WHERE FrmID='" + frmID + "' AND MethodModel IN('FlowBaseData','FlowEtc')";
                ...
                //删除实体组件对应的权限功能
                sql = "DELETE FROM GPM_PowerCenter WHERE CtrlPKVal IN(SELECT No FROM Frm_Method WHERE FrmID='" + frmID + "')";
                DBAccess.RunSQL(sql);
                ...
                //删除权限
                sql = "DELETE FROM Frm_DBRole WHERE FrmID='" + frmID + "'";
                DBAccess.RunSQL(sql);
