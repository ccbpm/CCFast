    ...
    public class FlowCheckError
    {
        ...
        public string DoCheck()
        {
            ...
            try
            {
                ...
                {
                    this.AddMsgError("检查Rpt表", "@错误:表单枚举,外键字段UIBindKey信息丢失,请描述该字段的设计过程，反馈给开发人员,并删除错误字段重新在表单上创建。错误字段信息如下:"+ str, null);
                }
                ...
                //检查是否是公文流程?
                sql = "SELECT COUNT(NodeID) as Num FROM WF_Node WHERE OfficeBtnEnable > 0 AND FK_Flow='" + flow.No + "' ";
                //if (DBAccess.RunSQLReturnValInt(sql, 0) == 0)
                //    this.flow.ItIsGovFlow = false;
                //else
                //    this.flow.ItIsGovFlow = true;
                this.flow.Update();
        ...
        /// 检查节点节点批处理审核, 设置是否正确.
        /// </summary>
        public void CheckMode_BatchRole()
        {
            //判断是否启用了批处理功能.
            foreach (Node nd in nds)
            {
                if (nd.HisBatchRole == BatchRole.WorkCheckModel)
                {
                    if (nd.HisDeliveryWay == DeliveryWay.BySelected)
                    {
                        this.AddMsgError("节点批处理设置", "设置节点批处理规则时，对应节点的接收人规则不能是让客户自由海选，请检查重新设置接收人规则。", nd);
                    }
                }
            }
        ...
        public void CheckMode_Gener()
        {
            ...
            foreach (Node nd in nds)
            {
                ...
                string mapDataNo = "ND" + nd.NodeID;
                if (nd.HisFormType == NodeFormType.RefOneFrmTree || nd.HisFormType == NodeFormType.RefNodeFrm)
                    mapDataNo = nd.NodeFrmID;
                if (nd.HisFlow.FlowDevModel == FlowDevModel.JiJian && nd.ItIsStartNode == false)
                    mapDataNo = nd.NodeFrmID;
                ...
                if (nd.HisNodeType == NodeType.UserNode)
                {
                    switch (nd.HisDeliveryWay)
                    {
                        ...
                        case DeliveryWay.BySQLAsSubThreadEmpsAndData:
                            ...
                            if (sql.Contains("@"))
                            {
                                this.AddMsgWarning("接收人规则", "您编写的sql变量填写可能不正确，实际执行中可能没有被完全替换下来，确认是否是系统变量.:" + sql, nd);
                                continue;
                            }
        ...
        public void CheckMode_Frms()
        {
            ...
            {
                if (DataType.IsNullOrEmpty(item.FrmID) == true)
                {
                    item.Delete();
                    continue;
                }

                if (frms.Contains(item.FrmID + ","))
                    continue;
                Node nd = new Node(item.NodeID);
                //删除垃圾数据
                if (nd.HisFormType == NodeFormType.RefOneFrmTree && nd.NodeFrmID.Equals(item.FrmID) == false)
                {
                    item.Delete();
                    continue;
                }
                ...
                if (nd.HisFormType != NodeFormType.EntityTS)
                {
                    MapData md = new MapData();
                    md.No = item.FrmID;
                    if (md.RetrieveFromDBSources() == 0)
                    {
                        this.AddMsgError("绑定表单库的表单", "节点绑定的表单ID=" + item.FrmID + "，但该表单已经不存在.", new Node(item.NodeID));
                        continue;
                    }
                    md.ClearCache();
                }
        ...
        public void CheckMode_SpecTable()
        {
            foreach (Node nd in nds)
            {
                ...
                {
                    if (nd.ItIsSubThread == true && md.PTable.Equals(this.flow.PTable) == false)
                    {
