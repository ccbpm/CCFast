...
namespace BP.WF
{
    ...
    public class Node : Entity
    {
        ...
        public string ARDeptModelDeptsSQL(Int64 workID)
        {
            ...
            switch (this.ARDeptModel)
            {
                case 0: //提交人所有的部门.
                    ...
                    break;
                case 1: //提交人登录部门.
                    sql = "SELECT FK_Dept FROM Port_DeptEmp WHERE FK_Emp='" + empNo + "' AND FK_Dept='" + WebUser.DeptNo + "'";
                    break;
                case 2: //提交人使用部门.
        ...
        public PushMsgs HisPushMsgs
        {
            get
            {
                ...
                if (obj == null)
                {
                  ...
                  PushMsgAttr.NodeID, this.NodeID);
                    ...
                    if (BP.Difference.SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
                    {
                        ...
                        {
                            if (item.EventNo == EventListNode.SendSuccess)
                                isHaveSend = true;

                            if (item.EventNo == EventListNode.ReturnAfter)
                                isHaveReturn = true;
                        }
                        ...
                        if (isHaveSend == false)
                        {
                            ...
                            pm.EventNo = EventListNode.SendSuccess;
                            pm.PushWayNo = "TodoEmps"; //*默认:让其使用短消息提醒.
                        ...
                        {
                            PushMsg pm = new PushMsg();
                            pm.EventNo = EventListNode.ReturnAfter;
                            pm.PushWayNo = "TodoEmps";  //*默认:让其使用短消息提醒. 
                            pm.SMSPushModel = "Email";
                            obj.AddEntity(pm);
                        }
        ...
        public FrmEvents FrmEvents
        {
            get
            {
                Entities ens = this.GetEntitiesAttrFromAutoNumCache(new FrmEvents(),
                    FrmEventAttr.RefPKVal, this.NodeID.ToString());
                //如果没有找到，就修改，写入数据的方法。 
                //if(ens.Count == 0)
                //{
                //    FrmEventAttr.RefPKVal, this.NodeID.ToString());
        ...
        public string Name
        {
            get
            {
                return this.GetValStrByKey(EntityNoNameAttr.Name);
            }
            set
            {
            ...
            get
            {
                switch (this.HisDeliveryWay)
                {
                    ...
                    case DeliveryWay.ByGuest:
                        return "按外部用户计算";
                    case DeliveryWay.ByMLeader0:
                        return "连续多级主管 - 指定角色模式 模式";
                    case DeliveryWay.ByMLeader1:
                        return "连续多级主管 - 指定指定通讯录等级 模式";
                    ...
                    case DeliveryWay.ByPreviousNodeFormEmpsField:
                        return "按字段是主表";
                    case DeliveryWay.ByPreviousNodeFormEmpsFrmDtl:
                        return "按字段是从表";
                    case DeliveryWay.ByPreviousNodeFormEmpsTeam:
                        return "按字段是权限组";
                    ...
                    case DeliveryWay.ByTeamOrgOnly:
                        return "按照用户组计算(本组织范围内)";
                    case DeliveryWay.ByTeamOnly:
                        return "按照用户组计算(全集团)";
                    case DeliveryWay.ByTeamDeptOnly:
                        return "按照用户组计算(本部门范围内)";
                    case DeliveryWay.BySelectedEmpsOrgModel:
                        return "按照组织模式人员选择器";
                    case DeliveryWay.BySelfUrl:
                        return "按照自定义Url模式的人员选择器";
                    case DeliveryWay.BySenderParentDeptLeader:
                        return "按照发送人的上级部门的负责人计算";
                    case DeliveryWay.BySenderParentDeptStations:
                        return "发送人上级部门指定的角色";
                    case DeliveryWay.ByStationSpecDepts:
                        return "按指定的部门集合与设置的岗位交集";
                    case DeliveryWay.ByStationSpecStas:
                        return "按指定的角色集合与设置的部门交集";
                    case DeliveryWay.ByDeptSpecer:
                        return "按照分管领导计算";
                    ...
                    case DeliveryWay.ByCCFlowBPM:
                        return "按照ccflow的BPM模式处理";
                    case DeliveryWay.PreplaceWokerFree:
                        return "预置:自由选择";
                    case DeliveryWay.PreplaceWokerFix:
                        return "预置:固定模式";
                    default:
                        break;
                }
                return this.HisDeliveryWay.ToString();
            }
        }
        /// <summary>
        /// 考核规则
        ...
        protected override bool beforeDelete()
        {
            ...
            //}
            // 判断是否可以被删除. 
            num = DBAccess.RunSQLReturnValInt("SELECT COUNT(*) FROM WF_GenerWorkerlist WHERE FK_Node=" + this.NodeID + " AND IsPass=0 ", 0);
            if (num != 0)
            ...
            // DBAccess.RunSQL("DELETE FROM WF_CH WHERE DeptNo=" + this.NodeID);
        ...
        public string RepareMap(Flow fl)
        {
            ...
            if (attrs.Contains(MapAttrAttr.KeyOfEn, GERptAttr.AtPara, MapAttrAttr.FK_MapData, md.No) == false)
            {
                attr = new BP.Sys.MapAttr();
