...
namespace BP.WF
{
    ...
    public class GenerWorkFlowAttr : EntityNoNameAttr
    {
        ...
        /// 目录路径.
        /// </summary>
        public const string SortPath = "SortPath";
        /// <summary>
        /// 发起人名称
        /// </summary>
        ...
        /// 域/系统编号
        /// </summary>
        public const string DomainExt = "DomainExt";
    ...
    public class GenerWorkFlow : Entity
    {
        ...
        public string Sender
        {
            ...
            set
            {
                ...
                //只要name或者只要no的情況下标准格式为张三;
                if (BP.WF.Glo.UserInfoShowModel.ToString().Equals(UserInfoShowModel.UserNameOnly.ToString()) ||
                    BP.WF.Glo.UserInfoShowModel.ToString().Equals(UserInfoShowModel.UserIDOnly.ToString()))
                {
                    if (!value.Contains(";"))
                    {
        ...
        public string FlowSortNo
        {
            ...
            set
            {
                ...
                try
                {
                    string val = DBAccess.RunSQLReturnStringIsNull("SELECT SortPath FROM WF_FlowSort WHERE No='" + value + "'", null);
                    SetValByKey(GenerWorkFlowAttr.SortPath, val);
                }
                catch
                {
                    FlowSort sort = new FlowSort(value);
        ...
        public bool IsOver
        {
            get
            {
                if (this.WFState == WFState.CompleteByNode || this.WFState == WFState.CompleteEnd)
                    return true;
                return false;
            }
        }
        ...
        public WFState WFState
        {
            ...
            set
            {
                if (value == BP.WF.WFState.CompleteEnd || value == BP.WF.WFState.CompleteByNode)
        ...
        public string WFStateText
        {
            get
            {
                ...
                switch (ws)
                {
                    case WF.WFState.CompleteEnd:
                        return "已完成";
                    case WF.WFState.CompleteByNode:
                        return "已完成(中间节点)";
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(GenerWorkFlowAttr.SortPath, null, "目录路径", true, false, 0, 100, 10);
        ...
        public string RemoveEmps(string empNo, string note)
        {
            try
            {
                BP.WF.Dev2Interface.Node_RemoveTodolist(this.WorkID, empNo);
                return "移除成功.";
            }
            catch (Exception ex)
            {
        ...
        {
            /*string sql = "SELECT * FROM ND" + int.Parse(this.FlowNo) + "Track WHERE WorkID=" + this.WorkID + "  AND ActionType IN(0,1) ORDER BY RDT";
            DataTable dt = DBAccess.RunSQLReturnTable(sql);*/
            String sql = "SELECT WorkID,FK_Emp,B.Name AS FK_EmpText,FK_Node,'" + this.NodeName + "' AS FK_NodeText,RDT,IsPass From WF_GenerWorkerList A,Port_Emp B WHERE A.FK_Emp=B.No AND A.WorkID=" + this.WorkID + " AND A.FK_Node=" + this.NodeID;
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
            if (SystemConfig.AppCenterDBFieldCaseModel == FieldCaseModel.UpperCase)
            {
                dt.Columns["WORKID"].ColumnName = "WorkID";
                dt.Columns["FK_EMP"].ColumnName = "FK_Emp";
                dt.Columns["FK_EMPTEXT"].ColumnName = "FK_EmpText";
                dt.Columns["FK_NODE"].ColumnName = "FK_Node";
                dt.Columns["FK_NODETEXT"].ColumnName = "FK_NodeText";
                dt.Columns["RDT"].ColumnName = "RDT";
                dt.Columns["ISPASS"].ColumnName = "IsPass";
            }
        ...
        public string DoRollback(string nodeID, string note, string empNo)
        {
            try
            {
                return BP.WF.Dev2Interface.Flow_DoRebackWorkFlow(this.FlowNo, this.WorkID, int.Parse(nodeID), note, empNo);
            }
            catch (Exception ex)
            {
                return "err@" + ex.Message;
