...
namespace BP.WF
{
    ...
    public class Flow : BP.En.EntityNoName
    {
        ...
        public string DTSSpecNodes
        {
            get
            {
                return this.GetParaString("DTSSpecNodes");
            }
            set
            {
                this.SetPara("DTSSpecNodes", value);
            }
        }
        /// <summary>
        /// 同步方式
        /// </summary>
        public DTSWay DTSWay
        {
            get
            {
                return (DTSWay)this.GetValIntByKey(FlowAttr.DTSWay);
            }
            set
            {
        ...
        /// 是不是公文流程? 参数字段.
        /// </summary>
        public bool ItIsGovFlow
        {
            get
            {
                return this.GetParaBoolen("IsGovFlow", false);
            }
            set
            {
                this.SetPara("IsGovFlow", value);
            }
        }
        ...
        public Work NewWork(Emp emp, Hashtable paras)
        {
            ...
            if (CCFlowAPI.CheckIsCanStartFlow_InitStartFlow(this) == false)
                throw new Exception("info@流程已经启动.");
            ...
            try
            {
                ...
                {
                    /* 说明没有空白,就创建一个空白..*/
                    if (nd.FormType == NodeFormType.FoolForm || nd.FormType == NodeFormType.Develop || nd.FormType == NodeFormType.FoolTruck)
                    {
                        wk.ResetDefaultVal();
                    }
                    else
                    {
                        wk.ResetDefaultVal(wk.NodeFrmID, this.No, wk.NodeID);
                    }
                ...
                if (wk.OID != 0)
                {
                    ...
                    if (i == 0)
                    {
                        ...
                        ap.SetVal("RDT", DataType.CurrentDateTimess);
                        rpt.AtPara = ap.GenerAtParaStrs();

                        if (rpt.EnMap.PhysicsTable.Equals(wk.EnMap.PhysicsTable))
                        {
                            rpt.SetValByKey("OID", wk.OID);
                            rpt.Update();
                        }
                        else
            ...
            {
                /* 如果需要从上一个流程实例上copy数据. */
                string sql = "SELECT OID FROM " + this.PTable + " WHERE FlowStarter='" + WebUser.No + "' AND OID!=" + wk.OID + " ORDER BY RDT DESC";
            ...
            if (DataType.IsNullOrEmpty(PNodeIDStr) == false && DataType.IsNullOrEmpty(PWorkIDStr) == false)
            {
                ...
                if (dtls.Count > 0)
                {
                    ...
                    if (dtlsFrom.Count == dtls.Count)
                    {
                        foreach (MapDtl dtl in dtls)
                        {
                            ...
                            MapExts dtlMapExts = new MapExts(dtlFrom.No);
                            ...
                            foreach (GEDtl geDtlFromData in dtlsFromData)
                            {
                                ...
                                #region 处理从表中的搜索多选
                                foreach (MapExt en in dtlMapExts)
                                {
                                    //搜索多选
                                    if (en.ExtType.Equals("MultipleChoiceSearch"))
                                    {
                                        try
                                        {
                                            //上一笔数据
                                            string refPK = geDtlFromData.OID.ToString();
                                            // 新生成的数据
                                            string newRefPK = dtlData.OID.ToString();
                                            //查出上一笔多选的数据
                                            sql = "SELECT * FROM Sys_FrmEleDB WHERE FK_MapData=" + dtlFrom.No + " AND RefPKVal = " + refPK;
                                            DataTable frmEleDBs = DBAccess.RunSQLReturnTable(sql);
                                            //复制多选的数据
                                            foreach (DataRow frmEn in frmEleDBs.Rows)
                                            {
                                                FrmEleDB frmEleDB = new FrmEleDB();
                                                frmEleDB.Copy(frmEn);
                                                frmEleDB.RefPKVal = newRefPK;
                                                //重写MyPK
                                                frmEleDB.MyPK = frmEleDB.EleID + "_" + frmEleDB.FrmID + "_" + frmEleDB.RefPKVal + "_" + frmEleDB.Tag1;
                        ...
                        foreach (FrmNode fn in fns)
                        {
                            ...
                            foreach (FrmNode fnParent in fnsPearent)
                            {
                                if (fnParent.FrmEnableRole == FrmEnableRole.Disable)
                                    continue;
                                if (fn.FrmID.Equals(fnParent.FrmID) == false)
                                    continue;

                                //父流程表单.
                                BP.Sys.GEEntityOID geEnFrom = new GEEntityOID(fnParent.FrmID);
                ...
                #region 处理图片附件
                MapAttrs mapAttrs = new MapAttrs(wk.NodeFrmID);
                foreach (MapAttr en in mapAttrs)
                {
                    if (en.UIContralType == UIContralType.FrmImgAth)
                    {
                        //把上一笔数据的图片附件复制一份
                        ...
                        {
                            imgSrc = SystemConfig.PathOfDataUser + "/ImgAth/Data/" + en.KeyOfEn + "_" + CopyFormWorkID + ".png";
                            newImgSrc = SystemConfig.PathOfDataUser + "/ImgAth/Data/" + en.KeyOfEn + "_" + wk.OID + ".png";
                        }
                        else
                        {
                            imgSrc = SystemConfig.PathOfDataUser + "/ImgAth/Data/" + wk.NodeFrmID + "_" + en.KeyOfEn + "_" + CopyFormWorkID + ".png";
                            newImgSrc = SystemConfig.PathOfDataUser + "/ImgAth/Data/" + wk.NodeFrmID + "_" + en.KeyOfEn + "_" + wk.OID + ".png";
                        }
                        //判断是否上传了图片附件
                        if (File.Exists(imgSrc) == true)
                        {
                            //判断路径不为空
                            if (DataType.IsNullOrEmpty(imgSrc) == false && DataType.IsNullOrEmpty(newImgSrc) == false)
                            {
                                File.Copy(imgSrc, newImgSrc, true);
                            }
                        }
                ...
                #region 处理搜索多选
                MapExts mapExts = new MapExts(wk.NodeFrmID);
                foreach (MapExt en in mapExts)
                {
                    //搜索多选
                    if (en.ExtType.Equals("MultipleChoiceSearch"))
                    {
                        ...
                        frmEleDBs.Retrieve(FrmEleDBAttr.FK_MapData, wk.NodeFrmID, FrmEleDBAttr.RefPKVal, CopyFormWorkID);

                        //复制多选的数据
                        foreach (FrmEleDB frmEn in frmEleDBs)
                        {
                            FrmEleDB frmEleDB = new FrmEleDB();
                            frmEleDB.Copy(frmEn);
                            frmEleDB.RefPKVal = wk.OID.ToString();
                            //重写MyPK
                            frmEleDB.MyPK = frmEleDB.EleID + "_" + frmEleDB.FrmID + "_" + frmEleDB.RefPKVal + "_" + frmEleDB.Tag1;
                            if (frmEleDB.RetrieveFromDBSources() == 0)
                            {
            ...
            if (rpt.EnMap.Attrs.Contains("AtPara") == true)
                wk.SetValByKey(GERptAttr.AtPara, rpt.AtPara);
        ...
        {
            if (this.DTSWay == DTSWay.None)
                return "执行失败，您没有设置数据同步方案 (同步数据规则)，请您在 流程编号[" + this.No + "]，流程名称[" + this.Name + "]的 流程属性 -> 数据同步 -> 同步数据规则 中设置。";
        ...
        public DataSet GetFlow(string path)
        {
            ...
            dtFlow.Rows[0]["FlowMark"] = "";
            ds.Tables.Add(dtFlow);

            //增加同步信息
            SyncDatas syncDatas = new SyncDatas(this.No);
            ...
            syncDataFields.Retrieve(SyncDataFieldAttr.FlowNo, this.No);
            DataTable syncDataFieldDt = syncDatas.ToDataTableField("WF_SyncDataField");
            ...
            flowOrgs.Retrieve(FlowOrgAttr.FlowNo, this.No);
            ...
            nodeWorkChecks.Retrieve(NodeAttr.FK_Flow, this.No, NodeAttr.Step);
            ...
            pushMsgs.Retrieve(PushMsgAttr.FlowNo, this.No);
            ...
            if (mexts.IsExits("ExtType", "HtmlText") == true)
            {
                mapDt.Columns.Add("HtmlText");
        ...
        private string DoCheck_CheckRpt(Nodes nds)
        {
            string msg = "";
            string fk_mapData = "ND" + int.Parse(this.No) + "Rpt";
            string flowNo = int.Parse(this.No).ToString();
            ...
            //求已经存在的字段集合。
            sql = "SELECT KeyOfEn FROM Sys_MapAttr WHERE FK_MapData='ND" + flowNo + "Rpt'";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                ma.setMyPK("ND" + flowNo + "Rpt_" + ma.KeyOfEn);
                ma.setFrmID("ND" + flowNo + "Rpt");
            ...
            if (DBAccess.AppCenterDBType == DBType.UX)
            ...
            DBAccess.RunSQL("UPDATE Sys_MapAttr SET Name='活动时间' WHERE FK_MapData='ND" + flowNo + "Rpt' AND KeyOfEn='CDT'");
            DBAccess.RunSQL("UPDATE Sys_MapAttr SET Name='参与者' WHERE FK_MapData='ND" + flowNo + "Rpt' AND KeyOfEn='Emps'");
            #endregion 尾后处理.
        ...
        public string BillNoFormat
        {
            get
            {
                string str = this.GetValStringByKey(FlowAttr.BillNoFormat);
                return str;
        ...
        public string FlowSortNo
        {
            ...
            set
            {
                if (DataType.IsNullOrEmpty(value) == true)
                {
                    ...
                    {
                        this.SetValByKey(FlowAttr.FK_FlowSort, WebUser.OrgNo);
                        return;
                    }
                }
        ...
        public FrmEvents FrmEvents
        {
            get
            {
                ...
                if (ens.Count == 0)
                {
                    ens = this.GetEntitiesAttrFromAutoNumCache(new FrmEvents(),
                    FrmEventAttr.RefPKVal, this.No);
                }
        ...
        public string DoDelData()
        {
            ...
            {
                if (strs.Contains("@" + nd.FrmID) == true)
                    continue;

                strs += "@" + nd.FrmID + "@";
                try
                {
                    MapData md = new MapData(nd.FrmID);
            ...
            if (DBAccess.IsExitsObject(this.PTable) && DataType.IsNullOrEmpty(this.FlowSortNo) == false)
                DBAccess.RunSQL("DELETE FROM " + this.PTable);

            if (DBAccess.IsExitsTableCol("WF_CH", "FlowNo"))
                DBAccess.RunSQL("DELETE FROM WF_CH WHERE FlowNo='" + this.No + "'");
        ...
        /// </summary>
        /// <returns></returns>
        public string ExpAllDataToDir()
        {
            string tempPath = SystemConfig.PathOfTemp + this.No + "." + this.Name + "/";

            GenerWorkFlows gwfs = new GenerWorkFlows();
            gwfs.Retrieve("FK_Flow", this.No);

            Node nd = new Node(int.Parse(this.No + "01"));
            string frmID = nd.NodeFrmID;

            MapDtls dtls = new MapDtls(frmID);
            foreach (GenerWorkFlow gwf in gwfs)
            {
                if (gwf.WFState == WFState.Blank || gwf.WFState == WFState.Draft)
                    continue;

                //1.处理目录.
                string dir = tempPath + "" + gwf.WorkID.ToString() + "/";
                ...
                //2. 生成 WF_GenerWorkFlow主文件.
                DataSet dsWorkFlow = new DataSet();
                dsWorkFlow.Tables.Add(gwf.ToDataTableField());
                if (gwf.WFState == WFState.CompleteEnd || gwf.WFState == WFState.CompleteByNode)
                {
                }
                else
                {
                    GenerWorkerLists gwls = new GenerWorkerLists();
                    gwls.Retrieve("WorkID", gwf.WorkID);
                    dsWorkFlow.Tables.Add(gwls.ToDataTableField());
                }
                dsWorkFlow.WriteXml(dir + "WF_GenerWorkFlow.xml"); //主表数据.

                //3.生成WF_Track.xml
                string sql = "SELECT * FROM ND"+int.Parse(this.No)+"Track WHERE WorkID="+gwf.WorkID+" ORDER BY RDT";
                DataSet dsTrack = new DataSet();
                dsTrack.Tables.Add(DBAccess.RunSQLReturnTable(sql));
                dsTrack.WriteXml(dir + "Track.xml"); //审核轨迹数据.

                //4.生成开始节点表单数据.
             
                GEEntityOID en=new GEEntityOID(frmID,gwf.WorkID);

                DataSet dsFrm = new DataSet();
                dsFrm.Tables.Add(en.ToDataTableField("MainTable"));
        ...
        protected override bool beforeInsert()
        {
            ...
            if (DataType.IsNullOrEmpty(this.FlowSortNo) == true)
            {
                string treeNo = "";
                if (SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
                    treeNo = WebUser.OrgNo;
                if (SystemConfig.CCBPMRunModel == CCBPMRunModel.GroupInc)
                {
                    string sql = "SELECT No FROM WF_FlowSort WHERE OrgNo='" + WebUser.No + "'";
        ...
        public string DoDelete()
        {
            ...
            if (num != 0 && 1 == 2)
            {
                string info = "err@该流程下有未完成的流程【" + num + "】个您不能删除，可以通过如下SQL查询:" + sql + "";
                info += " @如果确实需要删除该流程，请执行删除语句:DELETE FROM WF_GenerWorkerlist WHERE IsPass=0 AND FK_Flow='" + this.No + "' ";
                info += " @或者到：流程属性、流程运维、查询出来流程并删除掉他们。";
                return info;
            }

            //检查流程有没有版本管理？
            if (this.FlowSortNo.Length > 1 && 1 == 2)
            {
            ...
            sql += "@ DELETE FROM WF_GenerWorkerlist WHERE FK_Node IN (SELECT NodeID From WF_Node WHERE FK_Flow='" + this.No + "')";
            sql += "@ DELETE FROM WF_GenerWorkFlow WHERE FK_Node IN (SELECT NodeID From WF_Node WHERE FK_Flow='" + this.No + "')";
            sql += "@ DELETE FROM WF_Cond WHERE RefFlowNo='" + this.No + "'";
            sql += "@ DELETE FROM WF_Cond WHERE FK_Flow='" + this.No + "'";

            //删除节点事件
            sql += "@ DELETE FROM  Sys_FrmEvent WHERE RefFlowNo='" + this.No + "'";

            //删除消息配置.
            sql += "@ DELETE FROM WF_PushMsg WHERE FlowNo='" + this.No + "'";
            ...
            //考核表
            //  sql += "@ DELETE FROM WF_CH WHERE FlowNo='" + this.No + "'";
            //删除抄送
            sql += "@ DELETE FROM WF_CCList WHERE FlowNo='" + this.No + "'";
            //抄送规则
            sql += "@ DELETE FROM WF_CCRole WHERE FlowNo='" + this.No + "'";
        ...
        public string VerCreateNew()
        {
            try
            {
                ...
                if (DataType.IsNullOrEmpty(this.FlowMark) == false)
                    newFlow.FlowMark = this.FlowMark;
        ...
        /// <returns></returns>
        public string CheckFlowMark(string mark, string flowNo)
        {
            this.FlowMark = mark;
            this.No = flowNo;
            string sql = "";
            if (SystemConfig.CCBPMRunModel == CCBPMRunModel.Single)
                sql = "SELECT No,Name FROM WF_Flow WHERE FlowMark='" + this.FlowMark + "' AND No!='" + this.No + "' AND PTable!='" + this.PTable + "'";
            else
                sql = "SELECT No,Name FROM WF_Flow WHERE OrgNo='" + BP.Web.WebUser.OrgNo + "' AND FlowMark='" + this.FlowMark + "' AND No!='" + this.No + "' AND PTable!='" + this.PTable + "'";
            ...
            if (dt.Rows.Count >= 1)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    msg += " FlowNo=" + dr["No"].ToString() + " FlowName=" + dr["Name"].ToString() + "";
                }
