...
namespace BP.WF
{
    ...
    public class FullSA
    {
        ...
        public void DoIt2023(WorkNode currWorkNode)
        {
            ...
            //判断当前节点是否启用流程完成条件
            if (currWorkNode.HisNode.CondsOfFlowComplete.Count >= 1
                    && currWorkNode.HisNode.CondsOfFlowComplete.GenerResult(this.geRpt, currWorkNode.WebUser))
            {
                return;
            }
            //获得到达的节点.
            Node toNode = currWorkNode.NodeSend_GenerNextStepNode_Ext(currND);
        ...
        public void InitToNode(WorkNode currWN, Node toNd, WebUserCopy webUser)
        {
            ...
            TodolistModel todolistModel = currWN.HisNode.TodolistModel;
            DataTable dt = null;
            
            try
            {
                dt = fw.DoIt(currWN.HisFlow, currWN, town,true);
            }
            catch (Exception ex)
            {
            ...
            if (toNd.ItIsEndNode == false)
            {
                ...
                if(toNodeToID == 0)
                {
                    if (toNode.CondModel == DirCondModel.ByLineCond)
                    {
