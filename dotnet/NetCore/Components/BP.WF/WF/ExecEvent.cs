    ...
    public class ExecEvent
    {
        ...
        public static string DoFrm(MapData md, string eventType, Entity en, string atParas = null)
        {
            ...
            if (EventListFrm.FrmLoadBefore.Equals(eventType) == true)
                msg=BP.En.OverrideFile.FrmEvent_LoadBefore(md.No, en);

            //装载之后.
            if (EventListFrm.FrmLoadAfter.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmEvent_FrmLoadAfter(md.No, en);

            ///保存之前.
            if (EventListFrm.SaveBefore.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmEvent_SaveBefore(md.No, en);

            //保存之后.
            if (EventListFrm.SaveAfter.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmEvent_SaveAfter(md.No, en);

            //启动审核.
            if (EventListFrm.CheckStart.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmBill_CheckStart(md.No, en);

            //审核完成之后.
            if (EventListFrm.CheckOver.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmBill_CheckOver(md.No, en);

            //单据回滚.
            if (EventListFrm.Reback.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmBill_Reback(md.No, en);

            //撤销发送
            if (EventListFrm.UnSend.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmBill_UnSend(md.No, en);

            //归档前
            if (EventListFrm.OverBefore.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmBill_OverBefore(md.No, en);

            //归档后
            if (EventListFrm.OverAfter.Equals(eventType) == true)
                msg = BP.En.OverrideFile.FrmBill_OverAfter(md.No, en);
            #endregion 首先执行通用的事件重载方法.

            if (msg == null)
                msg = "";

            string str = md.FrmEvents.DoEventNode(eventType, en)+msg;
            string mystrs = null;
            if (md.HisFEB != null)
        ...
        string atPara = null)
        {
            ...
            {
                //协作模式下还未运行结束就显示下一个节点的抄送信息
                //if (wn.HisFlow.ItIsFullSA == true && wn.HisGenerWorkFlow.HuiQianTaskSta == HuiQianTaskSta.None )
            ...
            {
                if (item.EventNo.Equals(doType) == false)
                    continue;
