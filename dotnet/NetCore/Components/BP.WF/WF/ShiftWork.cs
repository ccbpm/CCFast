...
namespace BP.WF
{
    public class ShiftWork
    {
        ...
        /// 工作移交
        /// </summary>
        /// <param name="fromUser">工作ID</param>
        /// <param name="workID">工作ID</param>
        /// <param name="toEmp">要移交的人</param>
        /// <param name="msg">移交信息</param>
        /// <returns>执行结果</returns>
        public static string Node_Shift_ToEmp(string fromUser,Int64 workID, string toEmp, string msg)
        {
            if (DataType.IsNullOrEmpty(fromUser))
                fromUser = WebUser.No;
                ...
                || nd.TodolistModel == TodolistModel.TeamupGroupLeader)
            {
                ...
                int myNum = gwl.Retrieve(GenerWorkerListAttr.FK_Emp, fromUser, GenerWorkerListAttr.FK_Node, gwf.NodeID, GenerWorkerListAttr.WorkID, workID);

                /*  string sql = "UPDATE WF_GenerWorkerlist SET IsRead=0, FK_Emp='" + emp.UserID + "', EmpName='" + emp.Name + "' WHERE FK_Emp='" + fromUser + "' AND FK_Node=" + gwf.NodeID + " AND WorkID=" + workID;
                ...
                if (myNum == 1)
                {
                    ...
                    gwl.Sender=femp.UserID + "," + femp.Name;
                    gwl.Insert();
                }
                ...
                if (myNum == 0)
                {
                    ...
                    foreach (GenerWorkerList item in mygwls)
                    {
                        ...
                        item.Sender = femp.UserID + "," + femp.Name;
                        item.Insert(); //执行插入.
                        break;
                    }
                }
                ...
                if (pms1.Count == 0)
                {
                    ...
                    pm.NodeID = nd.NodeID;
                    pm.EventNo = EventListNode.ShitAfter;
                    pms1.AddEntity(pm);
                }
                foreach (PushMsg pm in pms1)
                    pm.DoSendMessage(nd, nd.HisWork, null, null, null, emp.UserID);
                if (gwf.WFState != WFState.Runing)
                    gwf.SetPara("WFState", (int)gwf.WFState);

                gwf.WFState = WFState.Shift;
                //gwf.TodoEmpsNum = 1;
                gwf.TodoEmps = emp.UserID + "," + emp.Name + ";";
                gwf.TodoEmps = gwf.TodoEmps.Replace(femp.UserID + "," + femp.Name + ";", "") + emp.UserID + "," + emp.Name + ";";

                gwf.Update();
                return "@工作移交成功。@您已经成功的把工作移交给：" + emp.UserID + " , " + emp.Name + info;
            }
           ...
           // gwf.TodoEmpsNum = 1;
            ...
            //记录日志.
            Glo.AddToTrack(ActionType.Shift, nd.FlowNo, workID, gwf.FID, nd.NodeID, nd.Name,
                femp.UserID, femp.Name, nd.NodeID, nd.Name, toEmp, emp.Name, msg, null);
        ...
        public static string Node_Shift_ToEmps(string fromUser,Int64 workID, string toEmps, string msg)
        {
            ...
            //删除自己的待办.
            DBAccess.RunSQL("DELETE FROM WF_GenerWorkerList WHERE WorkID=" + gwf.WorkID + " AND FK_Node=" + gwf.NodeID + " AND FK_Emp='" + fromUser + "'");

            //记录日志.
            Glo.AddToTrack(ActionType.Shift, nd.FlowNo, workID, gwf.FID, nd.NodeID, nd.Name,
                femp.UserID, femp.Name, nd.NodeID, nd.Name, toEmps, "移交给多个人", msg, null);
