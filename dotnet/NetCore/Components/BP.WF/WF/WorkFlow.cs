        ...
        public static void DeleteFlowByReal(Int64 workid, bool isDelSubFlow)
        {
            ...
            if (i != 0)
            {
                if (gwf.IsOver == false)
                {
                    ...
                    pms1.Retrieve(PushMsgAttr.NodeID, gwf.NodeID, PushMsgAttr.EventNo, EventListFlow.AfterFlowDel);
            ...
            {
                if (strs.Contains("@" + frmNode.FrmID) == true)
                    continue;

                strs += "@" + frmNode.FrmID + "@";
                try
                {
                    MapData md = new MapData(frmNode.FrmID);
            ...
            pms.Retrieve(PushMsgAttr.NodeID, gwf.NodeID, PushMsgAttr.EventNo, EventListFlow.AfterFlowDel);
...
#warning 应该删除一个子线程后，就需要计算完成率的问题。但是目前应用到该场景极少,因为。能够看到河流点信息，说明已经到达了完成率了。
        ...
        /// <returns></returns>
        public string DoFlowOver(ActionType at, string stopMsg, Node currNode, GERpt rpt, WFState wfStateOver = WFState.CompleteEnd, string empNo = "", string empName = "")
        {
            if (null == currNode)
                return "err@当前节点为空..";
            ...
            {
                throw new Exception("err@流程状态设置错误,不是结束状态:CompleteEnd/CompleteByNode");
            }
            ...
            if (rpt == null)
            {
                GERpt rptGe = this.HisFlow.HisGERpt;
                if (wn.HisNode.ItIsSubThread == true)
                    rptGe.SetValByKey("OID", this.HisGenerWorkFlow.FID);
                else
            ...
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle
                ...
                || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR8
            ...
            ps.Add("WFState", (int)wfStateOver); //设置结束状态.
            ...
            if (DataType.IsNullOrEmpty(gwf.GetParaString("BillFrmID")) == false)
            {
                ...
                GEEntityOID gE = new GEEntityOID(frmID, gwf.WorkID); //设置完成.
            ...
            //修改绑定表单中BillState的状态
            if (wn.HisNode.HisFormType == NodeFormType.RefOneFrmTree)
            {
                Work wk = wn.HisNode.HisWork;
                string table = wk.EnMap.PhysicsTable;
                if (DBAccess.IsExitsTableCol(table, "BillState") == true)
                {
                    DBAccess.RunSQL("UPDATE " + table + " SET BillState=100 WHERE OID=" + this.WorkID);
                }
                /* wk.OID=this.WorkID;
                 if (wk.RetrieveFromDBSources() == 1 && wk.EnMap.Attrs.Contains("BillState") == true)
                 {
                     wk.SetValByKey("BillState", 100);
                     wk.DirectUpdate();
                 }*/
            }
            if (sendSuccess != null)
        ...
        public string DoFix(string fixMsg)
        {
            ...
            Node nd = new Node(this.HisGenerWorkFlow.NodeID);
            if (nd.TodolistModel == TodolistModel.Order || nd.TodolistModel == TodolistModel.Teamup || nd.TodolistModel == TodolistModel.TeamupGroupLeader)
            {
                ps = new Paras();
                ps.SQL = "UPDATE WF_GenerWorkerlist SET IsPass=" + dbstr + "IsPass WHERE WorkID=" + dbstr + "WorkID AND FK_Node=" + this.HisGenerWorkFlow.NodeID + " AND FK_Emp=" + dbstr + "FK_Emp";
                ps.Add(GenerWorkerListAttr.IsPass, 9);
                ps.Add(GenerWorkerListAttr.WorkID, this.WorkID);
                ps.Add(GenerWorkerListAttr.FK_Emp, WebUser.No);
                DBAccess.RunSQL(ps);
            }
            else
            {
        ...
        public string DoUnFix(string unFixMsg)
        {
            ...
            GenerWorkFlow gwf = new GenerWorkFlow(this.WorkID);
            gwf.WFState = WFState.Runing;
            gwf.SetValByKey("WFSta", 0);
            gwf.Update();

            /* 获取它的工作者，向他们发送消息。*/
            GenerWorkerLists wls = new GenerWorkerLists(this.WorkID);
            string url =  SystemConfig.HostURL + "/" + this.VirPath + this.AppType + "/WorkOpt/OneWork/OneWork.htm?CurrTab=Track&FK_Flow=" + this.HisFlow.No + "&WorkID=" + this.WorkID + "&FID=" + this.HisGenerWorkFlow.FID + "&FK_Node=" + this.HisGenerWorkFlow.NodeID;
            ...
            foreach (GenerWorkerList wl in wls)
            {
                ...
                emps += wl.EmpNo + "," + wl.EmpName + ";";
                //写入消息。
                BP.WF.Dev2Interface.Port_SendMsg(wl.EmpNo, title, mailDoc, "Fix" + wl.WorkID,"Fix", gwf.FlowNo, wl.NodeID, wl.WorkID, wl.FID);
            }
            ...
            Paras  ps = new Paras();
            ps.SQL = "UPDATE " + this.HisFlow.PTable + " SET WFState=" + dbstr + "WFState WHERE OID=" + dbstr + "OID";
            ps.Add(GERptAttr.WFState, WFState.Runing);
            ps.Add(GERptAttr.OID, this.WorkID);
            DBAccess.RunSQL(ps);
            ...
            //写入日志.
            BP.WF.Dev2Interface.WriteTrack(this.HisFlow.No, this.HisGenerWorkFlow.NodeID, this.HisGenerWorkFlow.NodeName, this.WorkID, this.FID, "解除冻结:"+ unFixMsg, ActionType.Info, "Fix", null, null, "冻结");
