...
#warning 没有考虑两种流程数据存储模式。
            ...
            pms.Retrieve(PushMsgAttr.NodeID, this.ReturnToNode.NodeID, PushMsgAttr.EventNo, EventListNode.ReturnAfter);
            ...
            if (this.ReturnToNode.ItIsGuestNode)
            {
                ...
                return "<p>工作已经被您退回到" + this.ReturnToNode.Name + ",</p><p style='margin:10px 0;'>退回给" + gwf.GuestName + ".</p>" + text;
            }
            else
            {
                return "<p>工作已经被您退回到" + this.ReturnToNode.Name + ",</p><p style='margin:10px 0;'>退回给" + returnToEmp.Name + ".</p>" + text;
            }
        }
        /// <summary>
        /// 要退回到父流程上去
        ...
        private string ExeReturn2_4()
        {
            ...
            //退回消息事件 
            PushMsgs pms = new PushMsgs();
            pms.Retrieve(PushMsgAttr.NodeID, this.HisNode.NodeID, PushMsgAttr.EventNo, EventListNode.UndoneAfter);
            foreach (PushMsg pm in pms)
            {
        ...
        private string ExeReturn5_2()
        {
            ...
            //退回消息事件
            PushMsgs pms = new PushMsgs();
            pms.Retrieve(PushMsgAttr.NodeID, this.HisNode.NodeID, PushMsgAttr.EventNo, EventListNode.ReturnAfter);
            foreach (PushMsg pm in pms)
            {
        ...
        private string ExeReturn3_2()
        {
            ...
            if (this.IsBackTrack == false)
            {
                ...
                //获取当前从表的汇总数据
                MapDtls dtls = new MapDtls(this.HisNode.NodeFrmID);
                foreach (MapDtl dtl in dtls)
                {
                    /*如果是合流数据*/
                    if (dtl.ItIsHLDtl)
        ...
        private string ExeReturn1_1()
        {
            ...
            {
                return "<p>工作已经被您退回到" + this.ReturnToNode.Name + ",</p> <p style='margin:10px 0;'>退回给" + empReturn.Name + ".</p>" + text;
            }
        }
        /// <summary>
        /// 增加日志
