...
namespace BP.WF
{
    ...
    public class WorkNodePlus
    {
            ...
            DateTime dtOfShould, DateTime dtOfWarning, Int64 nextUsersFID, bool isFenLiuToSubThread, Int64 nextUsersWorkID)
        {
            ...
            //协作组长模式
            string zcr = "";
            if (town.HisNode.TodolistModel == TodolistModel.TeamupGroupLeader)
            {
                int teamleaderConfirm = town.HisNode.GetParaInt("TeamleaderConfirm");
                zcr = town.HisNode.GetParaString("TeamleaderCfmVal");
                if (teamleaderConfirm == 1 && DataType.IsNullOrEmpty(zcr) == true)
                    throw new Exception("err@多人处理规则：协作组长模式确认组长时没有填写组长");
            }
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (wl.ItIsEnable == false)
                    IsEnable = "0";
                if (DataType.IsNullOrEmpty(zcr) == false && wl.EmpNo.Equals(zcr) == false)
                    wl.SetPara("HuiQianZhuChiRen", zcr);
            ...
            sql += ",SDT='" + sdt + "'";
            sql += ",RDT='" + DataType.CurrentDateTimess + "'";
        ...
        public static void AddNodeFrmTrackDB(Flow fl, Node nd, Track track, Work wk)
        {
            ...
            if (nd.HisFormType == NodeFormType.SheetTree)
            {
                ...
                foreach (FrmNode fn in fns)
                {
                    ...
                    BP.Sys.GEEntityOID ge = new GEEntityOID(fn.FrmID);
                    ge.OID = track.WorkID;
                    ...
                    if (md.HisFrmType == FrmType.ChapterFrm)
                    {
                        ...
                        foreach (MapAttr attr in attrs)
                        {
                            if (attr.UIVisible == false)
                                continue;
                            BP.Sys.FrmDBVer.AddKeyOfEnDBTrack(ver, fn.FrmID, track.WorkID.ToString(), track.MyPK, ge.GetValStringByKey(attr.KeyOfEn), attr.KeyOfEn);
                        }
                        string json = AddNodeFrmDtlDB(nd.NodeID, track.WorkID, fn.FrmID);
                        string aths = AddNodeFrmAthDB(nd.NodeID, track.WorkID, fn.FrmID);
                        BP.Sys.FrmDBVer.AddFrmDBTrack(ver, fn.FrmID, track.WorkID.ToString(), track.MyPK, null, json, aths, true);
                    ...
                    string dtlJson = AddNodeFrmDtlDB(nd.NodeID, track.WorkID, fn.FrmID);
                    string athJson = AddNodeFrmAthDB(nd.NodeID, track.WorkID, fn.FrmID);
                    BP.Sys.FrmDBVer.AddFrmDBTrack(ver, fn.FrmID, track.WorkID.ToString(), track.MyPK, ge.ToJson(), dtlJson, athJson, false);
                }
                return;
            }
        ...
        public static string DTSData(Flow fl, GenerWorkFlow gwf, GERpt rpt, Node currNode, bool isStopFlow)
        {
            string msg = "";

            //判断同步类型.
            if (fl.DTSWay == DTSWay.None)
                return msg;

            //流程结束时候同步，但是流程没有结束.
            if (fl.DTSWay == DTSWay.WhenStopFlow && isStopFlow == false)
                return msg;

            //按照指定的节点同步.
            if (fl.DTSWay == DTSWay.AfterSpecifiedNode && fl.DTSSpecNodes.Equals(currNode.NodeID.ToString()) == false)
                return msg;

            SyncDatas ens = new SyncDatas();
            ens.Retrieve("FlowNo", fl.No);

            foreach (SyncData item in ens)
            {
                if (item.SyncType.Equals("DBSrc") == true)
                {
                    msg += DTSData_DBSrc(item, fl, gwf, rpt, currNode, isStopFlow);
                }
                if (item.SyncType.Equals("APIOfSelf") == true) // 同步到API接口
                {
        ...
        /// <param name="isStopFlow"></param>
        /// <returns></returns>
        public static string DTSData_APIOfSelf(SyncData sd, Flow fl, GenerWorkFlow gwf, GERpt rpt, Node currNode, bool isStopFlow)
        {
            long workID = gwf.WorkID;
            string nodeFrmID = currNode.NodeFrmID;
            //推送的数据
            string info = "{";
            //推送的主表数据
            string mainTable = "";
            ...
            foreach (MapDtl dtl in mapDtls)
            {
                ...
                foreach (GEDtl geDtl in geDtls)
                {
                    ...
                    attachmentDBs.Retrieve(FrmAttachmentDBAttr.FK_MapData, dtlNo, FrmAttachmentDBAttr.RefPKVal, geDtl.OID, null);
            ...
            foreach (FrmAttachment attachment in attachments)
            {
                ...
                dbs.Retrieve(FrmAttachmentDBAttr.FK_FrmAttachment, attachment.MyPK, FrmAttachmentDBAttr.FK_MapData, nodeFrmID, FrmAttachmentDBAttr.RefPKVal, workID, null);
                if (dbs.Count <= 0)
                {
                    continue;
                }
        ...
        public static string DTSData_DBSrc(SyncData sd, Flow fl, GenerWorkFlow gwf, GERpt rpt, Node currNode, bool isStopFlow)
        {
            //获取同步的字段.
            SyncDataFields sfs = new SyncDataFields();
            sfs.Retrieve("RefPKVal", sd.MyPK);

            string sql = "SELECT count(*) as Num  FROM " + sd.PTable + " WHERE " + sd.TablePKName + "='" + gwf.WorkID + "'";
            SFDBSrc src = new SFDBSrc(sd.DBSrc);
            int num = src.RunSQLReturnValInt(sql);
            if (num == 0)
            {
                if (sd.TablePKType.ToLower().Contains("char"))
                    sql = "INSERT INTO " + sd.PTable + " (" + sd.TablePKName + ") VALUES('" + gwf.WorkID + "')";
                else
                    sql = "INSERT INTO " + sd.PTable + " (" + sd.TablePKName + ") VALUES(" + gwf.WorkID + ")";
                src.RunSQL(sql);
            }
            Entity frmData = null;
            if (sd.FrmID.Equals(rpt.ClassID) == true)
                frmData = rpt;

            sql = "UPDATE " + sd.PTable + " SET ";
            foreach (SyncDataField sf in sfs)
            {
                if (DataType.IsNullOrEmpty(sf.ToFieldKey) == true)
                    continue;
                //获得数据.
                string str = frmData.GetValStringByKey(sf.ToFieldKey);
                if (sf.GetValStringByKey("AttrType").Equals("1") == true)
                    sql += sf.AttrKey + "= '" + str + "',"; //string类型的.
                else
                {
            ...
            if (sd.TablePKType.Contains("char") == true || sd.TablePKType.Equals("1") == true)
                sql += " WHERE " + sd.GetValStrByKey("TablePKName") + " ='" + gwf.WorkID + "'";
            else
                sql += " WHERE " + sd.GetValStrByKey("TablePKName") + "=" + gwf.WorkID;
            src.RunSQL(sql);
            return "同步流程数据到:" + sd.Note + ",成功.";
        }
        /// <summary>
        /// 处理协作模式下的删除规则
        /// </summary>
        ...
        /// <param name="gwf"></param>
        public static string GenerWorkerListDelRole(Node nd, GenerWorkFlow gwf)
        {
            string emps = ""; 
