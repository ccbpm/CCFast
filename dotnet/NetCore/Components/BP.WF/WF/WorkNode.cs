...
namespace BP.WF
{
    ...
    public class WorkNode
    {
        ...
        public GenerWorkerLists Func_GenerWorkerLists(WorkNode town)
        {
            ...
            //如果是自动计算未来接受人.
            if (this.HisFlow.ItIsFullSA == true)
            {
                string sql = "SELECT FK_Emp FROM WF_SelectAccper WHERE FK_Node=" + town.HisNode.NodeID + "  AND WorkID=" + this.WorkID;
        ...
        private GenerWorkerLists Teamup_InitWorkerLists(FindWorker fw, WorkNode town)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count == 0)
                    throw new Exception("err@不可能出现的错误:Teamup_InitWorkerLists");
                if (dt.Rows.Count == 1)
                {
                    dt = fw.DoIt(this.HisFlow, this, town);
                    if (dt == null)
        ...
        private GenerWorkerLists InitWorkerLists(WorkNode town, DataTable dt, Int64 fid = 0)
        {
            ...
            if (dt.Rows.Count == 1)
            {
                ...
                wl.EmpName = emp.Name;
                if (dt.Columns.Contains("FK_Dept"))
                {
            ...
            if (at == ActionType.Forward)
            {
                try
                {
                    ...
                    GenerWorkerList gwl = new GenerWorkerList(this.WorkID, this.HisNode.NodeID, WebUser.No);
        ...
        private Node NodeSend_GenerNextStepNodeExt()
        {
            ...
            /*//如果是自动计算未来接受人.
            if (this.HisFlow.ItIsFullSA == true && this.HisNode.ItIsStartNode == false )
            {
                //判断是否需要重新计算？根据版本号。
            ...
            if (this.HisNode.CondModel == DirCondModel.ByPopSelect)
            {
                ...
                if (DataType.IsNullOrEmpty(nodes) || nodes.Equals(",") == true)
                {
                    if (this.HisNode.HisToNodes.Count == 1)
                    {
                        Node mynd = this.HisNode.HisToNodes[0] as Node;

                        if (mynd.AutoJumpRole0 == true
                            || mynd.AutoJumpRole1 == true
                            || mynd.AutoJumpRole2 == true
                            || mynd.AutoJumpRole3 == true)
                        {
                            throw new Exception("err@流程模板设计错误，节点[" + mynd.Name + "]设置了跳转规则，但是上一个节点的方向条件是选择的不起作用，请取消跳转规则或者设置自动计算模式的转向规则，不让其选择。");
                        }
                ...
                foreach (string item in mynodes)
                {
                    ...
                    //排除到达自身节点.
                    if (this.HisNode.NodeID.ToString().Equals(item))
                        continue;

                    Node mynd = new Node(int.Parse(item));
                    if (mynd.AutoJumpRole0 == true
                         || mynd.AutoJumpRole1 == true
                         || mynd.AutoJumpRole2 == true
                         || mynd.AutoJumpRole3 == true)
        ...
        public Node NodeSend_GenerNextStepNode_Ext1()
        {
            ...
            while (true)
            {
                ...
                if (mynd.ItIsEndNode)
                {
                    ...
                    if (this.HisGenerWorkFlow.TransferCustomType == TransferCustomType.ByCCBPMDefine)
                    {
                        ...
                        String msg = this.HisWorkFlow.DoFlowOver(ActionType.FlowOver, "流程已经走到最后一个节点，流程成功结束。",
                                mynd, this.rptGe, WFState.CompleteEnd, Executor, ExecutorName);
                ...
                if (this.HisFlow.ItIsFullSA == true)
                {
                    String sql = "SELECT FK_Emp FROM WF_SelectAccper WHERE FK_Node=" + nd.NodeID + "  AND WorkID=" + this.WorkID;
                    dt = DBAccess.RunSQLReturnTable(sql);
                }
                ...
                if (nd.AutoJumpRole2)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        //若上个节点也是跳过的就取最新上一步处理人
                        string sql1 = "SELECT COUNT(*) FROM WF_GenerWorkerlist WHERE FK_Emp='" + dr[0] +
                                     "' AND WorkID=" + this.WorkID + " AND FK_Node=" +
                                     (beforeSkipNodeID > 0 ? beforeSkipNodeID : prvNodeID);
                        if (DBAccess.RunSQLReturnValInt(sql1) == 0)
                        {
                            string sql2 = "SELECT FK_Node,WorkID,FK_Emp FROM WF_GenerWorkerlist WHERE  WorkID=" + this.WorkID + " ORDER BY RDT DESC ";
                            DataTable dt2 = DBAccess.RunSQLReturnTable(sql2);
                            prvNodeID = (int)dt2.Rows[0]["FK_Node"];
                        }
        ...
        public void DealReturnOrderTeamup()
        {
            ...
            {
                ens.Retrieve(GenerWorkerListAttr.WorkID, this.WorkID, GenerWorkerListAttr.FK_Node, this.JumpToNode.NodeID);
                if (ens.Count == 0)
                    throw new Exception(BP.WF.Glo.multilingual("@没有找到接收人期望的数据，在协作模式的按照顺序退回的时候.", "WorkNode", "not_found_receiver_expected_data", new String[0]));

                foreach (GenerWorkerList curgwl in ens)
                {
                    curgwl.SDT = sdt;
                    curgwl.RDT = DataType.CurrentDateTimess;
                    curgwl.ItIsPass = false;
                    curgwl.ItIsRead = false;
                    if (this.JumpToNode.TodolistModel == TodolistModel.Order && curgwl.Idx != 1)
                        curgwl.PassInt = curgwl.Idx + 100;
                    curgwl.Update();
                    varAcceptersID += curgwl.EmpNo + ",";
                    varAcceptersName += curgwl.EmpName + ",";
                    todoEmps += curgwl.EmpNo + "," + curgwl.EmpName + ";";
                }
            ...
            else
            {
                ...
                gwl.ItIsPass = false;
                gwl.Update();
                varAcceptersID += gwl.EmpNo;
                varAcceptersName += gwl.EmpName;
        ...
        /// </summary>
        /// <returns></returns>
        public Node NodeSend_GenerNextStepNode_Ext(Node currNode)
        {
            ...
            if (this.SendHTOfTemp != null)
            {
                ...
                {
                    if (rptGe.Row.ContainsKey(key) == false)
            ...
            if (this.HisNode.ItIsSubThread == true)
            {
                ...
                if (work.RetrieveFromDBSources() == 1)
                {
                    AtPara atPara = work.atPara;
                    foreach (String key in atPara.HisHT.Keys)
                    {
                        if (DataType.IsNullOrEmpty(key) == true)
                            continue;
                        this.rptGe.SetPara(key, atPara.GetValStrByKey(key));
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public bool Func_CheckCompleteCondition()
        {
            if (this.HisNode.ItIsSubThread == true)
            ...
            try
            {
                    ...
                    && this.HisNode.CondsOfFlowComplete.GenerResult(this.rptGe, this.WebUser))
                {
                    ...
                    string stopMsg = this.HisNode.CondsOfFlowComplete.ConditionDesc;
                    /* 如果流程完成 */
        ...
        private string Func_DoSetThisWorkOver()
        {
            ...
            if (this.IsStopFlow == true)
                this.HisGenerWorkFlow.WFState = WFState.CompleteEnd;
            else
                this.HisGenerWorkFlow.WFState = WFState.Runing;
        ...
        private void NodeSend_11(Node toND)
        {
            ...
            {
                int teamleaderConfirm = town.HisNode.GetParaInt("TeamleaderConfirm");
                if (teamleaderConfirm != 1)
            ...
            {
                if (this.HisNode.ItIsStartNode)
                    this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&Token=" + WebUser.Token + "&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FlowNo + "' ><img src='" + BP.Difference.SystemConfig.HostURLOfBS + "/WF/Img/Action/UnSend.png' style=display:inline border=0/>撤销本次发送</a>", SendReturnMsgType.Info);
                else
                    this.addMsg(SendReturnMsgFlag.ToEmpExt, null, "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&Token=" + WebUser.Token + "&WorkID=" + this.HisWork.OID + "&FK_Flow=" + toND.FlowNo + "' ><img src='" + BP.Difference.SystemConfig.HostURLOfBS + "/WF/Img/Action/UnSend.png' style=display:inline border=0 />撤销本次发送</a> ", SendReturnMsgType.Info);
            }*/
                ...
                if (nd.USSWorkIDRole == 0)
                {
                    ...
                    foreach (FrmNode fn in fns)
                    {
                        ...
                        if (fn.FrmEnableRole != FrmEnableRole.Allways)
                            continue;
                        if (DataType.IsNullOrEmpty(fn.FrmID) == true)
                        {
                            fn.Delete();
                            continue;
                        }

                        GEEntityOID ge = new GEEntityOID(fn.FrmID, workIDSubThread);
                        if (fn.WhoIsPK != WhoIsPK.FID)
                        {
                            //ge.OID = this.HisWork.OID;
                            ge.Copy(this.HisWork);
                            ge.OID = workIDSubThread;
                            ge.SetValByKey("FID", this.HisWork.OID);
                            if (ge.IsExits == false)
                    ...
                    gwf.DomainExt = this.HisGenerWorkFlow.DomainExt; //域.
        ...
        private void NodeSend_24_SameSheet(Node toNode)
        {
            ...
            foreach (GenerWorkerList wl in current_gwls)
            {
                ...
                if (gwf.RetrieveFromDBSources() == 0)
                {
                    ...
                    //域.
                    gwf.DomainExt = this.HisGenerWorkFlow.DomainExt;
                    ...
                    //获取干流程的AtPara值
                    AtPara paras = this.HisGenerWorkFlow.atPara;
                    foreach (string str in paras.HisHT.Keys)
                    {
            ...
            {
                if (this.HisNode.ItIsStartNode)
                    this.addMsg("UnDo", "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&Token=" + WebUser.Token + "&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FlowNo + "' ><img src='" + BP.Difference.SystemConfig.HostURLOfBS + "/WF/Img/Action/UnSend.png' style=display:inline border=0/>撤销本次发送</a>.");
                else
                    this.addMsg("UnDo", "@<a href='./WorkOpt/UnSend.htm?DoType=UnSend&UserNo=" + WebUser.No + "&Token=" + WebUser.Token + "&WorkID=" + this.WorkID + "&FK_Flow=" + toNode.FlowNo + "' ><img src='" + BP.Difference.SystemConfig.HostURLOfBS + "/WF/Img/Action/UnSend.png' style=display:inline border=0/>撤销本次发送</a>.");
            }*/
...
#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
        ...
        public void CheckFrm1ToN()
        {
            ...
            foreach (FrmNode fn in frmNDs)
            {
                ...
                //初始化这个实体.
                GEEntity geEn = new GEEntity(fn.FrmID, pk);
        ...
        public void CheckFrmHuiZongToDtl()
        {
            ...
            foreach (FrmNode fn in frmNDs)
            {
                ...
                //初始化这个实体,获得这个实体的数据.
                GEEntity rpt = new GEEntity(fn.FrmID, pk);
                //
        ...
        public bool CheckFrmIsNotNull()
        {
            ...
            if (this.HisNode.HisFormType == NodeFormType.SheetTree)
            {
                ...
                foreach (FrmNode item in nds)
                {
                    ...
                    //主表实体.
                    GEEntityOID en = new GEEntityOID(item.FrmID);
                    en.OID = pkVal;
            ...
            {
                MapData md = new MapData(item.FrmID);
                ...
                {
                    if (myff.FrmID.Equals(item.FrmID) == false)
                        continue;
                    isHave = true;
                    break;
                }
                ...
                //获取表单值
                GEEntityOID en = new GEEntityOID(md.No);
                en.OID = pk;
        ...
        public Work CopySheetTree()
        {
            ...
            foreach (FrmNode item in frmNDs)
            {
                ...
                foreach (FrmField ff in ffs)
                {
                    if (ff.FrmID.Equals(item.FrmID) == false)
                        continue;
        ...
        public bool DealTeamUpNode()
        {
            ...
            int xzoRole = this.HisNode.GetParaInt("XieZuoOverRole"); //协作完成规则


            decimal xzoRate = (decimal)this.HisNode.GetParaFloat("XieZuoOverRate"); //协作完成率
                                                                                    //审核组件的立场-完成率
            if (xzoRole == 1)
            {
                //获取审核通过的审核立场的数据
                ...
                {
                    if (gwl.EmpNo.Equals(WebUser.No) || gwl.PassInt == 1)
                    {
                ...
                decimal agreeRate = (decimal)agreeCount / gwls.Count * 100;
                decimal nodeRate = (decimal)noCount / gwls.Count * 100;
                if (agreeRate >= xzoRate || nodeRate >= (100 - xzoRate))
                {
            ...
            if (xzoRole == 2)
            {
                ...
                if (ens != null)
                {
                    decimal passRate = (decimal)(ens.Count + 1) / gwls.Count * 100;
                    if (xzoRate >= passRate)
                    {
                        string huiqianNo = this.HisGenerWorkFlow.HuiQianZhuChiRen;
                        string huiqianName = this.HisGenerWorkFlow.HuiQianZhuChiRenName;
            ...
            if (xzoRole == 3)
            {
                String emps = this.HisNode.GetParaString("XieZuoOverRate");
                if (DataType.IsNullOrEmpty(emps) == false && ("," + emps + ",").Contains("," + WebUser.No + ",") == true)
                {
                    string huiqianNo = this.HisGenerWorkFlow.HuiQianZhuChiRen;
                    string huiqianName = this.HisGenerWorkFlow.HuiQianZhuChiRenName;
            ...
            if (xzoRole == 4)
            {
                ...
                foreach (GenerWorkerList gwl in gwls)
                {
                    if (gwl.PassInt == 1)
                    {
                        foreach (String empNo in empArr)
                        {
                            if (gwl.EmpNo.Equals(empNo))
                            {
                                countPass++;
                            }
                        }
            ...
            foreach (GenerWorkerList gwl in gwls)
            {
                ...
                //写入日志.
                if (this.HisGenerWorkFlow.HuiQianTaskSta != HuiQianTaskSta.None)
                    this.AddToTrack(actionType, todoNos, todoEmps, this.HisNode.NodeID, this.HisNode.Name, BP.WF.Glo.multilingual("会签", "WorkNode", "cross_signing"));
                else
                    this.AddToTrack(actionType, todoNos, todoEmps, this.HisNode.NodeID, this.HisNode.Name, BP.WF.Glo.multilingual("协作发送", "WorkNode", "cross_signing"));
        ...
        public SendReturnObjs NodeSend(Node jumpToNode, string jumpToEmp, bool IsReturnNode = false)
        {
            ...
            if (this.IsStopFlow == true)
            {
                ...
                this.rptGe.WFState = WFState.CompleteEnd;
                this.rptGe.Update();
            ...
            if (this.TodolistModel == TodolistModel.Order)
            {
                if (this.DealOradeNode() == true)
                {
                    ...
                    //删除增加的抄送信息
                    WorkOpt workOpt = new WorkOpt();
                    workOpt.setMyPK(this.WebUser.No + "_" + this.HisNode.NodeID + "_" + this.WorkID);
                    workOpt.Delete();
            ...
            if (this.HisNode.HisDeliveryWay == DeliveryWay.ByMLeader0)
            {
                //如果当前还有队列，生成待办列表，退出
                string findLeader = "SELECT Leader as FK_Emp FROM Port_Emp WHERE No='" + WebUser.No + "'";
                DataTable dtEmp = DBAccess.RunSQLReturnTable(findLeader);
                //是否有直接主管
                var hasLeader = !String.IsNullOrEmpty(dtEmp.Rows[0][0].ToString());
                //判断当前是不是审批终点，根据角色

                NodeStations nodeStations = new NodeStations();
                //nodeStations.Retrieve(NodeStationAttr.FK_Node, this.HisNode.NodeID);

                QueryObject qo = new QueryObject(nodeStations);
                qo.AddWhere(NodeStationAttr.FK_Node, this.HisNode.NodeID);
                qo.addAnd();
                qo.AddWhereInSQL(NodeStationAttr.FK_Station, "SELECT FK_Station from Port_DeptEmpStation where FK_Emp='" + WebUser.No + "'");
                qo.DoQuery();

                bool isLastApporver = nodeStations.Count <= 0 ? false : true;
                ...
                if (hasLeader && !isLastApporver)
                {
                    ...
                    Emp emp = new Emp(dtEmp.Rows[0][0].ToString());

                    GenerWorkerList gwl = new GenerWorkerList();
                    gwl.WorkID = this.WorkID;
                    gwl.FID = this.HisGenerWorkFlow.FID;
                    gwl.NodeID = this.HisNode.NodeID;
                    gwl.NodeName = this.HisNode.Name;
                    gwl.EmpNo = emp.No;
                    gwl.EmpName = emp.Name;
                    gwl.DeptNo = emp.DeptNo;
                    gwl.DeptName = emp.DeptText;
                    gwl.WhoExeIt = this.HisNode.WhoExeIt;
                    gwl.SDT = DataType.CurrentDateTime;
                    gwl.DTOfWarning = DataType.CurrentDateTime;
                    gwl.CDT = DataType.CurrentDateTime;
                    gwl.FlowNo = this.HisNode.FlowNo;
                    gwl.Sender = WebUser.No + "," + WebUser.Name;
                    gwl.PassInt = 0;
                    gwl.ItIsRead = true;
                    gwl.Insert();
                    ...
                    generWorkerList.Retrieve(GenerWorkerListAttr.FK_Emp, WebUser.No, GenerWorkerListAttr.WorkID, this.WorkID, GenerWorkerListAttr.FK_Node, this.HisNode.NodeID);
                    generWorkerList.ItIsPass = true;
                    generWorkerList.Update();
                    //当前登录人增加日志
                    this.AddToTrack(ActionType.Order, generWorkerList.EmpNo, generWorkerList.EmpName, this.HisNode.NodeID,
                       this.HisNode.Name, BP.WF.Glo.multilingual("队列发送", "WorkNode", "queue_transferred"));
                    //下一步主管提醒
            ...
            if (this.TodolistModel == TodolistModel.Teamup)
            {
                ...
                {
                    skipEmps = WorkNodePlus.GenerWorkerListDelRole(this.HisNode, this.HisGenerWorkFlow);
                }
                ...
                todoEmps = todoEmps.Replace(WebUser.No + "," + WebUser.Name, "");
                if (!DataType.IsNullOrEmpty(skipEmps))
                {
                ...
                if (this.DealTeamUpNode() == true)
                {
                    ...
                    //this.HisGenerWorkFlow.HuiQianTaskSta = HuiQianTaskSta.HuiQianing;
                    //调用发送成功事件.
                    string sendSuccess = ExecEvent.DoNode(EventListNode.SendSuccess, this, this.HisMsgObjs, null);
            ...
            try
            {
                ...
                #region 处理退回的情况.
                if (this.HisGenerWorkFlow.WFState == WFState.ReturnSta || this.HisGenerWorkFlow.GetParaInt("WFState") == (int)WFState.ReturnSta)
                {
                    ...
                    else
                    {
                        ...
                        if (nds.Count == 1)
                        {
                            ...
                            else
                            {
                                ...
                                this.HisGenerWorkFlow.TransferCustomType = TransferCustomType.ByCCBPMDefine;
                            }
                        }
                        if (nds.Count > 1)
                        {
                ...
                #region 执行抄送.
                //执行抄送.
                if (this.HisNode.ItIsEndNode == false && this.IsStopFlow == false)
                {
                    //执行抄送
                    CC(this.HisNode);
                }
                ...
                if (this.HisFlow.DTSWay != DTSWay.None)
...
#warning 如果这里设置了自动跳转，现在去掉了. 2014-11-07.
            ...
            foreach (SubFlowAuto sub in subs)
            {
                ...
                if (sub.HisSubFlowModel == SubFlowModel.SameLevel && this.HisGenerWorkFlow.PWorkID != 0)
                {
                    ...
                    if (sub.SendModel == 0)
                    {
                        ...
                        //设置父子关系.
                        BP.WF.Dev2Interface.SetParentInfo(sub.SubFlowNo, subWorkID, this.HisGenerWorkFlow.PWorkID);
        ...
        private void WhenTranscactionRollbackError(Exception ex)
        {
            ...
            try
            {
                ...
                if (gwf.RetrieveFromDBSources() == 0)
                    return;
                //还原WF_GenerWorkList
                if (gwf.IsOver == false && this.HisNode.ItIsStartNode == false)
                {
        ...
        public void DoCopyCurrentWorkDataToRpt()
        {
            ...
            if (this.HisNode.ItIsEndNode || this.IsStopFlow)
                rptGe.WFState = WFState.CompleteEnd;
            else
        ...
        public void AddToTrack(ActionType at, GenerWorkerList gwl, string msg, Int64 subTreadWorkID, Node nd)
        {
            ...
            if (ndFrom.GetValBooleanByKey("SubmitSFEnable") == true)
            {
                ...
                GenerWorkerList gwl1 = new GenerWorkerList();
                gwl1.WorkID = this.HisWork.OID;
                gwl1.NodeID = ndFrom.NodeID;
                gwl1.EmpNo = WebUser.No;
                int i = gwl1.RetrieveFromDBSources();
                if (i == 1)
                    t.NodeData = "@DeptNo=" + gwl1.DeptNo + "@DeptName=" + gwl1.DeptName + "@StaNo=" + gwl1.GetValStringByKey("StaNo");
            }
            else
        ...
        public void AddToTrack(ActionType at, string toEmp, string toEmpName, int toNDid, string toNDName, string msg, Node ndFrom, string tag = null)
        {
            ...
            {
                //获取当前人员的GenerWorkerList数据
                GenerWorkerList gwl = new GenerWorkerList(this.HisWork.OID, ndFrom.NodeID, WebUser.No);
                t.NodeData = "@DeptNo=" + gwl.DeptNo + "@DeptName=" + gwl.DeptName + "@StaNo=" + gwl.GetValStringByKey("StaNo");
            }
            else
                t.NodeData = "@DeptNo=" + WebUser.DeptNo + "@DeptName=" + WebUser.DeptName;
        ...
        public void SendMsgToThem(GenerWorkerLists gwls)
        {
            ...
            {
                if (pm.EventNo.Equals(EventListNode.WorkArrive) == false)
                    continue;
        ...
        private bool IsStopFlow
        {
            ...
            set
            {
                ...
                if (_IsStopFlow == true)
                {
                    ...
                    {
                        this.rptGe.WFState = WFState.CompleteEnd;
                        this.rptGe.Update("WFState", (int)WFState.CompleteEnd);
                    }
                }
            }
        }
...
#warning 为了不让其显示在途的工作需要， =3 不是正常的处理模式。
        ...
        /// 记录跳转节点
        /// </summary>
        private Node _SkipNode = null; //用于出现跳转中发送抄送节点的问题修改
        ...
        public bool ItIsComplete
        {
            get
            {
                if (this.HisGenerWorkFlow.WFState == WFState.CompleteEnd || this.HisGenerWorkFlow.WFState == WFState.CompleteByNode)
                    return true;
                else
                    return false;
            }
