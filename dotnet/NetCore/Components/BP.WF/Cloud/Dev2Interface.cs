    ...
    public class Dev2Interface
    {
        ...
        public static string Port_CreateOrg(string orgNo, string orgName)
        {
            ...
            try
            {
                ...
                #region 8. SAAS模式创建默认图片.
                string msg = "";
                if (SystemConfig.CCBPMRunModel == Sys.CCBPMRunModel.SAAS)
                {
                    try
                    {
                        string file = SystemConfig.PathOfDataUser + "\\OrgData\\Default.png";
                        string fileTo = SystemConfig.PathOfDataUser + "\\OrgData\\" + orgNo + ".png";
                        System.IO.File.Copy(file, fileTo);

                        string file2 = SystemConfig.PathOfDataUser + "\\OrgData\\DefaultIcon.png";
                        string file2To = SystemConfig.PathOfDataUser + "\\OrgData\\" + orgNo + "Icon.png";
                        System.IO.File.Copy(file2, file2To);
                    }
                    catch (Exception ex)
                    {
                        msg += ex.Message;
                    ...
                    if (type.Equals("2"))
                        pass = BP.Tools.Cryptography.HashPassword(pass);
                    DBAccess.RunSQL("UPDATE Port_Emp SET Pass='" + pass + "' WHERE No='" + orgNo + "_001'");
                    DBAccess.RunSQL("UPDATE Port_Emp SET Pass='" + pass + "' WHERE No='" + orgNo + "_002'");

                }
                #endregion 8. SAAS模式创建默认图片.
        ...
        public static string Port_Login(BP.Cloud.Emp emp)
        {
            ...
            HttpContextHelper.SessionSet("FK_Dept", emp.DeptNo);
            HttpContextHelper.SessionSet("FK_DeptText", emp.DeptText);
            ...
            bool isMyStarter = false, string domain = null, bool isContainFuture = false)
        {
            ...
            string domainSQL = "";
            if (domain == null)
                domainSQL = " AND DomainExt='" + domain + "' ";
            //获取用户当前所在的节点
            String currNode = "";
