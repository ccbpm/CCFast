...
namespace BP.WF
{
    ...
    public class CCFlowAPI
    {
            ...
            bool isView = false,string pageType="H5")
        {
            ...
            try
            {
                    ...
                    && DataType.IsNullOrEmpty(wk.HisPassedFrmIDs) == false)
                {
                    ...
                    if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR3 || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
                    {
                ...
                if (isView == false)
                {
                    ...
                    if (nd.ItIsStartNode == false)
                    {
                        ...
                        {
                            if (Dev2Interface.Flow_IsCanDoCurrentWork(gwf.WorkID, WebUser.No) == false)
                                throw new Exception("err@您[" + WebUser.No + "," + WebUser.Name + "]不能执行当前工作, 当前工作已经运转到[" + gwf.NodeName
                                        + "],处理人[" + gwf.TodoEmps + "]。");
        ...
        public static DataTable GetFlowPressMsg(GenerWorkFlow gwf)
        {
            ...
            dtPree.Columns.Add("Title", typeof(string));
            dtPree.Columns.Add("Msg", typeof(string));

            DataRow drMsg = null;
            String sqlPress = "SELECT MobileInfo FROM Sys_SMS WHERE SendTo='" + WebUser.No + "' AND MsgType='DoPress' AND WorkID=" + gwf.WorkID + "  AND IsAlert=0 Order By RDT DESC";
            ...
            if (dtPress.Rows.Count > 0)
            {
                ...
                DBAccess.RunSQL("UPDATE Sys_SMS SET IsAlert=1  WHERE SendTo='" + WebUser.No + "' AND MsgType='DoPress' AND WorkID=" + gwf.WorkID + "  AND IsAlert=0");
            }
        ...
        public static DataTable GetFlowAlertMsg(GenerWorkFlow gwf,Node nd,string fk_flow, Int64 workID, Int64 fid)
        {
            ...
            if (pms.Retrieve(PushMsgAttr.NodeID, gwf.NodeID,
                PushMsgAttr.EventNo, EventListNode.PressAfter) > 0)
            {
