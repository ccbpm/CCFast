...
namespace BP.WF
{
    ...
    public class CCFormAPI : Dev2Interface
    {
        ...
        private static DataSet GenerDBForVSTOExcelFrmModelOfEntity(string enName, object pkval, string atParas, string specDtlFrmID = null)
        {
            ...
            DataTable dtMapDtls = new DataTable();
            dtMapDtls.TableName = "Sys_MapDtl";
            dtMapDtls.Columns.Add("No", typeof(string));
            ...
            foreach (Attr attr in map.Attrs)
            {
                ...
                dr[MapAttrAttr.KeyOfEn] = attr.Key;
            ...
            foreach (EnDtl item in map.Dtls)
            {
                ...
                DataRow drMapDtl = dtMapDtls.NewRow();
                drMapDtl["No"] = item.EnsName;
                dtMapDtls.Rows.Add(drMapDtl);
        ...
        public static DataSet GenerDBForVSTOExcelFrmModel(string frmID, object pkval, string atParas, string specDtlFrmID = null)
        {
            ...
            foreach (MapDtl item in md.MapDtls)
            {
                ...
                foreach (DataRow dr in dtMapAttr.Rows)
                {
                    ...
                    if (me != null && me.DoWay.Equals("0") == false) //有范围限制时
                    {
            ...
            foreach (MapDtl dtl in md.MapDtls)
            {
                ...
                dtDtl.TableName = dtl.No; //edited by liuxc,2017-10-10.如果有别名，则使用别名，没有则使用No
            ...
            foreach (DataRow dr in dtMapAttr.Rows)
            {
                ...
                #region 处理下拉框数据范围. for 小杨.
                me = mes.GetEntityByKey(MapExtAttr.ExtType, MapExtXmlList.AutoFullDLL, MapExtAttr.AttrOfOper, keyOfEn) as MapExt;
                if (me != null && me.DoWay.Equals("0") == false)
                {
                    string fullSQL = me.Doc.Clone() as string;
                    if (DataType.IsNullOrEmpty(fullSQL) == true)
                        throw new Exception("err@没有给AutoFullDLL配置SQL：MapExt：=" + me.MyPK + ",原始的配置SQL为:" + me.Doc);
        ...
        public static Entity DealPageLoadFullVue(Entity en, MapExts mapExts, MapAttrs mattrs)
        {
            ...
            foreach (MapExt mapExt in mapExts)
            {
                ...
                {
                    mapExt.SetValByKey("Tag5", mapExt.DoWay);
           ...
           Int64 workid = 0, Int64 fid = 0, Int64 pworkid = 0, bool isContantSelf = true, int fk_node = 0, string fk_mapData = null)
        {
            ...
            //查询使用的workId
            string ctrlWayId = "";
            if (FK_FrmAttachment.Contains("AthMDtl") == true || athDesc.GetParaBoolen("IsDtlAth") == true || athDesc.FrmID.StartsWith("TS."))
                ctrlWayId = pkval;
            else
            {
                MapData mapData = new MapData(athDesc.FrmID);
                if (fk_node == 0 && (mapData.EntityType == EntityType.FrmDict || mapData.EntityType == EntityType.FrmBill || mapData.EntityType == EntityType.FrmEntityNoName))
                    ctrlWayId = pkval;
                else
            ...
            //从表附件
            if (FK_FrmAttachment.Contains("AthMDtl") || athDesc.GetParaBoolen("IsDtlAth") == true || athDesc.FrmID.StartsWith("TS."))
            {
        ...
        /// <param name="specDtlFrmID">指定明细表的参数，如果为空就标识主表数据，否则就是从表数据.</param>
        /// <returns>数据</returns>
        public static DataSet GenerDBForCCFormDtl(string frmID, MapDtl dtl, string pkval, string atParas, string dtlRefPKVal, Int64 fid, bool ishistoryData = false, String trackID = "")
        {
            //数据容器,就是要返回的对象.
            DataSet myds = new DataSet();
            //实体.
            ...
            else
            {
                ...
                en.SetValByKey("OID", int.Parse(pkval));
                if (en.RetrieveFromDBSources() == 0)
                {
                    en.SetValByKey("OID", int.Parse(dtlRefPKVal));
                    en.RetrieveFromDBSources();
                }
            }
            ...
            attachments.Retrieve(FrmAttachmentAttr.FK_MapData, dtl.No);
            DataTable Sys_FrmAttachment = attachments.ToDataTableField("Sys_FrmAttachment");
            ...
            foreach (MapAttr attr in attrs)
            {
                ...
                if (attr.LGType == FieldTypeS.Enum)
                {
                    ...
                    {
                        SysEnumMain main = new SysEnumMain(attr.UIBindKey);
                        if (main.EnumType == 1)
            ...
            else
            {
                ...
                if (DataType.IsNullOrEmpty(trackID) == false)
                {
                    ...
                    if (i == 1)
                    {
                        String dtlStr = DBAccess.GetBigTextFromDB("Sys_FrmDBVer", "MyPK", ver.MyPK, "FrmDtlDB");
                        DataSet dtlDS = BP.Tools.Json.ToDataSet(dtlStr);
                        dtDtl = dtlDS.Tables[dtl.No];
                    }
                }
                else
                {
            ...
            if (dtDtl.Rows.Count == 0)
            {
                ...
                if (DataType.IsNullOrEmpty(dtl.InitDBAttrs) == false)
                {
                    ...
                    MapAttrs mapAttrs = dtl.MapAttrs;
                    MapAttr attr = null;
                    foreach (string keyOfEn in keys)
                    {
                        Entity ent = mapAttrs.GetEntityByKey(dtl.No + "_" + keyOfEn);
                        if (ent == null)
                            continue;
                        attr = ent as MapAttr;
                        ...
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            if (mapAttrs.Contains(dtl.No + "_" + keyOfEn + "T"))
                                endtl.SetValByKey(keyOfEn + "T", dr[1]);
                ...
                if (me != null && me.DoWay.Equals("None") == false && DataType.IsNullOrEmpty(me.Doc) == false)
                {
                    string paras = "";
                    ...
                    {
                        //值为空或者null不替换
                        if (row[key] == null || row[key].Equals("") == true || key.Equals("OID"))
                            continue;
                        paras += "@"+key+"=" + row[key].ToString();
                    }
                    me.SetValByKey("Tag5", me.DoWay);
                    me.SetValByKey("Tag1", dtl.No);
                    me.GetFullDataDtl(paras, en.GetValStringByKey("OID"));
            ...
            {
                //如果为损溢表，需要判断是否增加数据或者修改数据
                if (dtl.GetValIntByKey(MapDtlAttr.ListShowModel) == 6)
                {
                    string treeKey = dtl.InitDBAttrs;
                    MapAttrs mapAttrs = dtl.MapAttrs;
                    Entity ent = mapAttrs.GetEntityByKey(dtl.No + "_" + treeKey);
                    if (ent != null)
                    {
                        MapAttr attr = ent as MapAttr;
                        DataTable dt = myds.Tables[attr.UIBindKey];
                        if (dt != null)
                        {
                            GEDtl endtl = null;
                            string treeNos = ",";
                            ...
                            {
                                //判断从表数据是否仍在数据源集合中
                                DataRow[] dtr = dt.Select("No=" + dr[treeKey]);
                                //删除从表数据
                                if (dtr.Length == 0)
                                {
                                    isHaveChange = true;
                                    DBAccess.RunSQL("DELETE FROM " + dtl.PTable + " WHERE OID=" + dr["OID"]);
                                    continue;
                                }
                                treeNos += dr[treeKey] + ",";
                                if (attr.UIContralType == UIContralType.DDL && attr.LGType == FieldTypeS.Normal)
                                {
                                    //判断KeyOfEnT是否相同
                                   if(dr[treeKey + "T"].ToString().Equals(dtr[0][1].ToString()) == false)
                                    {
                                        isHaveChange = true;
                                        DBAccess.RunSQL("UPDATE " + dtl.PTable + " SET " + treeKey + "T ='" + dtr[0][1] + "' WHERE OID=" + dr["OID"]);
                                    }
                                    continue;
                                }
                            ...
                            foreach (DataRow dr in dt.Rows)
                            {
                                if (treeNos.Contains("," + dr[0].ToString() + ","))
                                    continue;
                                isHaveChange = true;
                                endtl = new GEDtl(dtl.No);
                                endtl.ResetDefaultVal();
                                endtl.SetValByKey(attr.KeyOfEn, dr[0]);
                                if (mapAttrs.Contains(dtl.No + "_" + attr.KeyOfEn + "T"))
                                    endtl.SetValByKey(attr.KeyOfEn + "T", dr[1]);
                                endtl.RefPK = dtlRefPKVal;
                                endtl.FID = fid;
                                endtl.Insert();
                            }
                            if(isHaveChange== true)
                                dtDtl = GetDtlInfo(dtl, en, dtlRefPKVal);
                        }
                    }
                }
        ...
        public static DataTable GetDtlInfo(MapDtl dtl, Entity en, string dtlRefPKVal, bool isReload = false)
        {
            QueryObject qo = null;
            GEDtls dtls = new GEDtls(dtl.No);
            try
        ...
        public static int GetDtlCount(MapDtl dtl, Entity en, String dtlRefPKVal, bool isReload)
        {
            QueryObject qo = null;
            GEDtls dtls = new GEDtls(dtl.No);
            try
            ...
            catch (Exception ex)
            {
                ...
                Cache.SQL_Cache.Remove(dtl.No);
                if (isReload == false)
                    return GetDtlCount(dtl, en, dtlRefPKVal, true);
                else
                    throw new Exception("获取从表[" + dtl.Name + "]失败,错误:" + ex.Message);
            }
