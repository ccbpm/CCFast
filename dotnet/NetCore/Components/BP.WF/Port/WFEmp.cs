...
namespace BP.WF.Port
{
    ...
    public class WFEmp : EntityNoName
    {
       ...
       /* public string Email
        ...
        {
            //1. 获取邮件的配置信息.
            string sql = "SELECT * FROM WF_Emp WHERE No='"+WebUser.No+"'";
            DataTable dt= DBAccess.RunSQLReturnTable(sql);
            string EM_SeverType = dt.Rows[0]["EM_SeverType"] as string;
            string EM_Email = dt.Rows[0]["EM_Email"] as string;
            string EM_Pass = dt.Rows[0]["EM_Pass"] as string;
            string EM_IMAP_SeverIP = dt.Rows[0]["EM_IMAP_SeverIP"] as string;
            string EM_IMAP_Port = dt.Rows[0]["EM_IMAP_Port"] as string;
            string EM_IMAP_SSL = dt.Rows[0]["EM_IMAP_SSL"] as string;
            string EM_SMTP_SeverIP = dt.Rows[0]["EM_SMTP_SeverIP"] as string;
            string EM_SMTP_Port = dt.Rows[0]["EM_SMTP_Port"] as string;
            string EM_SMTP_SSL = dt.Rows[0]["EM_SMTP_SSL"] as string;
            //2. 获取邮件.
            try
            {
            ...
            return "执行成功,收取到:"+dt.Rows.Count+"新邮件.";
        }
