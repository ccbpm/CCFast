...
namespace BP.Port.WeiXin
{
    ...
    public class WeiXinEntity
    {
        ...
        /// <param name="accessToken"></param>
        /// <returns></returns>
        public static UserEntity getUserInfo(string userid, string accessToken)
        {
            ...
            string str= DataType.ReadURLContext(url, 39000, Encoding.UTF8);
            BP.DA.Log.DebugWriteError(str);
            UserEntity user = FormatToJson.ParseFromJson<UserEntity>(str);
            return user;
        }
        #endregion 基本配置.
