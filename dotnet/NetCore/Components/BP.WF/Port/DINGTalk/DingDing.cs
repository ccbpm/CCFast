...
namespace BP.Port.DINGTalk
{
    ...
    public class DingDing
    {
        ...
        public string GetUserID(string code)
        {
            ...
            try
            {
                ...
                if (!DataType.IsNullOrEmpty(user.userid)) {
                    UserInfoResult result = getUserInfo(access_token,user.userid);
        ...
        public UserInfoResult getUserInfo(string access_token,string userid) {
...
//nameOfPath.Do();
                ...
                str = new HttpWebResponseUtility().HttpResponseGet(url);
                //获取返回参数
                Ding_Role_request ding_Role_Request = FormatToJson.ParseFromJson<Ding_Role_request>(str);
                BP.DA.Log.DebugWriteError("Ding_Role_request:"+str);
                if (!ding_Role_Request.errcode.Equals("0"))
                    return false;
                //获取返回值
                Ding_Role_PageVo ding_Role_PageVo = ding_Role_Request.result;
                //获取角色分组
                List<Ding_Role_Group_List> ding_Role_Group_Lists = ding_Role_PageVo.list;
                foreach (Ding_Role_Group_List group_List in ding_Role_Group_Lists)
                {
                    //增加角色分组
                    StationType stationType = new StationType();
                    stationType.No = group_List.groupId;
                    stationType.Name = group_List.name;
                    stationType.DirectInsert();

                    //同步角色
                    List<Ding_Role_List> ding_Role_Lists = group_List.roles;
                    foreach (Ding_Role_List role in ding_Role_Lists)
                    {
                        Station station = new Station();
                        station.No = role.id;
                        station.Name = role.name;
                        station.FK_StationType=group_List.groupId;
                        station.DirectInsert();

                        //开始同步人员-部门-岗位表
                        string role_user_url = "https://oapi.dingtalk.com/topapi/role/simplelist?access_token=" + access_token;

                        IDictionary<string, object> bodyStr = new Dictionary<string, object>();
                        bodyStr.Add("role_id", role.id);
                        str = new HttpWebResponseUtility().HttpResponsePost_Json(role_user_url, BP.Tools.Json.ToJson(bodyStr));
                        BP.DA.Log.DebugWriteError("Ding_Role_User_Request:" + str);
                        //str = new HttpWebResponseUtility().HttpResponseGet(role_user_url);
                        Ding_Role_User_Request request= FormatToJson.ParseFromJson<Ding_Role_User_Request>(str);
                        Ding_Role_User_PageVo pageVo = request.result;
                        List<Ding_Role_User_List> users = pageVo.list;
                        foreach (Ding_Role_User_List user in users)
                        {
                            //获取用户详细信息
                            Emp emp=new Emp(user.userid);
                            //插入部门人员岗位表
                            DeptEmpStation deptEmpStation = new DeptEmpStation();
                            deptEmpStation.EmpNo = user.userid;
                            deptEmpStation.StationNo = role.id;
                            deptEmpStation.DeptNo = emp.DeptNo;
                            deptEmpStation.MyPK = emp.DeptNo + "_" + role.id + "_" + user.userid;
