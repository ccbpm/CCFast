...
namespace BP.WF
{
    ...
    public class Glo
    {
        ...
        public static string UpdateMapExtH5()
        {
            ...
            SFProcedure pro = new SFProcedure();
            ...
            DBAccess.RunSQL("UPDATE Sys_FrmEvent SET RefPKVal=FrmID WHERE (RefPKVal Is Null Or RefPKVal='') AND (FK_Flow Is Null or FK_Flow='' )");
            ...
            #region 升级mapExt full data.
            string mysql1 = "SELECT MyPK FROM Sys_MapExt WHERE ExtType='FullData' AND DoWay='1'";
            ...
            foreach (MapExt item in exts)
            {
                ...
                item.SetValByKey("ExtType", "TBFullCtrl");
                ...
                //分析Tag1 ； 填充从表。
                // $Dict_YaoPinJieYuDengJiDtl1: select a.YongYaoSJ YongYaoRiQi, a.PiCi PiCi, a.PeiZhiFHR PeiYaoRen, a.YongLiang YongLiang, a.ZuiXiaoJL ZuiXiaoJiLiang, a.ZuiXiaoJLDW YongLiangDANWei, a.FaYaoSL ShuLiang, a.HisBarCode PingQianHao, a.BingRenXM BingRenXingMing, a.KeShiMC KeShi, a.ChuangWei ChuangWei  from V_QMS_Drug_FeiZhengZhi_MX a   where 1 = 1    and a.YaoPinBH = ~@key~and a.PeiSongRQ = ~@PeiSongRiQi~order by a.YongYaoSJ,a.PiCi,a.PeiZhiFHR
                //插入从表数据.
                if (DataType.IsNullOrEmpty(item.Tag1) == false && item.Tag1.Contains(":"))
                {
                    string[] strs = item.Tag1.Split(':');
                    string dtlName = strs[0].Replace("$", "");
                    string dtlSql = strs[1].ToString();
                    MapExt me = new MapExt();
                    me.MyPK = item.FrmID + "_" + item.AttrOfOper + "_" + item.ExtModel + "_" + dtlName;
                    ...
                    me.RefPKVal = item.FrmID + "_" + item.AttrOfOper + "_" + item.ExtModel;
                    me.Insert();
                }
            }
            ...
            foreach (MapExt item in exts)
            {
                ...
                item.AttrOfOper = "RenYuanJianKangDangAn";
                ...
                if (DataType.IsNullOrEmpty(item.Tag1) == false && item.Tag1.Contains(":"))
                {
                    string[] strs2 = item.Tag1.Split('$');

                    foreach (string mystr in strs2)
                    {
                        ...
                        string dtlID = strs[0].Replace("$", "");
                        string dtlSql = strs[1].ToString();
                        MapExt me = new MapExt();
                        me.MyPK = dtlID + "_PageLoadFullDtl";
                        me.ExtType = "PageLoadFullDtl";
                        me.ExtModel = "PageLoadFullDtl";
                        me.DoWay = "Self";
                        me.Doc = dtlSql.Replace("$" + dtlID, "");
                        me.Tag1 = dtlID;
                        me.AttrOfOper = dtlID;
                        me.setFK_MapData(dtlID);
                        me.ItisLoadFull = true;
                        me.Insert();
                    }
                }
            }
            ...
            //查询有问题.
            mysql1 = "select MyPK from Sys_MapExt WHERE MyPK='ActiveDDL_'+FK_MapData+'_'+AttrOfOper";
            exts = new MapExts();
            exts.RetrieveInSQL(mysql1);
            foreach (MapExt item in exts)
            {
            ...
            foreach (MapAttr item in attrsBtn)
            {
                ...
                // $Dict_YaoPinJieYuDengJiDtl1: select a.YongYaoSJ YongYaoRiQi, a.PiCi PiCi, a.PeiZhiFHR PeiYaoRen, a.YongLiang YongLiang, a.ZuiXiaoJL ZuiXiaoJiLiang, a.ZuiXiaoJLDW YongLiangDANWei, a.FaYaoSL ShuLiang, a.HisBarCode PingQianHao, a.BingRenXM BingRenXingMing, a.KeShiMC KeShi, a.ChuangWei ChuangWei  from V_QMS_Drug_FeiZhengZhi_MX a   where 1 = 1    and a.YaoPinBH = ~@key~and a.PeiSongRQ = ~@PeiSongRiQi~order by a.YongYaoSJ,a.PiCi,a.PeiZhiFHR
                //插入从表数据.
                if (DataType.IsNullOrEmpty(ext.Tag1) == false && ext.Tag1.Contains(":"))
                {
                    string[] strs = ext.Tag1.Split(':');
                    string dtlName = strs[0].Replace("$", "");
                    string dtlSql = strs[1].ToString();
                    MapExt me = new MapExt();
                    me.MyPK = item.FrmID + "_" + ext.AttrOfOper + "_" + ext.ExtModel + "_" + dtlName;
                    me.ExtType = "FullDataDtl";
                    me.DoWay = "Self";
                    me.Doc = dtlSql;
                    me.Tag1 = dtlName;
                    me.AttrOfOper = ext.AttrOfOper;
                    me.setFK_MapData(item.FrmID);
                    me.SetPara("EnName", "TS.MapExt.FullDtlSelf");
                    me.RefPKVal = ext.FrmID + "_" + ext.AttrOfOper + "_" + ext.ExtModel;
                    me.Insert();
                }
            }
            #endregion 升级按钮. OK.
        ...
        /// 当前版本号-为了升级使用.
        /// </summary>
        public static int Ver = 20240918;
        /// <summary>
        /// 执行升级
        /// </summary>
        ...
        public static string UpdataCCFlowVer()
        {
            ...
            ////检查缺少OID的表单.
            //MapDatas mdFrms = new MapDatas();
            //mdFrms.RetrieveAll();
            //string err2= "";
            //foreach (MapData md in mdFrms)
            //{
            //    if (md.EntityType == EntityType.EntityTree || md.EntityType == EntityType.FrmEntityNoName)
            //    {
            //        string sqlOID = "SELECT Count(*) as Num FROM Sys_MapAttr WHERE KeyOfEn='No' AND FK_MapData='" + md.No + "'";
            //        int countNum = DBAccess.RunSQLReturnValInt(sqlOID);
            //        if (countNum == 0)
            //            err2 += "@表单:" + md.No + "," + md.Name + ",不存在No字段,类型:"+md.EntityType.ToString();
            //    }
            //    else
            //    {
            //        string sqlOID = "SELECT Count(*) as Num FROM Sys_MapAttr WHERE KeyOfEn='OID' AND FK_MapData='" + md.No + "'";
            //        int countNum = DBAccess.RunSQLReturnValInt(sqlOID);
            //        if (countNum == 0)
            //            err2 += "@表单:" + md.No + "," + md.Name + ",不存在OID字段.类型:"+md.EntityType.ToString();
            //    }
            //}
            //return "err@" + err2;
            //
         ...
         //   if (DBAccess.IsExitsTableCol("Sys_MapData", "BillCheckModel") == true)
           ...
           if (BP.Difference.SystemConfig.AppCenterDBType.ToString() == "MSSQL")
            ...
            try
            {
                ...
                BP.Sys.SFTables sftables = new BP.Sys.SFTables();
                sftables.Retrieve("FK_SFDBSrc", "localWebApi");
                if (sftables.Count == 0)
                {
                    BP.Sys.SFTable sftable = new BP.Sys.SFTable();
                    sftable.No = "Demo_DictNoName_Emps";
                    sftable.Name = "人员列表";
                    sftable.SetValByKey("FK_SFDBSrc", "localWebApi");
                    sftable.SetValByKey("RequestMethod", "Get");
                    sftable.DirectInsert();
                    sftable = new BP.Sys.SFTable();
                    sftable.No = "Demo_DictTree_Depts";
                    sftable.Name = "部门列表";
                    sftable.SetValByKey("FK_SFDBSrc", "localWebApi");
                    sftable.CodeStruct = CodeStruct.Tree;
                    sftable.SetValByKey("RequestMethod", "Get");
                    sftable.DirectInsert();
                }
                BP.Sys.SFSearchs sfSearchs = new BP.Sys.SFSearchs();
                sfSearchs.Retrieve("FK_SFDBSrc", "localWebApi");
                if (sfSearchs.Count == 0)
                {

                }
                BP.Sys.SFProcedures procedures = new BP.Sys.SFProcedures();
                procedures.Retrieve("FK_SFDBSrc", "localWebApi");
                if (procedures.Count == 0)
                {
                    BP.Sys.SFProcedure procedure = new BP.Sys.SFProcedure();
                    procedure.No = "Demo_Produces";
                    procedure.Name = "过程";
                    procedure.SetValByKey("ConnString", "http://localhost:56146/WF/Demo/");
                    procedure.SetValByKey("FK_SFDBSrc", "localWebApi");
                    procedure.SetValByKey("RequestMethod", "Get");
        ...
        public static bool CondExpSQL(string sqlExp, Hashtable ht, Int64 myWorkID)
        {
            ...
            sql = sql.Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
        ...
        public static string DealExp(string exp, Entity en, string errInfo = "")
        {
            ...
            exp = exp.Replace("@WebUser.DeptNo;", WebUser.DeptNo);
            ...
            if (en != null)
            {
                exp = exp.Replace("@PWorkID", en.PKVal.ToString());
        ...
        public static string DealSQLExp(string exp, Entity en, string errInfo)
        {
            ...
            exp = exp.Replace("@WebUser.FK_DeptName;", WebUser.DeptName);
            exp = exp.Replace("@WebUser.FK_Dept;", WebUser.DeptNo);
            exp = exp.Replace("@WebUser.DeptNo;", WebUser.DeptNo); //@dsm 同步到java
            exp = exp.Replace("@WebUser.DeptName;", WebUser.DeptName);  //@dsm 同步到java
