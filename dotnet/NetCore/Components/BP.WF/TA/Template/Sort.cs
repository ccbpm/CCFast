...
namespace BP.TA
{
    ...
    public class Sort : EntityTree
    {
        ...
        public string DomainExt
        {
            get
            {
                return this.GetValStrByKey(SortAttr.DomainExt);
            }
            set
            {
                this.SetValByKey(SortAttr.DomainExt, value);
            }
        }
        #endregion 属性.
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(SortAttr.DomainExt, null, "域/系统编号", true, false, 0, 100, 30);
                map.SetHelperAlert(SortAttr.DomainExt, "用于区分不同系统的流程,比如:一个集团有多个子系统每个子系统都有自己的流程,就需要标记那些流程是那个子系统的.");
        ...
        {
            ////更新流程引擎控制表.
            //string sql = "UPDATE WF_GenerWorkFlow SET DomainExt='" + this.DomainExt + "' WHERE FK_Sort='" + this.No + "'";
            //DBAccess.RunSQL(sql);

            //// sql = "UPDATE WF_Flow SET DomainExt='" + this.DomainExt + "' WHERE FK_Sort='" + this.No + "'";
            ////DBAccess.RunSQL(sql);
