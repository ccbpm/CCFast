...
namespace BP.WF
{
    ...
    {
        private static StringBuilder GenerHtmlOfFool(MapData mapData, string frmID, string workid, Entity en, string path, string flowNo = null, string FK_Node = null, NodeFormType formType = NodeFormType.FoolForm)
        {
            ...
            {
                //审核意见
                sql = "SELECT NDFromT,Msg,RDT,EmpFromT,EmpFrom,NDFrom FROM ND" + int.Parse(flowNo) + "Track WHERE WorkID=" + Int64.Parse(workid) + " AND ActionType=" + (int)ActionType.WorkCheck + " ORDER BY RDT ";
            ...
            sb.Append("<div  style='border:0px;display:flex;align-items:center;justify-content:space-between;' >");
            ...
            foreach (GroupField gf in gfs)
            {
                ...
                if (gf.CtrlID == "" && gf.CtrlType == "")
                {
                    ...
                    foreach (MapAttr attr in mapAttrs)
                    {
                        ...
                        switch (attr.LGType)
                        {
                            case FieldTypeS.Normal:  // 输出普通类型字段.
                                ...
                                else
                                {
                                    ...
                                    string ctrlType = attr.GetParaString("CtrlType");
                                    //如果是关联流程
                                    ...
                                    if (attr.UIContralType == UIContralType.AthShow)
                                    {
                                        ...
                                        ath.setMyPK(attr.MyPK);
                                        if (ath.RetrieveFromDBSources() != 0)
                                        {
                                            foreach (FrmAttachmentDB item in athDBsFrom)
                                            {
                        ...
                        {
                            AtPara atPara = attr.atPara;
                            string checkedTips = atPara.GetValStrByKey("checkedTips");
                            string unCheckedTip = atPara.GetValStrByKey("unCheckedTip");
                            if (DataType.IsNullOrEmpty(text) || text.Equals("0"))
                                text = "[&#10005]" + (DataType.IsNullOrEmpty(unCheckedTip)?attr.Name:unCheckedTip);
                            else
                                text = "[&#10004]" + (DataType.IsNullOrEmpty(checkedTips) ? attr.Name : checkedTips);
                        }
                ...
                if (gf.CtrlType == "Dtl")
                {
                    if (DataType.IsNullOrEmpty(gf.CtrlID) == true)
                        continue;
                    /* 如果是从表 */
                    MapDtl mapdtl = new MapDtl(gf.CtrlID);
                    if(DataType.IsNullOrEmpty(FK_Node)==false && FK_Node.Equals("0") == false)
                    {
                        mapdtl = new MapDtl();
                        mapdtl.No = gf.CtrlID + '_' + FK_Node;
                        int count = mapdtl.RetrieveFromDBSources();
                        if (count == 0)
                            mapdtl.No = gf.CtrlID;
                    }
                    ...
                    try
                    {
                        attrsOfDtls = new MapAttrs(mapdtl.No);
                    }
                    catch (Exception ex)
                    {
                    }
                    ...
                    bool isHaveGradeHeader = DataType.IsNullOrEmpty(gradeHeader) == true ? false : true;
                    bool isSecondHeader = DataType.IsNullOrEmpty(secondHeader) == true ? false : true;
                    sb.Append("<tr><td valign=top colspan=4 >");
                    ...
                    if (isHaveGradeHeader == false && isSecondHeader == false)
                    {
                        ...
                        foreach (MapAttr item in attrsOfDtls)
                        {
                            ...
                            sb.Append("<th style='width:" + columWidth + "%;text-align:center;white-space: normal !important;'>" + item.Name + "</th>");
                        }
                        sb.Append("</tr>");
                        //不存在三级表头
                    }
                    else if (isHaveGradeHeader == false && isSecondHeader == true)
                    {
                        ...
                        foreach (MapAttr item in attrsOfDtls)
                        {
                            ...
                            if (item.UIVisible == false)
                                continue;
                            String[] fields = GetMulitFields(item.KeyOfEn, secondHeader);
                            if (fields == null)
                            {
                                firstStr.Append("<th style='width:" + columWidth + "%;text-align:center;white-space: normal !important;' rowspan=2>" + item.Name + "</th>");
                            }
                            else
                            {
                                ...
                                secondStr.Append("<th style='width:" + columWidth + "%;text-align:center;white-space: normal !important;' rowspan=1>" + item.Name + "</th>");
                            }
                        }
                        secondStr.Append("</tr>");
                        firstStr.Append("</tr>");
                    ...
                    else if (isHaveGradeHeader == true && isSecondHeader == true)
                    {
                        ...
                        foreach (MapAttr item in attrsOfDtls)
                        {
                            ...
                            if (item.UIVisible == false)
                                continue;
                            String[] fields = GetMulitFields(item.KeyOfEn, gradeHeader);

                            if (fields == null)
                            {
                                String[] sfields = GetMulitFields(item.KeyOfEn, secondHeader);
                                if (sfields == null)
                                {
                                    firstStr.Append("<th style='width:" + columWidth + "%;text-align:center;white-space: normal !important;' rowspan=3>" + item.Name + "</th>");
                                }
                                else
                                {
                                    ...
                                    if (tfields == null)
                                    {
                                        ...
                                        secondStr.Append("<th style='width:" + columWidth + "%;text-align:center;white-space: normal !important;' rowspan=2>" + item.Name + "</th>");
                                    ...
                                    else
                                    {
                                        ...
                                        threeStr.Append("<th style='width:" + columWidth + "%;text-align:center;white-space: normal !important;' >" + item.Name + "</th>");
                    ...
                    foreach (GEDtl dtl in dtls)
                    {
                        ...
                        foreach (MapAttr item in attrsOfDtls)
                        {
                            ...
                            {
                                case FieldTypeS.Normal:  // 输出普通类型字段.
                                    if (item.MyDataType == 1 && item.UIContralType == UIContralType.DDL)
                                    {

                                        if (attrsOfDtls.Contains("KeyOfEn",item.KeyOfEn + "Text") == true)
                                            text = dtl.GetValRefTextByKey(item.KeyOfEn);
                                        if (DataType.IsNullOrEmpty(text))
                                            if (attrsOfDtls.Contains("KeyOfEn", item.KeyOfEn + "T") == true)
                ...
                if (gf.CtrlType == "Ath")
                {
                    ...
                    FrmAttachmentDBs athDBs = BP.WF.CCFormAPI.GenerFrmAttachmentDBs(ath, workid, ath.MyPK, DataType.IsNumStr(workid) ? Int64.Parse(workid) : 0);
                    ...
                    if (ath.UploadType == AttachmentUploadType.Multi || ath.UploadType == AttachmentUploadType.Single)
                    {
                        sb.Append("<tr><td valign=top colspan=4 >");
                        sb.Append("<ul>");
                ...
                if (gf.CtrlType == "Frame" && flowNo != null)
                {
                    ...
                    url = url.Replace("@WebUser.DeptNo;", WebUser.DeptNo);
        ...
        private static void GenerHtmlOfDevelop(MapData mapData, string frmID, string workid, Entity en, string path, string indexFile, string flowNo = null, string FK_Node = null)
        {
            ...
            {
                //获取审核组件的信息
                string sql = "SELECT NDFromT,Msg,RDT,EmpFromT,EmpFrom,NDFrom FROM ND" + int.Parse(flowNo) + "Track WHERE WorkID=" + Int64.Parse(workid) + " AND ActionType=" + (int)ActionType.WorkCheck + " ORDER BY RDT ";
                dt = DBAccess.RunSQLReturnTable(sql);
            }
        ...
        private static string GetDtlHtmlByID(MapDtl dtl, string workid, float width)
        {
        ...
        private static string GetAthHtmlByID(FrmAttachment ath, string workid, string path)
        {
            StringBuilder sb = new System.Text.StringBuilder();
        ...
        public static string MakeCCFormToPDF(Node node, Int64 workid, string flowNo, string pdfName, string filePath)
        {
            ...
            {
                resultMsg = setPDFPath("ND" + node.NodeID, workid.ToString(), flowNo, gwf);
            ...
            if ((int)node.HisFormType == (int)NodeFormType.SheetTree)
            {
                ...
                foreach (FrmNode item in nds)
                {
                    ...
                    if (flowNo != null && item.WhoIsPK == WhoIsPK.PWorkID) //如果是父子流程
                        workid = gwf.PWorkID;
                    //获取表单的信息执行打印
                    string billUrl = BP.Difference.SystemConfig.PathOfDataUser + "InstancePacketOfData/" + "ND" + node.NodeID + "/" + workid + "/" + item.FrmID + "index.htm";
                    resultMsg = MakeHtmlDocument(item.FrmID, workid, flowNo, path, billUrl, "ND" + node.NodeID);

                    if (resultMsg.IndexOf("err@") != -1)
                        return resultMsg;

                    ht.Add("htm_" + item.FrmID, BP.Difference.SystemConfig.GetValByKey("HostURLOfBS", "../../DataUser/") + "/InstancePacketOfData/" + "ND" + node.NodeID + "/" + workid + "/" + item.FrmID + "index.htm");

                    //#region 把所有的文件做成一个zip文件.
                    if (System.IO.Directory.Exists(pdfTempPath) == false)
                        System.IO.Directory.CreateDirectory(pdfTempPath);

                    string pdfFormFile = pdfTempPath + "/" + item.FrmID + ".pdf";
                    string pdfFileExe = BP.Difference.SystemConfig.PathOfDataUser + "ThirdpartySoftware/wkhtmltox/wkhtmltopdf.exe";
                    try
                    {
        ...
        public static string MakeBillToPDF(string frmId, Int64 workid, string filePath, string pdfName)
        {
            ...
            if (DataType.IsNullOrEmpty(pdfPath) == true)
                pdfPath = path + "/pdf";
            DataRow dr = null;
            resultMsg = setPDFPath(frmId, workid.ToString(), null, null);
            if (resultMsg.IndexOf("err@") != -1)
                return resultMsg;
        ...
        /// </summary>
        /// <param name="frmId">表单ID</param>
        /// <param name="pkval">数据ID</param>
        /// <param name="filePath">PDF路径</param>
        /// <param name="pdfName">PDF名称</param>
        /// <returns></returns>
        public static string MakeEntityNoNameToPDF(string frmId, string pkval, string filePath, string pdfName)
        {
            string resultMsg = "";
            ...
            //存放信息地址
            string path = BP.Difference.SystemConfig.PathOfDataUser + "InstancePacketOfData/" + bill.No + "/" + pkval;
            ...
            //获取表单的信息执行打印
            string billUrl = BP.Difference.SystemConfig.PathOfDataUser + "InstancePacketOfData/" + bill.No + "/" + pkval + "/" + "index.htm";
            resultMsg = MakeHtmlNoNameDocument(bill.No, pkval, path, billUrl);

            if (resultMsg.IndexOf("err@") != -1)
                return resultMsg;

            ht.Add("htm", BP.Difference.SystemConfig.GetValByKey("HostURLOfBS", "../../DataUser/") + "InstancePacketOfData/" + frmId + "/" + pkval + "/" + "index.htm");
            ...
            try
            {
                Html2Pdf(pdfFileExe, resultMsg, pdfFormFile);
                if (DataType.IsNullOrEmpty(filePath) == true)
                    ht.Add("pdf", BP.Difference.SystemConfig.GetValByKey("HostURLOfBS", "../../DataUser/") + "InstancePacketOfData/" + frmId + "/" + pkval + "/pdf/" + pdfName + ".pdf");
                else
                    ht.Add("pdf", pdfPath + "/" + pdfName + ".pdf");
            }
            catch (Exception ex)
            {
                ...
                Html2Pdf(pdfFileExe, resultMsg, pdfFormFile);
                ht.Add("pdf", BP.Difference.SystemConfig.GetValByKey("HostURLOfBS", "") + "/InstancePacketOfData/" + frmId + "/" + pkval + "/pdf/" + bill.Name + ".pdf");
            }
            ...
            Int64 workid, string flowNo, string fileNameFormat, bool urlIsHostUrl, string basePath)
        {
            ...
            //获取表单的信息执行打印
            string billUrl = BP.Difference.SystemConfig.PathOfDataUser + "InstancePacketOfData/" + "ND" + node.NodeID + "/" + workid + "/" + frmNode.FrmID + "index.htm";
            resultMsg = MakeHtmlDocument(frmNode.FrmID, workid, flowNo, path, billUrl, "ND" + node.NodeID);
            ...
            string pdfFormFile = pdfPath + "/" + frmNode.FrmID + ".pdf";
            string pdfFileExe = BP.Difference.SystemConfig.PathOfDataUser + "ThirdpartySoftware/wkhtmltox/wkhtmltopdf.exe";
            try
            {
                Html2Pdf(pdfFileExe, resultMsg, pdfFormFile);
                if (urlIsHostUrl == false)
                    ht.Add("pdf", BP.Difference.SystemConfig.GetValByKey("HostURLOfBS", "../../DataUser/") + "InstancePacketOfData/" + "ND" + node.NodeID + "/" + workid + "/pdf/" + frmNode.FrmID + ".pdf");
                else
                    ht.Add("pdf", BP.Difference.SystemConfig.GetValByKey("HostURL", "") + "/DataUser/InstancePacketOfData/" + "ND" + node.NodeID + "/" + workid + "/pdf/" + frmNode.FrmID + ".pdf");
            ...
            catch (Exception ex)
            {
                ...
                Html2Pdf(pdfFileExe, resultMsg, pdfFormFile);
                ht.Add("pdf", BP.Difference.SystemConfig.GetValByKey("HostURLOfBS", "") + "/InstancePacketOfData/" + "ND" + node.NodeID + "/" + workid + "/pdf/" + frmNode.FrmID + ".pdf");
            }
        ...
        //前期文件的准备
        private static string setPDFPath(string frmID, string workid, string flowNo, GenerWorkFlow gwf)
        {
            //准备目录文件.
        ...
        public static string MakeHtmlDocument(string frmID, Int64 workid, string flowNo, string path, string indexFile, string nodeID)
        {
            try
            {
                ...
                if (mapData.HisFrmType == FrmType.Develop)
                {
                    ...
                    GenerHtmlOfDevelop(mapData, mapData.No, workid.ToString(), enn, path, indexFile, flowNo, nodeID);
                    return indexFile;
                }
                ...
                if (System.IO.File.Exists(tempFile) == false)
                {
                    ...
                    if (mapData.HisFrmType == FrmType.FoolForm)
                    {
                        ...
                        sb = BP.WF.MakeForm2Html.GenerHtmlOfFool(mapData, frmID, workid.ToString(), en, path, flowNo, nodeID);
        ...
        public static string MakeHtmlNoNameDocument(string frmID, string pkval, string path, string indexFile)
        {
            try
            {
                //生成表单信息.
                ...
                if (mapData.HisFrmType == FrmType.Develop)
                {
                    ...
                    DataType.WriteFile(indexFile, ddocs);
                    GenerHtmlOfDevelop(mapData, mapData.No, pkval, en, path, indexFile,null,null);
                    return indexFile;
                }
                ...
                {
                    docs = DataType.ReadTextFile(BP.Difference.SystemConfig.PathOfDataUser + "InstancePacketOfData/Template/indexFool.htm");
                    sb = BP.WF.MakeForm2Html.GenerHtmlOfFool(mapData, frmID, pkval, en, path, null, null);
                    docs = docs.Replace("@Width", mapData.FrmW.ToString() + "px");
                    docs = docs.Replace("@EndInfo", "");
                }
