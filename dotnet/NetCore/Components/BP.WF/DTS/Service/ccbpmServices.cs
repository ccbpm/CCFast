    ...
    public class ccbpmServices : Method
    {
        ...
        private void DoOverDueFlow()
        {
            ...
            foreach (DataRow row in generTab.Rows)
            {
                ...
                int count = pushMsgs.Retrieve(PushMsgAttr.FlowNo, node.FlowNo, PushMsgAttr.NodeID, node.NodeID, PushMsgAttr.EventNo, EventListNode.NodeWarning);
                int maxHour = 0;
                int minHour = 0;
                if (count != 0)
                {
            ...
            foreach (DataRow row in generTab.Rows)
            {
                ...
                try
                {
                    ...
                    #region 启动逾期消息设置
                    PushMsgs pushMsgs = new PushMsgs();
                    int count = pushMsgs.Retrieve(PushMsgAttr.FlowNo, node.FlowNo, PushMsgAttr.NodeID, node.NodeID, PushMsgAttr.EventNo, EventListNode.NodeOverDue);
                    int maxDay = 0;
                    int minDay = 0;
                    if (count != 0)
                    {
