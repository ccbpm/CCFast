...
namespace BP.WF.Data
{
    ...
    public class GenerWorkFlowExt : Entity
    {
        ...
        public WFState WFState
        {
            ...
            set
            {
                if (value == WF.WFState.CompleteByNode || value == WF.WFState.CompleteEnd)
        ...
        public string RepairDataIt()
        {
            ...
            {
                //没有数据就插入.
                rpt.FlowStartRDT = this.RDT;
                rpt.Title = this.Title;
                rpt.FlowStarter = this.Starter;
                rpt.NY = DataType.CurrentYearMonth;
                rpt.FlowEmps = this.Emps;
                rpt.FlowEnderRDT = this.SDTOfNode;
                rpt.DeptNo = this.FK_Dept;
                rpt.DeptName = this.DeptName;
                rpt.FlowEnder = this.TodoEmps;
                rpt.FlowEndNode = this.FK_Node;
                rpt.WFState = this.WFState;
                rpt.FID = this.FID;
                rpt.SaveAsOID((int)this.WorkID);
            }
            Work wk = nd.HisWork;
            wk.OID = this.WorkID;
