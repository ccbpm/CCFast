    ...
    public class WF_MyCC : DirectoryPageBase
    {
        ...
        public string MyCC_Init()
        {
            ...
            if (ccs.Count == 1)
            {
                ...
                if (nd.HisNodeType == NodeType.CCNode)
                {
                    this.NodeID = int.Parse(ccs[0].GetValStringByKey("NodeIDWork"));
                    _currNode = new Node(this.NodeID);
                }
            }

            //如果当前节点类型是抄送节点类型，表单类型就根据发送过来的节点类型计算
            NodeFormType formType = this.currND.HisFormType;
            int nodeIDWork = this.GetRequestValInt("NodeIDWork");
            if (this.currND.HisNodeType == NodeType.CCNode)
            {
                if (nodeIDWork != 0)
                {
                    Node node = new Node(nodeIDWork);
                    formType = node.HisFormType;
                }
            }
            ...
            if (formType == NodeFormType.SheetTree
                 || formType == NodeFormType.SheetAutoTree
            ...
            if (formType == NodeFormType.FoolForm && this.ItIsMobile == false)
            {
            ...
            //自定义表单
            if ((formType == NodeFormType.SelfForm || this.currFlow.FlowDevModel == FlowDevModel.SelfFrm) && this.ItIsMobile == false)
            {
                if (this.WorkID == 0)
                {
                    currWK = this.currFlow.NewWork();
            ...
            if (formType == NodeFormType.EntityTS)
            {
                ...
                string url = "MyFlowEntityTS.htm?EnName=" + formUrl + "&OID=" + this.WorkID + "&PKVal=" + this.WorkID + "&PageType=MyCC";
                if (this.WorkID == 0)
                {
                    currWK = this.currFlow.NewWork();
                    this.WorkID=currWK.OID;
            ...
            foreach (FrmNode frmNode in frmNodes)
            {
                ...
                switch (frmNode.FrmEnableRole)
                {
                    ...
                    case FrmEnableRole.WhenHaveFrmPara: //判断是否有参数.
                        ...
                        {
                            if (frms.Equals(frmNode.FrmID) ==false)
                                continue;
                        }
