...
namespace BP.WF.HttpHandler
{
    ...
    {
        public static readonly object GWTemplateFileNameLock = new object();
        #region 公文模板处理.
        /// <summary>
        /// 开始节点选择公文模板
        /// </summary>
        ...
        public string GPN_DocTemplates()
        {
            try
            {
                lock (GWTemplateFileNameLock)
                {
                    ...
                    string frmVSTOFilePath = BP.Difference.SystemConfig.PathOfDataUser + "DocFlow/";
                    string frmVSTOFlowFilePath = frmVSTOFilePath + flowNo;
                    if (Directory.Exists(frmVSTOFlowFilePath) == false)
                    {
                        return "err@未找到模板";
                    }
                    DirectoryInfo root = new DirectoryInfo(frmVSTOFlowFilePath);
                    foreach (FileInfo f in root.GetFiles())
                    {
                        ...
                        {
                            int index = f.FullName.IndexOf("DocFlow");
                            string filePath = f.FullName.Substring(index + 7);
                            DataRow drNew = dt.NewRow();
                            drNew["No"] = HttpUtility.UrlEncode(filePath, Encoding.UTF8);
                            drNew["Name"] = HttpUtility.UrlEncode(f.Name, Encoding.UTF8);
                            dt.Rows.Add(drNew);
                        }
                        else
                        {
                            return "err@模板文件不是doc或docx类型";
                        }
                    }
        ...
        /// <returns></returns>
        public object IsGongWenByteExist()
        {
            try
            {
                string nodeId = GetRequestVal("nodeID");
                ...
                Work wk = nd.HisWork;
                wk.SetValByKey("OID", Int64.Parse(workID));
        ...
        /// </summary>
        /// <returns></returns>
        public string Flow_DoFix()
        {
           return BP.WF.Dev2Interface.Flow_DoFix(this.WorkID, false, this.Msg);
        }
        /// <summary>
        /// 执行解冻.
        /// </summary>
        /// <returns></returns>
        public string Flow_DoUnFix()
        {
            return BP.WF.Dev2Interface.Flow_DoUnFix(this.WorkID,  this.Msg);
        }
        /// <summary>
        /// 获得冻结列表.
        /// </summary>
        ...
        public string GenerMsg_Init()
        {
            ...
            //生成消息的ID.
            string sql = "";
            sql = "SELECT RDT FROM WF_GenerWorkerlist WHERE IsPass=0 AND FK_Emp='" + BP.Web.WebUser.No + "' ORDER BY RDT DESC ";
        ...
        public string PrintDoc_Init()
        {
            ...
            if (DataType.IsNullOrEmpty(frmID) == true && (this.NodeID != 0 && this.NodeID != 9999))
            {
                ...
                if (nd.HisFlow.FlowDevModel != FlowDevModel.JiJian && nd.HisFormType == NodeFormType.FoolForm && DataType.IsNullOrEmpty(frmID) == false)
                {
            ...
            if (DataType.IsNullOrEmpty(sourceType) == false && sourceType.Equals("EntityNoName"))
                return PrintDoc_EntityNoNameDoneIt(this.No, frmID, templete);
        ...
        public string PrintDoc_DoneIt(string FrmPrintTemplateNo = null)
        {
            ...
            try
            {
                ...
                string fileName = file;//wk.Row.GetValByKey("WenJianMing").ToString() + ".doc";获取表单的字段作为文件名

                string billUrl = "file@" + fileModelT + "@" + fileName + "@" + BP.WF.Glo.CCFlowAppPath + "DataUser/Bill/" + DateTime.Now.Year + "/" + WebUser.DeptNo + "/" + func.MyPK + "/" + file;
        ...
        public string PrintDoc_FormDoneIt(Node nd, long workID, long fid, string formID, FrmPrintTemplate func)
        {
            ...
            string atPara = DBAccess.RunSQLReturnString("SELECT TempalteFile FROM Sys_FrmPrintTemplate WHERE MyPK='" + func.MyPK + "'");
        ...
        public string PrintDoc_EntityNoNameDoneIt(string pkval, string formID, FrmPrintTemplate func)
        {
            GEEntityNoName en = new GEEntityNoName(formID, pkval);
            MapData mapData = new MapData(formID);
           
            string file = DateTime.Now.Year + "_" + WebUser.DeptNo + "_" + func.MyPK + "_" + pkval + ".doc";
            ...
            try
            {
                ...
                {
                    FrmAttachmentDBs athDBs = new FrmAttachmentDBs();
                    if (athDBs.Retrieve(FrmAttachmentDBAttr.FK_FrmAttachment, athDesc.MyPK, FrmAttachmentDBAttr.RefPKVal, pkval, "RDT") == 0)
                        continue;
        ...
        public string Packup_Init()
        {
            try
            {
                ...
                if (DataType.IsNullOrEmpty(sourceType) == false && sourceType.Equals("EntityNoName"))
                {
                    return MakeForm2Html.MakeEntityNoNameToPDF(this.GetRequestVal("FrmID"), this.No, null, null);
                }
...
#warning 替换变量. todo.
            ...
            try
            {
                ...
                switch (currNode.HisTurnToDeal)
                {
                    case TurnToDeal.SpecUrl:
                        ...
                        myurl = myurl.Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
                    ...
                    default:
                        ...
                        strs = strs.Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
        ...
        public string AccepterOfGener_SelectEmps()
        {
            ...
            if (isPinYin == true)
            {
                ...
                if (emp.Contains("/"))
                {
                    ...
                    if (SystemConfig.AppCenterDBType == DBType.Oracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.GBASE8CByOracle || SystemConfig.AppCenterDBType == DBType.KingBaseR3 || SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
        ...
        public string WorkCheck_Init()
        {
            ...
            int curNodeID = this.NodeID;
            if (curNodeID == 0)
                curNodeID = gwf.NodeID;
            //当前节点的审核信息
            ...
            WorkCheck wc = null;
            if (this.FID != 0)
                wc = new WorkCheck(this.FlowNo, this.NodeID, this.FID, 0, wcDesc.GetValIntByKey("FWCTimeModel", 0));
            else
                wc = new WorkCheck(this.FlowNo, this.NodeID, this.WorkID, this.FID, wcDesc.GetValIntByKey("FWCTimeModel", 0));
            ...
            if (wcDesc.FWCListEnable == true)
            {
                ...
                foreach (Track tk in tks)
                {
                    ...
                    if (tk.NDFrom == this.NodeID
                        ...
                        && (gwf.IsOver==false && (int)gwf.WFState != 12))
                        continue;
 ...
 */
                                        ...
                                        ))
                    {
                        ...
                        if (isLast && isDoc == false && gwf.IsOver==false)
                        {
                            isExitTb_doc = false;
                            row["IsDoc"] = true;
                            isDoc = true;
                ...
                && isReadonly == false && isHaveMyInfo == false)
            {
                ...
                if (wcDesc.FWCOrderModel == FWCOrderModel.SqlAccepter)
                {
                    ...
                    else
                    {
                        ...
                        row["IsDoc"] = true;
                        row["ParentNode"] = 0;
                        row["RDT"] = Dev2Interface.GetCheckRDT(this.FlowNo, this.WorkID, this.NodeID, WebUser.No);
        ...
        public string WorkCheck_Init2019()
        {
            ...
            NodeWorkCheck wcDesc = new NodeWorkCheck(curNodeID); // 当前节点的审核组件
            NodeWorkCheck frmWorkCheck = null;
            ...
            if (this.FID != 0)
                ...
                wc = new WorkCheck(this.FlowNo, this.NodeID, this.WorkID, 0, wcDesc.GetValIntByKey("FWCTimeModel", 0));
             ...
             * */
            string checkerPassed = ",";
            if (gwf.IsOver==false && (int)gwf.WFState != 12)
            {
                //Paras ps = new Paras();
                //ps.SQL = "SELECT FK_Emp FROM WF_GenerWorkerlist WHERE WorkID=" + SystemConfig.AppCenterDBVarStr + "WorkID AND IsPass=1 AND FK_Node=" + SystemConfig.AppCenterDBVarStr + "FK_Node  Order By RDT,CDT";
                //ps.Add("WorkID", this.WorkID);
...
#warning 处理审核信息,删除掉他.
        ...
        public string WorkTrack_Init()
        {
            if (DataType.IsNullOrEmpty(WebUser.No) == true)
                return "err@登录信息丢失,请重新登录.";
            DataSet ds = new DataSet();
            //流程实例信息
            GenerWorkFlow gwf = new GenerWorkFlow(this.WorkID);
            ...
            //审核组件表
            String trackTable = "ND" + Int32.Parse(this.FlowNo) + "Track";
            //当前节点的审核组件
            ...
            if (gwf.IsOver==false)
            {
                ...
                {
                    case DBType.MSSQL:
                        sql = "SELECT DISTINCT(FK_Node) From(SELECT FK_Node From (SELECT ROW_NUMBER() over(ORDER BY RDT) as rn,FK_Node FROM WF_GenerWorkerList WHERE WorkID=" + this.WorkID + ") B) A";
                        break;
                    default:
                        sql = "SELECT DISTINCT(FK_Node) From(SELECT FK_Node From WF_GenerWorkerList WHERE WorkID=" + this.WorkID + "  ORDER BY RDT) A";
                        break;
                }
            ...
            if (isReadonly == true)
                ...
                isCanDo = Dev2Interface.Flow_IsCanDoCurrentWork(this.WorkID, WebUser.No);
            //是否屏蔽正在审批的节点的审批意见(在在途，已完成等查看页面使用)
            bool isShowCurrNodeInfo = true;
            if (isReadonly == true && gwf.WFState == WFState.Runing && gwf.NodeID == this.NodeID)
                isShowCurrNodeInfo = false;
            ...
            tkDt.Columns.Add("SigantureEnabel", typeof(int));
            tkDt.Columns.Add("FWCAth", typeof(int));
            tkDt.Columns.Add("Exer", typeof(string));
            tkDt.Columns.Add("Step", typeof(int));	// 步骤
            //流程附件.
            DataTable athDt = new DataTable("Aths");
            athDt.Columns.Add("NodeID", typeof(int));
            athDt.Columns.Add("MyPK", typeof(string));
            ...
            foreach (Track tk in tracks)
            {
                ActionType actionType = tk.HisActionType;
                //倒叙
                if (wc.FWCTimeModel == 0)
                {
                    if ((trackIdx - 1) >= 0)
                ...
                //  此部分被zhoupeng注释, 在会签的时候显示不到意见。
                // 如果是当前的节点.当前人员可以处理, 已经审批通过的人员.
                if (tk.HisActionType != ActionType.Shift && tk.NDFrom == this.NodeID
                    && isCanDo == true
                    && tk.EmpFrom.Equals(WebUser.No) == false
                    && checkerPassed.Contains("," + tk.EmpFrom + ",") == false)
                    continue;
                ...
                switch (tk.HisActionType)
                {
                    ...
                    case ActionType.Adjust:
                        if (tk.NDFrom == curNodeID && actionType != ActionType.Return && actionType != ActionType.ReturnAndBackWay)
                            curTrackEmpNos += tk.EmpTo + ",";
                        row = tkDt.NewRow();
                        row["MyPk"] = tk.MyPK;
            ...
            if (gwf.IsOver == false)
            {
                ...
                row = tkDt.NewRow();
                row["NodeID"]=nodeInfo.NodeID;
                row["NDFrom"]="0";
                row["NodeName"]=nodeInfo.FWCNodeName;
                row["ParentNode"]=0;
                row["RDT"]="";
                row["Msg"]="审批中";
                string todoEmps = gwf.TodoEmps;
                if (todoEmps.Contains(";") == false)
                {
                    string[] strs = todoEmps.Split(",");
                    empNos = strs[0];
                    if (strs.Length > 1) empNames = strs[1];
                }
                else
                {
                    string[] strs = todoEmps.Split(";");
                    foreach(string str in strs){
                        if (DataType.IsNullOrEmpty(str) == true)
                            continue;
                        string[] empNoName = str.Split(",");
                        empNos += empNoName[0] + ",";
                        if(empNoName.Length>1)
                        empNames += empNoName[1] + ",";
                    }
                ...
                row["EmpToT"]=empNames;
                row["DeptName"]="";
                if (nodeInfo.ItIsStartNode)
                    row["ActionType"]=(int)ActionType.Forward;
                if (wcDesc.GetValIntByKey("FWCTimeModel", 0) == 0)
                    tkDt.Rows.InsertAt(row, 0);
                else
                    tkDt.Rows.Add(row);
            }
            //获取未来运算到的节点
            if (gwf.IsOver==false && wcDesc.FWCIsShowTruck == true)
            {
                Nodes nds = new Nodes(this.FlowNo);
                WF_WorkOpt_OneWork oneWork = new WF_WorkOpt_OneWork();
                string nodeIDs = oneWork.GetFutureNodeNotContainsEmps();
                ...
                List<string> idsList = nodeIDs.Split(',').ToList();
                List<string> runIds = runNodes.Split(',').ToList();

                // MyView增加当前节点
                if (isReadonly)
                {
                    string nd = this.NodeID.ToString();
                ...
                foreach (string nodeID in idsList)
                {
                    ...
                    if (ens != null)
                    {
                        ...
                        foreach (SelectAccper sa in csa)
                        {
                            empNos += sa.EmpNo + ",";
                            empNames += sa.EmpName + ",";
                        }
            ...
            ds.Tables.Add(tkDt);
            //判断当前节点的是否可以审批
            if (wcDesc.HisFrmWorkCheckSta == FrmWorkCheckSta.Enable && isCanDo && isReadonly == false)
            {
                Node nd = new Node(this.NodeID);
            ...
            Dictionary<object, int> nodeSort = new Dictionary<object, int>();
            int idx = 0;
            List<int> filterActionType = new List<int> { 2, 21, 201 };

            foreach (DataRow dataRow in rowList)
            {
                string actionType = dataRow["ActionType"].ToString();
                if (DataType.IsNullOrEmpty(actionType))
                    continue;
                if (filterActionType.Contains(Convert.ToInt32(actionType)))
                {
                    continue;
                }

                object nodeId = dataRow["NodeID"];
                if (nodeSort.TryGetValue(nodeId, out int currentIndex))
                {
                    dataRow["Step"] = currentIndex;
                }
                else
                {
                    nodeSort[nodeId] = idx;
        ...
        public string WorkCheck_Save()
        {
            ...
            string checkTime = GetRequestVal("CheckTime");
            ...
            {
                // 写入审核信息，有可能是update数据。
                Dev2Interface.Node_WriteWorkCheck(this.WorkID, msg, wcDesc.FWCOpLabel, wcDesc.SigantureEnabel == 2 ? writeImg : "", null, checkTime);
            ...
            if (wcDesc.HisFrmWorkCheckType == FWCType.Check)  //审核组件
            {
                ...
                Dev2Interface.Node_WriteWorkCheck(this.WorkID, msg, wcDesc.FWCOpLabel,
                    wcDesc.SigantureEnabel == 2 || wcDesc.SigantureEnabel == 3 || wcDesc.SigantureEnabel == 5 ? writeImg : "", fwcView, checkTime);
            }
        ...
        public string SelectEmps_Init()
        {
            ...
            if (SystemConfig.AppCenterDBType == DBType.DM)
                sql = "SELECT \"No\",Name,ParentNo FROM Port_Dept WHERE No=" + dbStr + "No OR ParentNo=" + dbStr + "ParentNo ORDER BY Idx";
            else
                sql = "SELECT No,Name,ParentNo FROM Port_Dept WHERE No=" + dbStr + "No OR ParentNo=" + dbStr + "ParentNo ORDER BY Idx";
            Paras paras = new Paras();
            paras.SQL = sql;
            paras.Add("No", fk_dept);
            paras.Add("ParentNo", fk_dept);
            ...
            DataTable dtDept = DBAccess.RunSQLReturnTable(paras);
            if (dtDept.Rows.Count == 0)
            {
                ...
                paras.Add("No", fk_dept);
                paras.Add("ParentNo", fk_dept);
                dtDept = DBAccess.RunSQLReturnTable(paras);
            }
        ...
        public string Accepter_Init()
        {
            ...
            if (select.ItIsAutoLoadEmps == true)
            {
                ...
                else
                {
                    ...
                    else if (SystemConfig.AppCenterDBType == DBType.Oracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.GBASE8CByOracle || SystemConfig.AppCenterDBType == DBType.KingBaseR3 || SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
        ...
        private string Accepter_SendExt(Int64 workid, int toNodeID, string selectEmps, string Auther)
        {
            try
            {
                ...
                if (currNode.HisFormType != NodeFormType.SDKForm && currNode.HisCancelRole != CancelRole.None)
                    msg = "@IsCanUnSend=1" + msg;
                if (DataType.IsNullOrEmpty(objs.VarAcceptersID) == false && objs.VarAcceptersID.Split(",").Length >= 2 && currNode.ItIsTask)
                    msg = "@IsCanTask=1" + msg;
                ...
                switch (currNode.HisTurnToDeal)
                {
                    ...
                    default:
                        ...
                        msg = msg.Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
        ...
        public string Rollback_Init()
        {
            ...
            string sql = "SELECT RDT,NDFrom, NDFromT,EmpFrom,EmpFromT,ActionTypeText  FROM ND" + int.Parse(this.FlowNo) + "Track WHERE WorkID=" + this.WorkID + " AND(" + andsql + ") Order By RDT DESC";
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
            ...
            if (SystemConfig.AppCenterDBFieldCaseModel != FieldCaseModel.None)
            {
                ...
                dt.Columns[3].ColumnName = "EmpFrom";
                dt.Columns[4].ColumnName = "EmpFromT";
                dt.Columns[5].ColumnName = "ActionTypeText";
            }
        ...
        public string TZWorkerRole_Init()
        {
            ...
            bool isFullSA = wn.HisFlow.ItIsFullSA;
        ...
        public string TransferCustom_Init()
        {
            ...
            if (tcs.Count == 0)
            {
                ...
                wk.Retrieve();
                WorkNode wn = new WorkNode(wk, currNode);
                bool isFullSA = currNode.HisFlow.ItIsFullSA;
                wn.HisFlow.ItIsFullSA = true;
        ...
        public string CH_Init()
        {
            ...
            foreach (GenerWorkerList gwl in gwls)
            {
                ...
                if (chNode != null)
                {
                    ...
                    chNode.SetValByKey("EmpNo",  gwl.EmpNo);
                    chNode.SetValByKey("EmpNoT", gwl.EmpName);
                    continue;

                }
                chNode = new CHNode();
                chNode.WorkID = this.WorkID;
                chNode.SetValByKey("NodeID", gwl.NodeID);
            ...
            foreach (Node node in nds)
            {
                ...
                if (node.GetParaBoolen("IsYouLiTai") == true)
                {
                    if (isFirstY == true)
                    {
                        foreach (DataRow drYL in dtYL.Rows)
                        {
                            ...
                            chNode.SetValByKey("NodeID", int.Parse(drYL["NodeID"].ToString()));
                ...
                chNode = new CHNode();
                chNode.WorkID = this.WorkID;
                chNode.SetValByKey("NodeID", node.NodeID);
        ...
        public string CH_Save()
        {
            ...
            foreach (Node nd in nds)
            {
                ...
                chNode.WorkID = this.WorkID;
                chNode.SetValByKey("NodeID",  nd.NodeID);
        ...
        /// </summary>
        /// <returns></returns>
        public string OfficeTrack_Init()
        {
            Paras ps = new Paras();
            ps.SQL = "SELECT MyPK,NDFrom,NDFromT,Msg,EmpFrom,EmpFromT,RDT,Msg FROM ND" + int.Parse(this.FlowNo) + "Track WHERE ActionType=" + SystemConfig.AppCenterDBVarStr + "ActionType AND WorkID=" + SystemConfig.AppCenterDBVarStr + "WorkID ORDER BY RDT ";
            ps.Add("ActionType", (int)BP.WF.ActionType.GovDoc);
            ps.Add("WorkID", this.WorkID);
        ...
        public string SubFlowGuid_Send()
        {
            ...
            BP.DA.Log.DebugWriteInfo("开始时间:" + DataType.CurrentDateTimeCNOfLong);
            ...
            try
            {
                ...
                IHttpContextAccessor httpContextAccessor = HttpContextHelper._httpContextAccessor;
                // 设置最大线程
        ...
        public void SendSingleSubFlow(SubFlowHand subFlowH, Hashtable ht, GenerWorkFlow gwf)
        {
            ...
            //设置父子关系
            if (subFlowH.SubFlowModel == SubFlowModel.SubLevel)
                BP.WF.Dev2Interface.SetParentInfo(subFlowH.SubFlowNo, workidSubFlow, this.WorkID);
            if (subFlowH.SubFlowModel == SubFlowModel.SameLevel)
            {

                BP.WF.Dev2Interface.SetParentInfo(subFlowH.SubFlowNo, workidSubFlow, gwf.PWorkID); //父子关系
                                                                                                   //设置同级关系
        ...
        public string TS_Send()
        {
            ...
            {
                if (!DataType.IsNullOrEmpty(workOpt.SendDepts) && !DataType.IsNullOrEmpty(workOpt.SendStas))
                {
                    string sql = "SELECT FK_Emp FROM Port_DeptEmpStation WHERE FK_Dept IN (" + BP.Port.Glo.GenerWhereInSQL(workOpt.SendDepts) + ") AND FK_Station  IN (" + BP.Port.Glo.GenerWhereInSQL(workOpt.SendStas) + ") ";
                    DataTable dt = DBAccess.RunSQLReturnTable(sql);
                    string strs = "";
                    foreach (DataRow dr in dt.Rows)
                        strs += dr[0] + ",";
        ...
        public  string TS_CC()
        {
            ...
            workOpt.setMyPK(WebUser.No+"_"+this.NodeID+"_"+this.WorkID);
		    if(workOpt.RetrieveFromDBSources()==1){
			    string emps = workOpt.CCEmps;
                emps +=","+ BP.Port.Glo.GenerEmpNosByDeptNos(workOpt.CCDepts);
			    emps +=","+ BP.Port.Glo.GenerEmpNosByStationNos(workOpt.CCStations);
			    return Dev2Interface.Node_CCToEmps(workOpt.NodeID, workOpt.WorkID, emps, "来自" + WebUser.Name + "的抄送", workOpt.CCNote);
		    }
