...
namespace BP.WF.HttpHandler
{
    ...
    public class CCMobile : DirectoryPageBase
    {
        ...
        public string Login_Submit()
        {
            ...
            if(SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
            {
                ...
                else
                {
                    ...
                    emps.Retrieve(BP.Port.EmpAttr.UserID, userNo, BP.Port.EmpAttr.EmpSta, 1);
                    if (emps.Count == 0)
                        return "err@还未注册该账号或该账号已经禁用.";
                    saemp = emps[0] as BP.Port.Emp;
                }
        ...
        public string Default_Init()
        {
            ...
            {
                case DBType.MSSQL:
                    sql = " SELECT TOP " + top + "  FK_Flow,F.Name as FlowName,F.Icon FROM WF_GenerWorkFlow G ,WF_Flow F WHERE  F.IsCanStart=1 AND F.No=G.FK_Flow AND Starter='" + WebUser.No + "' ORDER By SendDT DESC";
                    break;
                case DBType.MySQL:
                case DBType.PostgreSQL:
                case DBType.UX:
                case DBType.HGDB:
                    sql = "SELECT DISTINCT * From (SELECT  FK_Flow,F.Name as FlowName,F.Icon FROM WF_GenerWorkFlow G ,WF_Flow F WHERE  F.IsCanStart=1 AND F.No=G.FK_Flow AND Starter='" + WebUser.No + "'  Order By SendDT  limit  " + (top*2)+ ") A limit " + top;
                    break;
                case DBType.Oracle:
                case DBType.DM:
                ...
                case DBType.GBASE8CByOracle:
                    ...
                    sb.Append(" (SELECT DISTINCT G.FK_Flow as \"FK_Flow\",F.Name as \"FlowName\",F.Icon,G.SendDT as \"SendDt\" ");
                    sb.Append(" FROM WF_GenerWorkFlow G ,WF_Flow F ");
                    sb.Append(" WHERE F.IsCanStart=1");
                    sb.Append(" AND F.No=G.FK_Flow ");
                    sb.Append(" AND G.Starter='admin' ");
                    sb.Append(" GROUP BY G.FK_Flow,F.Name,F.ICON,G.SendDT ");
                    sb.Append(" Order By G.SendDT) ");
        ...
        public string DB_GenerReturnWorks()
        {
            ...
            string sql = "SELECT NDFrom,NDFromT,EmpFrom,EmpFromT,Msg,RDT From " + trackTable + " WHERE WorkID=" + this.WorkID + " AND NDTo=" + this.NodeID + " And ActionType=2 Order By RDT";
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
        ...
        public string Search_Init()
        {
            ...
            if (SystemConfig.AppCenterDBType == DBType.Oracle || DBAccess.AppCenterDBType == DBType.GBASE8CByOracle || SystemConfig.AppCenterDBType == DBType.KingBaseR3 || SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
        ...
        public string Search_Search()
        {
            ...
            if (SystemConfig.AppCenterDBType == DBType.Oracle || DBAccess.AppCenterDBType == DBType.GBASE8CByOracle || SystemConfig.AppCenterDBType == DBType.PostgreSQL || BP.Difference.SystemConfig.AppCenterDBType == DBType.HGDB || SystemConfig.AppCenterDBType == DBType.UX
                || SystemConfig.AppCenterDBType == DBType.KingBaseR3 || SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
            {
                qo.DoQuery();
        ...
        public string getDingTalkUserInfo() {
            try
            {
                string code = this.GetRequestVal("rcode");
                //获取token
                ...
                BP.DA.Log.DebugWriteError(emp.No);
                BP.WF.Dev2Interface.Port_Login(emp.No);
                string userToken = BP.WF.Dev2Interface.Port_GenerToken();
                BP.DA.Log.DebugWriteError(userToken);
                return userToken;
            }
            catch (Exception ex)
            {
        ...
        {
            string ERROR_USER_VALIDATION_FAILED = "err@用户验证失败";
            string userId = null;
            try
            {
                ...
                //获取userid
                userId = WeiXinEntity.getUserId(code, accessToken);
                BP.DA.Log.DebugWriteError("111:" + userId);

                BP.Port.WeiXin.User user = FormatToJson.ParseFromJson<BP.Port.WeiXin.User>(userId);
                BP.Port.WeiXin.Emp emp = new BP.Port.WeiXin.Emp();

                string userIdValue = user.userid;
	     ...
	     * @throws Exception
	     */
        public string getWXAppletUserInfo()
        {
            ...
            try
            {
                ...
                Tel = BP.Tools.AESHelper.AESDecrypt(Tel, key);
                BP.Port.WeiXin.Emp emp = new BP.Port.WeiXin.Emp();
                //判断帐号
                if (emp.IsExit(BP.Port.EmpAttr.Tel, Tel))
                {
                    String userToken = "";
                    if (SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
                    {
