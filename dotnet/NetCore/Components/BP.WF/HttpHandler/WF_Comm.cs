...
namespace BP.WF.HttpHandler
{
    ...
    {
        public static readonly object pblock = new object();
        public static readonly object frmVSTOTemplateLock = new object();
        //读取公文和保存公文word锁
        public static readonly object gongWenWordTemplateLock = new object();
        ...
        public string Entity_Update()
        {
            try
            {
                ...
                if (en.GetValByKey(MapAttrAttr.UIContralType) != null && int.Parse((string)en.GetValByKey(MapAttrAttr.UIContralType)) == ((int)UIContralType.AthShow))
                {
        ...
        public string Entity_DoMethodReturnString()
        {
            try
            {
                ...
                if (mp == null)
                    return "err@没有找到类[" + this.EnName + "]方法[" + methodName + "],名称区分大小写.";
                ...
                if (DataType.IsNullOrEmpty(paras) == false)
                {
                    ...
                    foreach (ParameterInfo paramInfo in paramInfos)
                    {
                        ...
                        {
                            throw new Exception("err@类[" + this.EnName + "]方法[" + methodName + "]名称区分大小写,值" + str[idx] + "转换成" + paramInfo.ParameterType.Name + "失败");
                        }
                        idx++;
                    }
                }
            ...
            for (int i = 0; i < myparas.Length; i++)
            {
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppInt:
                        qo.AddWhere(key, int.Parse(valObj.ToString()));
                        break;
                    case DataType.AppMoney:
                    case DataType.AppFloat:
                    case DataType.AppDouble:
                        qo.AddWhere(key, float.Parse(valObj.ToString()));
                        break;
                    default:
        ...
        public string Entities_DoMethodReturnString()
        {
            ...
            System.Reflection.MethodInfo mp = tp.GetMethod(methodName);
            if (mp == null)
                return "err@没有找到类[" + this.EnsName + "]方法[" + methodName + "].名称区分大小写";
        ...
        public string uploadExcelFile()
        {
            try
            {
                ...
                string pkValue = this.GetRequestVal("pkValue");
                ...
                md.ExcelSaveFile(pkValue, bytes, mfe.DBSave);
                return "success";
            }
            catch (Exception e)
            {
                return "err@上传失败: ex: " + e.ToString();
            }
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string saveExcelTemplate()
        {
            try
            {
                string token = this.GetRequestVal("token");
                ...
                //设置文件名
                string fileNewName = frmID + ext;

                //文件存放路径
                string filePath = BP.Difference.SystemConfig.PathOfDataUser + "/FrmVSTOTemplate/" + fileNewName;
                HttpContextHelper.UploadFile(file, filePath);

                //保存到MapData的VSTOExcelFile字段中
        ...
        /// <returns></returns>
        public object SaveFrmVSTOTemplate()
        {

            try
            {
                lock (frmVSTOTemplateLock)
                {
                    string token = this.GetRequestVal("token");
                    string frmId = this.GetRequestVal("frmId");
                    IFormFile file = HttpContextHelper.RequestFiles(0);
                    ...
                    if (HttpContextHelper.RequestFilesCount < 0 || DataType.IsNullOrEmpty(token) || DataType.IsNullOrEmpty(frmId))
                    {
                        return "err@参数不能为空";
                    }
                    BP.WF.Dev2Interface.Port_LoginByToken(token);
                    ...
                    if (File.Exists(frmVSTOTemplateFilePath))
                    {
                        File.Delete(frmVSTOTemplateFilePath);
                    }
        ...
        /// <param name="frmAttachmentMyPk"></param>
        /// <returns></returns>
        public object SaveGongWenWord()
        {

            try
            {

                lock (gongWenWordTemplateLock)
                {
                    string token = this.GetRequestVal("token");
                    string fkNodeId = this.GetRequestVal("fkNodeId");
                    string workId = this.GetRequestVal("workId");
                    if (HttpContextHelper.RequestFilesCount < 0 || DataType.IsNullOrEmpty(token) || DataType.IsNullOrEmpty(fkNodeId))
                    {
                        return "err@参数不能为空";
                    }
                    BP.WF.Dev2Interface.Port_LoginByToken(token);
                    ...
                    Node nd = new Node(int.Parse(fkNodeId));
                    Work wk = nd.HisWork;
                    wk.OID = int.Parse(workId);
                    wk.SaveBytesToDB("DBFile", buffer);
        ...
        /// <param name="frmAttachmentMyPk"></param>
        /// <returns></returns>
        public object SaveFrmAttachment()
        {
            try
            {
                lock (pblock)
                {
                    ...
                    string token = this.GetRequestVal("token");
                    string frmAttachmentMyPk = this.GetRequestVal("frmAttachmentMyPk");
                    IFormFile file = HttpContextHelper.RequestFiles(0);
                    ...
                    if (frmAttachment.AthSaveWay == AthSaveWay.IISServer)
                    {
                        ...
                        {
                            //现将上传的文件转存为临时文件
                            tempFile = BP.Difference.SystemConfig.PathOfTemp + frmAttachmentDB.MyPK + ".tmp";
                            ...
                            //将原始文件删除掉
                            if (File.Exists(frmAttachmentDB.FileFullName) == true)
                                File.Delete(frmAttachmentDB.FileFullName);
                    ...
                    if (frmAttachment.AthSaveWay == AthSaveWay.DB)
                    {
                        ...
                        if (fileEncrypt == true && isEncrypt == true)
                        {
                            string tempEncryptFile = BP.Difference.SystemConfig.PathOfTemp + frmAttachmentDB.MyPK + ".encryptTmp";
                            EncHelper.EncryptDES(tempFile, tempEncryptFile);//加密

                            //保存到数据库中
                            frmAttachmentDB.SaveFileToDB("DBFile", tempEncryptFile);
                            if (File.Exists(tempEncryptFile))
                            {
                                File.Delete(tempEncryptFile);
                            }
                    ...
                    if (frmAttachment.AthSaveWay == AthSaveWay.FTPServer)
                    {
                        ...
                        if (fileEncrypt == true && isEncrypt == true)
                        {
                            string tmpEncrypt = BP.Difference.SystemConfig.PathOfTemp + frmAttachmentDB.MyPK + ".encryptTmp";
                            HttpContextHelper.UploadFile(file, tempFile);
                            ...
                            //删除掉加密的临时文件
                            if (File.Exists(tmpEncrypt) == true)
                                File.Delete(tmpEncrypt);
                        }
                        else
                        {
     ...
     //   public Paras currParas = null;
        /// <summary>
        /// 执行查询.这个方法也会被导出调用.
        /// </summary>
        /// <returns></returns>
        public QueryObject Search_Data(Entities ens, Entity en, Map map, UserRegedit ur, string action = null)
        {
            //获得关键字.
            AtPara ap = new AtPara(ur.Vals);
          ///  currParas = new Paras();
            ...
            if (DataType.IsNullOrEmpty(map.SearchFields) == false)
            {
                ...
                foreach (String str in searchFields)
                {
                    ...
                    foreach (String val in fieldValues)
                    {
                        ...
                        //    currParas.Add(field, " '%'||" + BP.Difference.SystemConfig.AppCenterDBVarStr + field + valIdx + "||'%'");
                ...
                if (frmBill.GetParaInt("HidenWay") == 1)
                {
                    if (DataType.IsNullOrEmpty(hidenField) == false)
                    {
                        ...
                        if (hidenField.IndexOf("_") != -1)
                            throw new Exception("err@隐藏条件" + hidenField + "还有未替换的_符号");
            ...
            if (map.DTSearchWay != DTSearchWay.None
                && DataType.IsNullOrEmpty(ur.DTFrom) == false
                && DataType.IsNullOrEmpty(ur.DTTo) == false
                && DataType.IsNullOrEmpty(ur.DTSearchKey) == false)
            {
                ...
                if (map.DTSearchWay == DTSearchWay.ByYearMonth || map.DTSearchWay == DTSearchWay.ByYear)
                {
                    if (isFirst == false)
                        ...
                        isFirst = false;
                    qo.AddWhere(ur.DTSearchKey, dtFrom);
                }
                ...
                //按日期查询
                if (map.DTSearchWay == DTSearchWay.ByDate && DataType.IsNullOrEmpty(ur.DTSearchKey) == false )
                {
                    if (isFirst == false)
                        qo.addAnd();
                    else
                    ...
                    {
                        qo.addLeftBracket();
                        qo.SQL = map.PhysicsTable + "." + ur.DTSearchKey + " <= '" + dtTo + "'";
                        qo.addRightBracket();
                    }
                    else
                    {
                        qo.addLeftBracket();
                        dtTo += " 23:59:59";
                        qo.SQL = map.PhysicsTable + "." + ur.DTSearchKey + " >= '" + dtFrom + "'";
                        qo.addAnd();
                        qo.SQL = map.PhysicsTable + "." + ur.DTSearchKey + " <= '" + dtTo + "'";
                        qo.addRightBracket();
                    }
                ...
                if (map.DTSearchWay == DTSearchWay.ByDateTime)
                {
                    ...
                    {
                        qo.addLeftBracket();
                        qo.SQL = ur.DTSearchKey + " <= '" + dtTo + "'";
                        qo.addRightBracket();
                    }
                    else
                    {
                        qo.addLeftBracket();
                        qo.SQL = ur.DTSearchKey + " >= '" + dtFrom + "'";
                        qo.addAnd();
                        qo.SQL = ur.DTSearchKey + " <= '" + dtTo + "'";
                        qo.addRightBracket();
                    }
            ...
            foreach (SearchNormal attr in en.EnMap.SearchNormals)
            {
                if (attr.ItIsHidden)
                {
                    ...
                    string mystr = valType.ToString();
                    if (DataType.IsNullOrEmpty(mystr) == true) continue;
            ...
            foreach (Attr attr in map.Attrs)
            {
                ...
                string val = HttpContextHelper.GetFormParams(attr.Field);
                ...
                if (attr.Field.Equals("Token"))
                    continue;
                if (attr.Field.Equals("No") || attr.Field.Equals("OrgNo"))
                    continue;
                if (attr.Field.Equals("Name"))
                    continue;
        ...
        public string Search_Exp()
        {
            ...
            Entities ens = ClassFactory.GetEns(this.EnsName);
            Entity en = ens.GetNewEntity;
            QueryObject qo = Search_Data(ens, en, en.EnMap, ur, "CommSearch");
            EnCfg encfg = new EnCfg();
            encfg.No = this.EnsName;
            //增加排序
        ...
        public string Exec(string clsName, string methodName, string paras = null)
        {
            ...
            try
            {
                ...
                System.Reflection.MethodInfo mp = tp.GetMethod(methodName);
                if (mp == null)
                    return "err@没有找到类[" + clsName + "]方法[" + methodName + "].名称区分大小写";
        ...
        public string DBAccess_RunSQLReturnTable()
        {
            ...
            string uriComponent = BP.Tools.AESHelper.AESDecrypt(sql, aesKey);
            sql = Uri.UnescapeDataString(uriComponent);

            if (DataType.IsNullOrEmpty(sql) == false && sql.IndexOf("SELECT") != -1)
            {
            ...
            sql = sql.Replace("@WebUser.DeptNo", WebUser.DeptNo);  //替换变量.
...
#warning zhoupeng把这个去掉了. 2018.10.24
        ...
        public string RunWebAPIReturnString()
        {
            ...
            if (res["code"].ToString().Equals("200")==true)
        ...
        {
            Hashtable ht = new Hashtable();
            string token = this.GetRequestFormVal("Token");
            //string token = this.GetRequestVal("Token");
            if (DataType.IsNullOrEmpty(token) == false)
            {
                ...
                {
                    if (SystemConfig.CustomerNo.Equals("GanLan") == true)
                    {
                    }
                    else
                    {
            ...
            ht.Add("IsFirstLogin", WebUser.IsFirstLogin);
            ht.Add("Roles", WebUser.Roles);
            ht.Add("RootNo", WebUser.RootNo);
            //检查是否是授权状态.
        ...
        public string Group_ContentAttrs()
        {
            ...
            foreach (Attr attr in attrs)
            {
                if (attr.ItIsEnum == true || attr.MyDataType == DataType.AppBoolean || attr.UIContralType == UIContralType.DDL || attr.UIContralType == UIContralType.RadioBtn)
                {
                    if (attr.UIContralType == UIContralType.CheckBok) continue;
                    DataRow dr = dt.NewRow();
                    dr["Field"] = attr.Key;
                    dr["Name"] = attr.Desc;
        ...
        private DataSet GroupSearchSet(Entities ens, Entity en, Map map, UserRegedit ur, DataSet ds, ActiveAttrs aas)
        {
            ...
            if (DataType.IsNullOrEmpty(groupKey) == false)
            {
                ...
                foreach (string key in SelectedGroupKeys)
                {
                    ...
                    if (attr.MyDataType == DataType.AppBoolean)
                    {
                        selectSQL += " CASE " + en.EnMap.PhysicsTable + "." + attr.Key;
                        selectSQL += " WHEN 0 THEN '否'";
                        selectSQL += " WHEN 1 THEN '是'";
                        selectSQL += " END \"" + attr.Key + "T\",";
                        continue;
                    }
                    //不是外键、枚举，就是外部数据源
            ...
            foreach (string key in analyKeys)
            {
                ...
                switch (strs[1])
                {
                    case "SUM":
                        ...
                        if (attr.Desc.Contains("(合计)") == false)
                    ...
                    case "AVG":
                        ...
                        if (attr.Desc.Contains("(平均)") == false)
                    ...
                    case "AMOUNT":
                        ...
                        if (attr.Desc.Contains("(累计)") == false)
            ...
            {
                if (dc.ColumnName.EndsWith("Text"))
                    dc.ColumnName = dc.ColumnName.Substring(0, dc.ColumnName.Length - 3);
            }
