...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_MyFlow : DirectoryPageBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string GetPreplaceWokerFreeNodes()
        {
            Int64 workID = this.WorkID;
            Nodes nodes = BP.WF.Dev2Interface.GetPreplaceWokerFreeNodes(this.WorkID, this.FlowNo, this.NodeID);
            DataTable dt = new DataTable();
            dt.Columns.Add("NodeID", typeof(int));
            dt.Columns.Add("Name");
            dt.Columns.Add("DeliveryWay", typeof(int));
            ...
            foreach (Node node in nodes)
            {
                ...
                dr["NodeID"] = nodeID;
                dr["Name"] = node.Name;
                dr["DeliveryWay"] = node.HisDeliveryWay.ToString();
                ...
                {
                    empNames.Append(",").Append(sa.EmpName);
                    empNos.Append(",").Append(sa.EmpNo);
                }
                if (empNames.Length > 0)
                {
                    dr["EmpNames"] = empNames.ToString().Substring(1);
                }
                else
                {
                    dr["EmpNames"] = "";
                ...
                if (empNos.Length > 0)
                {
                    dr["EmpNos"] = empNos.ToString().Substring(1);
                }
                else
                {
                    dr["EmpNos"] = "";
        ...
        public string Send_CheckFlowOver()
        {
            try
            {
                // todo:尚未判断完整.
                bool val = BP.WF.Dev2Interface.Flow_CheckFlowIsOver(this.WorkID);
                if (val == false)
                    return "0";
                else
                    return "1";
        ...
        public string MyFlow_Init()
        {
            ...
            {
                if (gwf.Starter.Equals(BP.Web.WebUser.No) == true)
                {
                    isCanDo = true;
                }
                //判断开始节点是否移交
                if (gwf.WFState == WFState.Shift)
                {
                    ...
                    Paras ps = new Paras();
                    ps.SQL = "SELECT EmpTo FROM ND" + int.Parse(gwf.FlowNo) + "Track WHERE ActionType=" + ActionType.Shift.ToString() + " AND WorkID=" + dbStr + "WorkID  AND NDTo=" + gwf.NodeID + "   ORDER BY RDT DESC";
                    ps.Add(TrackAttr.WorkID, gwf.WorkID);
                    DataTable mydt = DBAccess.RunSQLReturnTable(ps);
                    if (mydt.Rows.Count != 0 && mydt.Rows[0][0].Equals(WebUser.No) == true)
                        isCanDo = true;
                }
            }
            else
            ...
            if (this.currND.WhoExeIt == 2)
            {
                ...
                {
                    BP.DA.Log.DebugWriteInfo("myflow_int@判断是否是混合执行,提示未发送成功信息:" + ex.Message);
                }
            }
            #endregion 判断是否是混合执行.
            ...
            if (frmtype == NodeFormType.EntityTS)
            {
                string formUrl = this.currND.FormUrl;
                if (DataType.IsNullOrEmpty(formUrl) == true)
                    return "err@请联系管理,配置节点[" + this.currND.Name + "]的高代码TS";
                string url = "MyFlowEntityTS.htm?EnName=" + formUrl + "&OID=" + this.WorkID + urlParam + "&PageType=MyFlow";
        ...
        public string InitToolBar()
        {
            ...
            if (this.NodeID.ToString().EndsWith("01") == false)
            {
                ...
                //如果当前节点多人处理规则是协作组长模式，就修改流程状态
                if (nd.TodolistModel == TodolistModel.TeamupGroupLeader)
                {
                    int teamleaderConfirm = nd.GetParaInt("TeamleaderConfirm");
                    string zcr = nd.GetParaString("TeamleaderCfmVal");
                    if (teamleaderConfirm == 1 && DataType.IsNullOrEmpty(zcr) == false)
                    {
                        gwf.HuiQianTaskSta = HuiQianTaskSta.HuiQianing;
                        gwf.HuiQianZhuChiRen = zcr;
                        if (nd.TeamLeaderConfirmRole != TeamLeaderConfirmRole.HuiQianLeader)
                        {
                            nd.HuiQianRole = HuiQianRole.TeamupGroupLeader;
                            nd.TeamLeaderConfirmRole = TeamLeaderConfirmRole.HuiQianLeader;
                            nd.Update();
                        }
            ...
            try
            {
                ...
                if (btnLab.GetValBooleanByKey("FixEnable") == true && gwf.WFState != WFState.Fix)
                {
                    /* 冻结.. */
                    dr = dt.NewRow();
                    dr["No"] = "FixFlow";
                    dr["Name"] = btnLab.GetValStringByKey("FixFlow");
                    dr["Oper"] = "";
                    dt.Rows.Add(dr);

                }
                if (gwf.WFState == WFState.Fix)
                {
                    dr = dt.NewRow();
                    dr["No"] = "FixFlowUn";
                    dr["Name"] = "解冻";
                    ...
                    if (!(nd.ItIsStartNode == true && gwf.PWorkID == 0))
                    {
                        ...
                        else
                        {
                            if (nd.TodolistModel == TodolistModel.Order
                                    ...
                                    || (nd.TodolistModel == TodolistModel.TeamupGroupLeader && nd.HuiQianLeaderRole != HuiQianLeaderRole.OnlyOne))
                            {
                                teamReturner = "," + teamReturner + ",";
                                if (teamReturner.Contains("," + WebUser.No + ","))
                                    dt.Rows.Add(dr);
                            }
                        }
                ...
                {
                    //OfficeFileType  @0=Word文件@1=WPS文件@2=混合模式
                    if (btnLab.GetValStrByKey(BtnAttr.OfficeFileType).Equals("2"))
                    {
                        dr = dt.NewRow();
                        dr["No"] = "DocWordForWord";
                        dr["Name"] = btnLab.OfficeBtnLab + "OFFICE";
                        dr["Oper"] = "";
                        dr["AtPara"] = "@OfficeFileType=" + btnLab.GetValStrByKey(BtnAttr.OfficeFileType) + "@OfficeBtnEnable=" + btnLab.GetValStrByKey(BtnAttr.OfficeBtnEnable);
                        //OfficeFileType 0-word 1-wps  OfficeBtnEnable 0-不可用 1-可编辑 2-不可编辑
                        dt.Rows.Add(dr);
                        dr = dt.NewRow();
                        dr["No"] = "DocWordForWPS";
                        dr["Name"] = btnLab.OfficeBtnLab + "WPS";
                        dr["Oper"] = "";
                        dr["AtPara"] = "@OfficeFileType=" + btnLab.GetValStrByKey(BtnAttr.OfficeFileType) + "@OfficeBtnEnable=" + btnLab.GetValStrByKey(BtnAttr.OfficeBtnEnable);
                        //OfficeFileType 0-word 1-wps  OfficeBtnEnable 0-不可用 1-可编辑 2-不可编辑
                        dt.Rows.Add(dr);
                ...
                //公文版本
                if (btnLab.OfficeTrackEnalbe)
                {
                    dr = dt.NewRow();
                    dr["No"] = "OfficeTrackEnalbe";
                    dr["Name"] = btnLab.OfficeTrackLab;
                    dr["Oper"] = "";
                    dt.Rows.Add(dr);
                }
                ...
                {
                    dr = dt.NewRow();
                    dr["No"] = "DownVSTOEnable";
                    dr["Name"] = btnLab.DownVSTOLab;
                    dr["Oper"] = "";
                    dt.Rows.Add(dr);
                ...
                if (submitSFEnable != 0)
                {
                    ...
                    {
                        //获取当前人部门岗位
                        sql = "SELECT A.No AS DeptNo,A.Name AS DeptName From Port_Dept A,Port_DeptEmp B WHERE A.No=B.FK_Dept  AND B.FK_Emp='" + WebUser.UserID + "'";
                    ...
                    {
                        //获取当前人部门岗位
                        sql = "SELECT A.No AS DeptNo,B.No AS StationNo,A.Name AS DeptName,B.Name AS StationName From Port_Dept A,Port_Station B,Port_DeptEmpStation C WHERE A.No=C.FK_Dept AND B.No=C.FK_Station AND C.FK_Emp='" + WebUser.UserID + "'";
        ...
        /// </summary>
        /// <returns></returns>
        public string GetWriteTrack()
        {
            string dbStr = SystemConfig.AppCenterDBVarStr;
            Paras ps = new Paras();
            ps.SQL = "SELECT * FROM ND" + int.Parse(this.FlowNo) + "Track WHERE ActionType=" + dbStr + "ActionType AND WorkID=" + dbStr + "WorkID AND NDFrom=" + dbStr + "NDFrom";
        ...
        public string SaveWriteTrack()
        {
            ...
            try
            {
                Flow fl = new Flow(this.FlowNo);
                Node nd = new Node(this.NodeID);
                string msg = BP.WF.Glo.multilingual("用戶 [{0}] 在 [{1}] 修改了 [{2}] 流程 [{3}] 节点的数据，业务编号为[" + this.WorkID + "]", "WF_MyFlow", "save", this.GetRequestVal("TB_Rec"), DataType.CurrentDateTimess, fl.Name, nd.Name);
                //记录表单数据
                string nodeData = "";
                IFormCollection formData = HttpContextHelper.Request.Form;
                ...
                //记录调整数据的日志
                BP.WF.Dev2Interface.WriteTrack(this.FlowNo, nd.NodeID, nd.Name, this.WorkID, this.FID, msg, ActionType.Info, null, null, null, null, null, null, null, null, null, null, nodeData);
            }
            catch (Exception ex)
            {
                return "err@保存后记录日志出现错误:" + ex.Message;
            }
        ...
        public string Save()
        {
            try
            {
                ...
                if (this.PWorkID != 0)
                {
                    ...
                    {
                        BP.WF.Dev2Interface.SetParentInfo(this.FlowNo, this.WorkID, this.PWorkID);
                    }
                }
                return str;
            }
        ...
        public string FlowFormTree2021_Init()
        {
            ...
            //求出来要显示的表单集合.
            Hashtable ht = new Hashtable();
            Hashtable htFrmSln = new Hashtable();
            foreach (FrmNode frmNode in frmNodes)
            {
                ...
                switch (frmNode.FrmEnableRole)
                {
                    ...
                    case FrmEnableRole.WhenHaveData: //判断是否有数据.
                        MapData mapData = mds.GetEntityByKey(frmNode.FrmID) as MapData;
                        if (mapData == null)
                            continue;
                    ...
                    case FrmEnableRole.WhenHaveFrmPara: //判断是否有参数.
                        if (DataType.IsNullOrEmpty(frms) == true)
                            continue;
                        if (frms.Contains("," + frmNode.FrmID + ",") == false)
                            continue;
                        break;
                    ...
                    case FrmEnableRole.BySQL: // 按照SQL的方式.
                        ...
                        {
                            MapData FrmMd = mds.GetEntityByKey(frmNode.FrmID) as MapData;
                            return "err@表单" + frmNode.FrmID + ",[" + FrmMd.Name + "]在节点[" + frmNode.NodeID + "]启用方式按照sql启用但是您没有给他设置sql表达式.";
                        }
                        ...
                        mysql = mysql.Replace("@WebUser.DeptNo", WebUser.DeptNo);
                ...
                MapData md = mds.GetEntityByKey(frmNode.FrmID) as MapData;
                if (md == null)
                {
                    frmNode.Delete();
                    continue;
                ...
                nodeForm.SetValByKey("Icon", md.Icon);
                ...
                if (!ht.ContainsKey(md.No))
                {
                    ht.Add(md.No, md.HisFrmTypeInt);
                }
                if (!htFrmSln.ContainsKey(md.No))
                {
                    htFrmSln.Add(md.No, Convert.ToInt32(frmNode.FrmSln));
                }
            ...
            for (int i = 0; i < forms.Count; i++)
            {
                ...
                if (ht.ContainsKey(formdt.Rows[i]["No"]))
                {
                    formdt.Rows[i]["FrmType"] = ht[formdt.Rows[i]["No"]];
                }
                if (ht.ContainsKey(formdt.Rows[i]["No"]))
                {
                    formdt.Rows[i]["FrmSln"] = htFrmSln[formdt.Rows[i]["No"]];
                }
            }
        ...
        public string FlowFormTree_Init()
        {
            ...
            if (frmNodes.Count <= 3)
            {
                ...
                {
                    MapData md = new MapData(fn.FrmID);
                    mds.AddEntity(md);
                }
            }
            else
            ...
            foreach (FrmNode frmNode in frmNodes)
            {
                ...
                switch (frmNode.FrmEnableRole)
                {
                    ...
                    case FrmEnableRole.WhenHaveFrmPara: //判断是否有参数.
                        ...
                        if (frms.Contains(",") == false && frms.Equals(frmNode.FrmID) == false)
                            continue;

                        if (frms.Contains(",") == true && frms.Contains(frmNode.FrmID + ",") == false)
                            continue;
                    ...
                    case FrmEnableRole.BySQL: // 按照SQL的方式.
                        ...
                        if (DataType.IsNullOrEmpty(mysql) == true)
                        {
                            MapData FrmMd = new MapData(frmNode.FrmID);
                            return "err@表单" + frmNode.FrmID + ",[" + FrmMd.Name + "]在节点[" + frmNode.NodeID + "]启用方式按照sql启用但是您没有给他设置sql表达式.";
                        }
                ...
                foreach (MapData md in mds)
                {
                    if (frmNode.FrmID.Equals(md.No) == false)
                        continue;

                    if (DataType.IsNullOrEmpty(md.FormTreeNo))
                        md.FormTreeNo = treeNo;
        ...
        public string ThreadDtl_Init()
        {
            ...
            if (nd.HisRunModel == RunModel.FL || nd.HisRunModel == RunModel.FHL)
            {
                ...
                string sql = "SELECT NDFrom,NDFromT,EmpFrom,EmpFromT,Msg,RDT,NDTo,WorkID From " + trackTable + " WHERE WorkID IN(SELECT WorkID From WF_GenerWorkFlow WHERE FID=" + this.FID + " AND WFState=5) Order By RDT DESC";
                DataTable dtt = DBAccess.RunSQLReturnTable(sql);
                if (SystemConfig.AppCenterDBFieldCaseModel != FieldCaseModel.None)
                {
                    dtt.Columns[0].ColumnName = "NDFrom";
                    dtt.Columns[1].ColumnName = "NDFromT";
                    dtt.Columns[2].ColumnName = "EmpFrom";
                    dtt.Columns[3].ColumnName = "EmpFromT";
                    dtt.Columns[4].ColumnName = "Msg";
                    dtt.Columns[5].ColumnName = "RDT";
                    dtt.Columns[6].ColumnName = "NDTo";
                    dtt.Columns[7].ColumnName = "WorkID";
                }
        ...
        /// </summary>
        /// <returns></returns>
        public string Btn_ConfirmBtn()
        {
            GenerWorkFlow gwf = new GenerWorkFlow(this.WorkID);
            if (gwf.Paras_Confirm == true)
                gwf.Paras_Confirm = false;
            else
                gwf.Paras_Confirm = true;
