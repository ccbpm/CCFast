...
namespace BP.WF.HttpHandler
{
    ...
    public class WF : DirectoryPageBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string Confim_True_Init()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public string Confim_False_Init()
        {
        ...
        public string Focus_Init()
        {
            ...
            string domain = this.GetRequestVal("DomainExt");
        ...
        public string StartEaryer_Init()
        {
            ...
            {
                sql = "SELECT FK_Flow as \"No\",FlowName as Name, FK_FlowSort,B.Name as FK_FlowSortText,B.\"DomainExt\", COUNT(WorkID) as Num ";
                sql += " FROM WF_GenerWorkFlow A, WF_FlowSort B  ";
                sql += " WHERE Starter='" + BP.Web.WebUser.No + "'  AND A.FK_FlowSort=B.No  ";
                if (DataType.IsNullOrEmpty(this.FlowNo) == false)
                    sql += " AND A.FK_Flow='" + this.FlowNo + "'";
            ...
            if (SystemConfig.AppCenterDBFieldCaseModel != FieldCaseModel.None)
            {
                ...
                dtStart.Columns[4].ColumnName = "DomainExt";
            ...
            dtSort.Columns.Add("DomainExt", typeof(string));
            ...
            foreach (DataRow dr in dtStart.Rows)
            {
                ...
                string domain = dr["DomainExt"].ToString();
        ...
        public string Start_Init()
        {
            ...
            dtSort.Columns.Add("Name", typeof(string));
            dtSort.Columns.Add("DomainExt", typeof(string));
            dtSort.Columns.Add("MobileColorType", typeof(int));

            string nos = "";
            foreach (DataRow dr in dtStart.Rows)
            {
                string no = dr["FK_FlowSort"].ToString();
                bool isMatch = Regex.IsMatch(nos, @"\b" + no + @"\b");
                if (nos.Contains(no) == true)
                    continue;

                string name = dr["FK_FlowSortText"].ToString();
                string domain = dr["DomainExt"].ToString();
                string mobileColorType = dr["MobileColorType"].ToString();
                ...
                mydr[1] = name;
                mydr[2] = domain;
                mydr[3] = DataType.IsNullOrEmpty(mobileColorType)== true?0:Int32.Parse(mobileColorType);
                dtSort.Rows.Add(mydr);
            }
        ...
        public string Runing_Init()
        {
            ...
            dt = BP.WF.Dev2Interface.DB_GenerRuning(WebUser.No, this.FlowNo, false, this.DomainExt, isContainFuture); //获得指定域的在途.
            return BP.Tools.Json.ToJson(dt);
        }
        //近期工作
        ...
        public string Complete_Init()
        {
            ...
            if(DataType.IsNullOrEmpty(flowNo) == false)
            {
                ...
                ps.SQL = "SELECT  * FROM WF_GenerWorkFlow  WHERE (Emps LIKE '%@" + WebUser.No + "@%' OR Emps LIKE '%@" + WebUser.No + ",%' OR Emps LIKE '%," + WebUser.No + "@%') AND (WFState=3 OR WFState=11) AND FK_Flow=" + dbstr + "FK_Flow ORDER BY  RDT DESC";
                ps.Add("FK_Flow", flowNo);
            }
            else
            {
                ps.SQL = "SELECT  * FROM WF_GenerWorkFlow  WHERE (Emps LIKE '%@" + WebUser.No + "@%' OR Emps LIKE '%@" + WebUser.No + ",%' OR Emps LIKE '%," + WebUser.No + "@%') AND (WFState=3 OR WFState=11)  ORDER BY  RDT DESC";
            }
        ...
        /// <param name="flowNos">流程编号，多个用逗号分开.</param>
        /// <param name="toOneUserNo">移交给的人员编号.</param>
        /// <returns></returns>
        public string Batch_Shift()
        {
        ...
        public string Todolist_Init()
        {
            ...
            DataTable dt = BP.WF.Dev2Interface.DB_GenerEmpWorksOfDataTable(WebUser.No, this.NodeID,
                wfState, this.DomainExt, this.FlowNo, isRead, orderBy);
            return BP.Tools.Json.ToJson(dt);
        }
        ...
        public string Search_Count()
        {
            ...
            if (DataType.IsNullOrEmpty(tSpan) == true)
                tSpan = "-1";
            if (DataType.IsNumStr(tSpan) == false)
                tSpan = "-1";
            //查询关键字
            string keyWord = this.GetRequestVal("KeyWord");
        ...
        public string Search_Data()
        {
            ...
            if (tSpan != "-1")
            {
                sqlWhere += "AND TSpan = " + dbStr + "TSpan ";
                paras.Add("Sapn", tSpan);
            }

            if (keyWord != null)
            {
                if (BP.Difference.SystemConfig.AppCenterDBVarStr == "@" || BP.Difference.SystemConfig.AppCenterDBVarStr == "?")
                    sqlWhere += "AND Title like " + (BP.Difference.SystemConfig.AppCenterDBType == DBType.MySQL ? (" CONCAT('%', " + dbStr + "Title, '%')") : (" '%' + " + dbStr + "Title + '%'"));
                else
                    sqlWhere += "AND Title like  '%'||" + BP.Difference.SystemConfig.AppCenterDBVarStr + "Title||'%'";
                paras.Add("Title", keyWord);
            }

            if (this.FlowNo != null)
            {
                sqlWhere += "AND FK_Flow = " + dbStr + "FK_Flow ";
                paras.Add("FK_Flow", this.FlowNo);
            }
            ...
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle
                ...
                || SystemConfig.AppCenterDBType == DBType.DM
                 || DBAccess.AppCenterDBType == DBType.GBASE8CByOracle)
            ...
            paras.SQL = sql;
            DataTable mydt = DBAccess.RunSQLReturnTable(paras);
        ...
        public string PortSaaS_Init()
        {
            ...
            if (this.DoWhat.Equals("StartFrm"))
            {
                ...
                if (oid == 0)
                {
                    GEEntityOID gn = new GEEntityOID(this.FrmID);
        ...
        public string Port_Init()
        {
            ...
            // 近期处理
            if (this.DoWhat.Equals(DoWhatList.RecentWork) == true)
                return "url@RecentWork.htm?Token=" + token;
            ...
            //任务
            if (this.DoWhat.Equals(DoWhatList.Task) == true)
                return "url@/CCFast/Task/Task.htm?Token=" + token + "&UserNo=" + BP.Web.WebUser.No;

            //知识库
            if (this.DoWhat.Equals(DoWhatList.Knowledge) == true)
                return "url@/CCFast/KnowledgeManagement/Default.htm?Token=" + token + "&UserNo=" + BP.Web.WebUser.No;

            //超级管理员查看处理的所有流程。
            if (this.DoWhat.Equals(DoWhatList.GenerWorkFlowViews) == true)
                return "url@./Comm/Search.htm?EnsName=BP.WF.Data.GenerWorkFlowViews&WFSta=all&Token=" + token;

            //超级管理员查看处理的所有流程(批量Batch)。
        ...
        */
        ...
        /// </summary>
        /// <returns></returns>
        public string Flow_IsCanDo()
        {
            long worid = this.GetRequestValInt64("WorkID");
            bool isCan = BP.WF.Dev2Interface.Flow_IsCanDoCurrentWork(worid, WebUser.No);
            Hashtable ht = new Hashtable();
            ht.Add("isCanDo", isCan);
            return BP.Tools.Json.ToJson(ht);
        }
    }
}