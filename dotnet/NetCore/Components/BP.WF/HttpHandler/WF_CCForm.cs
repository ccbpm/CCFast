...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_CCForm : DirectoryPageBase
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string FrmAPI_GenerMyDataType()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public string FrmAPI_GenerFieldEndInfo()
        {
        ...
        public string Ath_Init()
        {
            try
            {
                ...
                bool ishistoryData = this.GetRequestValBoolen("IshistoryData");
                ...
                else
                {
                    ...
                    if (DataType.IsNullOrEmpty(trackID) == false)
                    {
                        ...
                        if (i == 1)
                        {
                            String athStr = DBAccess.GetBigTextFromDB("Sys_FrmDBVer", "MyPK", ver.MyPK, "FrmAthDB");
                            DataSet athDS = BP.Tools.Json.ToDataSet(athStr);
                            athDt = athDS.Tables[athDesc.NoOfObj];
                    ...
                    {
                        BP.Sys.FrmAttachmentDBs dbs = BP.WF.CCFormAPI.GenerFrmAttachmentDBs(athDesc, pkVal,
                                           this.FK_FrmAttachment, this.WorkID, this.FID, this.PWorkID, true, this.NodeID, this.FrmID); athDt = dbs.ToDataTableField("DBAths");
                        athDt = dbs.ToDataTableField("DBAths");
                    }
                }
                #region 如果图片显示.(先不考虑.)
                if (athDesc.FileShowWay == FileShowWay.Pict)
                {
                    ...
                    if (athDt.Rows.Count == 0 && athDesc.ItIsUpload == true)
                    {
                    ...
                    switch (this.GetRequestVal("DoTypeExt"))
                    {
                        case "ReqCtrl":
                            ...
                            if (jsonCtrl["code"].ToString().Equals("0") == false)
                                return "err@执行URL返回结果失败";
                            if (jsonCtrl["msg"] != null)
                                return "err@" + jsonCtrl["msg"];
                        ...
                        case "ReqDtlFullList":
                                        ...
                                        if (item.Value.Type == JTokenType.Array)
                                        {
                                            ...
                                            foreach (DataRow dr in dt.Rows)
                                            {
                                                ...
                                                foreach (DataColumn dc in dt.Columns)
                                                {
                                                    //填充从表里的枚举
                                                    MapAttr attr = null;
                                                    Entity ent = dtl.MapAttrs.GetEntityByKey(dtl.No + "_" + dc.ColumnName);
                                                    if (ent == null)
                                                        continue;
                                                    attr = ent as MapAttr;
                                                    ...
                                                    if (uiBindKey != "" && uiBindKey != null)
                                                    {
                                                        //根据枚举value填充
                                                        if (dr[dc.ColumnName].GetType() == typeof(string))
                                                        {
                                                            if (dr[dc.ColumnName].ToString() == "" || dr[dc.ColumnName].ToString() == " ")
                                                            {
                                                                mydtl.SetValByKey(dc.ColumnName, 0);
                                                                continue;
                                                            }
                                                            else
                                                            {
                                                                try
                                                                {
                                                                    mysql = "SELECT IntKey AS No FROM  " + BP.Sys.Base.Glo.SysEnum() + " WHERE EnumKey='" + uiBindKey + "' and Lab ='" + dr[dc.ColumnName].ToString() + "' ORDER BY IntKey ";
                                                                    int keyEum = DBAccess.RunSQLReturnValInt(mysql);
                                                                    mydtl.SetValByKey(dc.ColumnName, keyEum);
                                                                    continue;
                                                                }
                                                                catch (Exception ex)
                                                                {
                                                        ...
                                                        {
                                                            mydtl.SetValByKey(dc.ColumnName, dr[dc.ColumnName]);
                                                        }
                                                    }
                            ...
                            foreach (string str in strsDtl)
                            {
                                ...
                                foreach (DataRow dr in dtDtlFull.Rows)
                                {
                                    ...
                                    int index = dtDtlFull.Rows.IndexOf(dr);
        ...
        public string DtlFrm_Init()
        {
            ...
            GEEntityOID en = new GEEntityOID(this.EnsName);
            ...
            //设置主键.
            en.SetValByKey("OID", DBAccess.GenerOID(this.EnsName));
            #region 处理权限方案。
            if (this.NodeID != 0 && this.NodeID != 999999)
            {
            ...
            return "url@DtlFrm.htm?EnsName=" + this.EnsName + "&RefPKVal=" + this.RefPKVal + "&FrmType=" + (int)dtl.HisEditModel + "&OID=" + en.PKVal + "&IsNew=1" + paras;
        }
        ...
        public string FrmGener_Init_ForDBList()
        {
            try
            {
                ...
                if (expEn.Contains("@") == true)
                {
                    ...
                    expEn = expEn.Replace("@WebUser.DeptNo", WebUser.DeptNo);
        ...
        public string WpsFrm_SaveFile()
        {
            ...
            IFormFile file = HttpContextHelper.RequestFiles()[0];  //context.Request.Files;
        ...
        public string FrmGener_Init_ForEntityNoName()
        {
            MapData md = new MapData(this.EnsName);
            //是否启用装载填充？
            ...
            {
                //主表实体.
                GEEntityNoName en = new GEEntityNoName(this.EnsName);
                string pk = this.RefNo;
                en.No = pk;
                if (DataType.IsNullOrEmpty(pk) == true)
                {
                    en.ResetDefaultVal();
                ...
                DataSet ds = BP.Sys.CCFormAPI.GenerHisDataSet(this.EnsName);
                ...
                if (isLoadData == true && md.ItIsPageLoadFull && me != null && GetRequestValInt("IsTest") != 1)
                {
                    ...
                    en = BP.WF.CCFormAPI.DealPageLoadFull(en, me, attrs, dtls, IsSelf, 0, this.WorkID) as GEEntityNoName;
                }
            ...
            {
                GEEntityNoName myen = new GEEntityNoName(this.EnsName);
                myen.CheckPhysicsTable();
        ...
        public string FrmGener_Init()
        {
            ...
            try
            {
                ...
                if (isLoadData == true && md.ItIsPageLoadFull && me != null && GetRequestValInt("IsTest") != 1)
                {
                    ...
                    if (nd == null)
                    {
                        en = BP.WF.CCFormAPI.DealPageLoadFull(en, me, attrs, dtls, IsSelf, 0, this.WorkID) as GEEntityOID;
                    }
                    else
                    {
                        if ((nd.HisFormType == NodeFormType.SheetTree
                            ...
                            IsSelf = true;
                        en = BP.WF.CCFormAPI.DealPageLoadFull(en, me, attrs, dtls, IsSelf, nd.NodeID, this.WorkID) as GEEntityOID;
                    }
                }
        ...
        private DataSet Dtl_Init_Dataset()
        {
            ...
            //获得他的描述,与数据.
            DataSet ds = BP.WF.CCFormAPI.GenerDBForCCFormDtl(frmID, mdtl, this.RefPKVal, strs, dtlRefPKVal, this.FID, this.GetRequestValBoolen("IshistoryData"), this.GetRequestVal("TrackID"));
            return ds;
        }
        /// <summary>
        /// 上传发发票.
        ...
        public string Dtl_Invoice()
        {
            ...
            if (exts.Contains(ext.ToUpper()) == false)
                return "err@上传的文件格式有误,请上传" + exts + "格式的图片";
            Stream stream = HttpContextHelper.RequestFiles(0).OpenReadStream();//new MemoryStream();
            ...
            string host = "https://aip.baidubce.com/rest/2.0/ocr/v1/multiple_invoice?access_token=" + jd["access_token"].ToString();
            ...
            if (result.Contains("error_code"))
            {
                ...
                string msg = resultObj.GetValue("error_msg").ToString();
                return "err@发票识别错误:" + msg;
            }
            #endregion 百度云识别发票

            #region 开始保存数据.
            //JObject jObj = new JObject(result);
            result = JObject.Parse(result).GetValue("words_result").ToString();
            JArray json = JArray.Parse(result);
            GEDtl dtl = new GEDtl(this.FrmID);
            dtl.SetValByKey("RefPK", this.RefPKVal);
            foreach (JObject item in json)
            {
                string resultStr = item.GetValue("result").ToString();
                ...
                {
                    case "vat_invoice":
                        string strObj = resultObj.GetValue("InvoiceType").ToString();
                        typeStr = JArray.Parse(strObj)[0].Value<string>("word");
                        //获取发票代码   
                        string invoiceCode = resultObj.GetValue("InvoiceCode").ToString();
                        if (JArray.Parse(invoiceCode).Count == 0)
                            dtl.SetValByKey("InvoiceID", "");
                        else
                            dtl.SetValByKey("InvoiceID", JArray.Parse(invoiceCode)[0].Value<string>("word"));
                        // 发票号码  
                        string invoiceNum = resultObj.GetValue("InvoiceNum").ToString();
                        if (JArray.Parse(invoiceNum).Count == 0)
                            dtl.SetValByKey("InvoiceCode", "");
                        else
                            dtl.SetValByKey("InvoiceCode", JArray.Parse(invoiceNum)[0].Value<string>("word"));
                        //开票日期
                        string invoiceDate = resultObj.GetValue("InvoiceDate").ToString();
                        if (JArray.Parse(invoiceDate).Count == 0)
                            dtl.SetValByKey("InvoiceRelDT", "");
                        else
                            dtl.SetValByKey("InvoiceRelDT", JArray.Parse(invoiceDate)[0].Value<string>("word"));
                        //发票金额(价税合计)
                        string amountInFiguers = resultObj.GetValue("AmountInFiguers").ToString();
                        if (JArray.Parse(amountInFiguers).Count == 0)
                            dtl.SetValByKey("InvoiceJE", "");
                        else
                            dtl.SetValByKey("InvoiceJE", JArray.Parse(amountInFiguers)[0].Value<string>("word"));
                        //税额
                        string commodityTax = resultObj.GetValue("CommodityTax").ToString();
                        if (JArray.Parse(commodityTax).Count == 0)
                            dtl.SetValByKey("InvoiceTaxJE", "");
                        else
                            dtl.SetValByKey("InvoiceTaxJE", JArray.Parse(commodityTax)[0].Value<string>("word"));
                        // 使用的税率
                        string commodityTaxRate = resultObj.GetValue("CommodityTaxRate").ToString();
                        if (JArray.Parse(commodityTaxRate).Count == 0)
                            dtl.SetValByKey("InvoiceTaxRate", "");
                        else
                            dtl.SetValByKey("InvoiceTaxRate", JArray.Parse(commodityTaxRate)[0].Value<string>("word"));
                        //购买方名称
                        string purchaserName = resultObj.GetValue("PurchaserName").ToString();
                        if (JArray.Parse(purchaserName).Count == 0)
                            dtl.SetValByKey("InvoiceBuyName", "");
                        else
                            dtl.SetValByKey("InvoiceBuyName", JArray.Parse(purchaserName)[0].Value<string>("word"));
                        //购买方ID
                        string purchaserRegisterNum = resultObj.GetValue("PurchaserRegisterNum").ToString();
                        if (JArray.Parse(purchaserRegisterNum).Count == 0)
                            dtl.SetValByKey("InvoiceBuyCode", "");
                        else
                            dtl.SetValByKey("InvoiceBuyCode", JArray.Parse(purchaserRegisterNum)[0].Value<string>("word"));
                        //购买方地址电话
                        string purchaserAddress = resultObj.GetValue("PurchaserAddress").ToString();
                        if (JArray.Parse(purchaserAddress).Count == 0)
                            dtl.SetValByKey("InvoiceBuyAddr", "");
                        else
                            dtl.SetValByKey("InvoiceBuyAddr", JArray.Parse(purchaserAddress)[0].Value<string>("word"));
                        //购买方开户行及账号
                        string purchaserBank = resultObj.GetValue("PurchaserBank").ToString();
                        if (JArray.Parse(purchaserBank).Count == 0)
                            dtl.SetValByKey("InvoiceBuyBankInfo", "");
                        else
                            dtl.SetValByKey("InvoiceBuyBankInfo", JArray.Parse(purchaserBank)[0].Value<string>("word"));
                        //销售方名称
                        string sellerName = resultObj.GetValue("SellerName").ToString();
                        if (JArray.Parse(sellerName).Count == 0)
                            dtl.SetValByKey("InvoiceSaleName", "");
                        else
                            dtl.SetValByKey("InvoiceSaleName", JArray.Parse(sellerName)[0].Value<string>("word"));
                        //销售方ID
                        string sellerRegisterNum = resultObj.GetValue("SellerRegisterNum").ToString();
                        if (JArray.Parse(sellerRegisterNum).Count == 0)
                            dtl.SetValByKey("InvoiceSaleCode", "");
                        else
                            dtl.SetValByKey("InvoiceSaleCode", JArray.Parse(sellerRegisterNum)[0].Value<string>("word"));
                        //销售方地址电话
                        string sellerAddress = resultObj.GetValue("SellerAddress").ToString();
                        if (JArray.Parse(sellerAddress).Count == 0)
                            dtl.SetValByKey("InvoiceSaleAddr", "");
                        else
                            dtl.SetValByKey("InvoiceSaleAddr", JArray.Parse(sellerAddress)[0].Value<string>("word"));
                        //销售方开户行及账号
                        string sellerBank = resultObj.GetValue("SellerBank").ToString();
                        if (JArray.Parse(sellerBank).Count == 0)
                            dtl.SetValByKey("InvoiceSaleBankInfo", "");
                        else
                            dtl.SetValByKey("InvoiceSaleBankInfo", JArray.Parse(sellerBank)[0].Value<string>("word"));
                        break;
                    case "taxi_receipt":
                        typeStr = "出租车票";
                        break;
                    case "train_ticket":
                        typeStr = "火车票";
                        //车票号
                        string ticket_num = JArray.Parse(resultObj.GetValue("ticket_num").ToString())[0].Value<string>("word");
                        //始发站
                        string starting_station = JArray.Parse(resultObj.GetValue("starting_station").ToString())[0].Value<string>("word");
                        //车次号
                        string train_num = JArray.Parse(resultObj.GetValue("train_num").ToString())[0].Value<string>("word");
                        //到达站
                        string destination_station = JArray.Parse(resultObj.GetValue("destination_station").ToString())[0].Value<string>("word");
                        //出发日期
                        string ticket_date = JArray.Parse(resultObj.GetValue("date").ToString())[0].Value<string>("word");
                        //车票金额
                        string ticket_rates = JArray.Parse(resultObj.GetValue("ticket_rates").ToString())[0].Value<string>("word");
                        //席别
                        string seat_category = JArray.Parse(resultObj.GetValue("seat_category").ToString())[0].Value<string>("word");
                        //乘客姓名
                        string ticket_name = JArray.Parse(resultObj.GetValue("name").ToString())[0].Value<string>("word");
                        //身份证号
                        string id_num = JArray.Parse(resultObj.GetValue("ID_card").ToString())[0].Value<string>("word");
                        //序列号
                        string serial_number = JArray.Parse(resultObj.GetValue("serial_number").ToString())[0].Value<string>("word");
                        //售站
                        string sales_station = JArray.Parse(resultObj.GetValue("sales_station").ToString())[0].Value<string>("word");

                        //发票代码
                        dtl.SetValByKey("InvoiceID", ticket_num);
                        // 发票号码
                        string invoice_num = resultObj.GetValue("invoice_num").ToString();
                        if (JArray.Parse(invoice_num).Count == 0)
                            dtl.SetValByKey("InvoiceCode", invoice_num);
                        else
                            dtl.SetValByKey("InvoiceCode", JArray.Parse(resultObj.GetValue("invoice_num").ToString())[0].Value<string>("word"));
                        //开票日期
                        string invoice_date = resultObj.GetValue("invoice_date").ToString();
                        if (JArray.Parse(invoice_date).Count == 0)
                            dtl.SetValByKey("InvoiceRelDT", invoice_date);
                        else
                            dtl.SetValByKey("InvoiceRelDT", JArray.Parse(resultObj.GetValue("invoice_date").ToString())[0].Value<string>("word"));
                        //不含税金额
                        string fare = JArray.Parse(resultObj.GetValue("fare").ToString())[0].Value<string>("word");

                        //发票金额(价税合计)
                        dtl.SetValByKey("InvoiceJE", ticket_rates);
                        //税额
                        string tax =  resultObj.GetValue("tax").ToString();
                        if (JArray.Parse(tax).Count == 0)
                            dtl.SetValByKey("InvoiceTaxJE", invoice_date);
                        else
                            dtl.SetValByKey("InvoiceTaxJE", JArray.Parse(resultObj.GetValue("tax").ToString())[0].Value<string>("word"));
                        // 使用的税率
                        string tax_rate = resultObj.GetValue("tax_rate").ToString();
                        if (JArray.Parse(tax_rate).Count == 0)
                            dtl.SetValByKey("InvoiceTaxRate", tax_rate);
                        else
                            dtl.SetValByKey("InvoiceTaxRate", JArray.Parse(resultObj.GetValue("tax_rate").ToString())[0].Value<string>("word"));
                        //购买方名称
                    ...
                    case "air_ticket":
                        typeStr = "飞机行程单";
                        String name = JArray.Parse(resultObj.GetValue("name").ToString())[0].Value<string>("word");
                        String carrier = JArray.Parse(resultObj.GetValue("carrier").ToString())[0].Value<string>("word");
                        String date = JArray.Parse(resultObj.GetValue("date").ToString())[0].Value<string>("word");
                        //发票金额(价税合计)
                        dtl.SetValByKey("InvoiceJE", JArray.Parse(resultObj.GetValue("ticket_rates").ToString())[0].Value<string>("word"));
                        dtl.SetValByKey("InvoiceRelDT", date);
                        dtl.SetValByKey("InvoiceBuyName", name);
                        dtl.SetValByKey("InvoiceSaleName", carrier);
                        dtl.SetValByKey("InvoiceSaleCode", JArray.Parse(resultObj.GetValue("agent_code").ToString())[0].Value<string>("word"));
                        //获取其他信息
                        String startingStation = JArray.Parse(resultObj.GetValue("starting_station").ToString())[0].Value<string>("word");
                        String destinationStation = JArray.Parse(resultObj.GetValue("destination_station").ToString())[0].Value<string>("word");
                        String time = JArray.Parse(resultObj.GetValue("time").ToString())[0].Value<string>("word");
                        String flight = JArray.Parse(resultObj.GetValue("flight").ToString())[0].Value<string>("word");
        ...
        public string Dtl_Save()
        {
            ...
            foreach (JObject item in json)
            {
                ...
                foreach (Attr attr in attrs)
                {
                    ...
                    if (attr.UIContralType == UIContralType.CheckBok && attr.MyDataType == DataType.AppInt)
                    {
                        if (val.Equals("0"))
                ...
                if (this.FID == 0 && DataType.IsNumStr(dtl.RefPK))
                    dtl.FID = Int64.Parse(dtl.RefPK);
                else
                    dtl.FID = this.FID;

                //如果是新建，并且里面是ForFID的模式.
                //(已经是启用FID模式，就是子线程节点不用赋新值了先注释了)
        ...
        public string Dtl_SaveRow()
        {
            ...
            if (dtl.OID == 0)
            {
                ...
                bool isVue3 = this.GetRequestValBoolen("IsVue3");
                if (isVue3 == false)
                {
                    ...
                    string dbstr = SystemConfig.AppCenterDBVarStr;
                    Paras paras = new Paras();
                    if (BP.Difference.SystemConfig.AppCenterDBType == DBType.Oracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.GBASE8CByOracle || BP.Difference.SystemConfig.AppCenterDBType == DBType.PostgreSQL || BP.Difference.SystemConfig.AppCenterDBType == DBType.HGDB || BP.Difference.SystemConfig.AppCenterDBType == DBType.UX || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR3 || BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.DM)
                ...
                if (isVue3 == true)
                {
                    ...
                    MapDtls childDtls = new MapDtls(mdtl.No);
                    foreach (MapDtl childDtl in childDtls)
                    {
                        DBAccess.RunSQL("UPDATE " + childDtl.PTable + " SET RefPK='" + dtl.OID + "' WHERE RefPK='" + this.RefPKVal + "'");
                    }
                }
        ...
        public String DtlFrmGener_Init()
        {
            ...
            foreach (MapAttr attr in attrs)
            {
                ...
                if (me != null && me.DoWay.Equals("0") == false && myds.Tables.Contains(uiBindKey) == false) //是否存在.
                {
                    String fullSQL = me.Doc;
                    fullSQL = fullSQL.Replace("~", "'");
        ...
        public string Dtl_InitCount()
        {
            MapDtl mapDtl = new MapDtl(this.EnsName);
            //实体.
            GEEntityOID en = new GEEntityOID(mapDtl.FrmID);
            en.OID = this.WorkID;
            if (en.RetrieveFromDBSources() == 0)
                en.Insert();
            ...
            if (dtlRefPKVal.Equals("0") == true
                ...
                dtlRefPKVal = this.RefPKVal;
            //获取初始化数据，不存在就创建
            int count = BP.WF.CCFormAPI.GetDtlCount(mapDtl, en, dtlRefPKVal, false);
        ...
        public string PopVal_InitTree()
        {
            ...
            sqlObjs = sqlObjs.Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
        ...
        public string PopVal_Init()
        {
            ...
            if (me.PopValWorkModel == PopValWorkModel.Group)
            {
                ...
                {
                    // sqlObjs = Glo.DealSQLExp(sqlObjs, null, null);
        ...
        public string MoreAttach()
        {
            ...
            float latitude = this.GetRequestValFloat("Latitude");
            float longitude = this.GetRequestValFloat("Longitude");
            float orientation = this.GetRequestValFloat("Orientation");
            ...
            GEEntity en = null;
           string msg = "";
            if (athDesc.FrmID.StartsWith("TS.") == true)
            {
                pkVal = this.PKVal;
            }
            else
            {
                mapData = new MapData(athDesc.FrmID);

                //求出来实体记录，方便执行事件.
                en = new GEEntity(athDesc.FrmID);
                en.PKVal = pkVal;
                if (en.RetrieveFromDBSources() == 0)
                {
                    en.PKVal = this.FID;
            ...
            for (int i = 0; i < HttpContextHelper.RequestFilesCount; i++)
            {
                ...
                IFormFile file = HttpContextHelper.RequestFiles(i);
                ...
                if (athDesc.AthSaveWay == AthSaveWay.IISServer)
                {
                    ...
                    dbUpload.SetPara("Orientation", orientation);
                    dbUpload.Insert();
        ...
        public string DtlImpByExcel_Imp()
        {
            try
            {
                ...
                enums.RetrieveInSQL(SysEnumAttr.EnumKey, "SELECT UIBindKey From Sys_MapAttr WHERE FK_MapData='" + dtl.No + "' AND LGType=1");
                DataTable dtt = null;
                //获取外键、外部数据源
                DataSet ds = new DataSet();
                string treeKey = dtl.InitDBAttrs;
                Attr attrKey = null;
                foreach (BP.En.Attr attr in attrs)
                {
                    ...
                    if (attr.MyFieldType == FieldType.Enum || attr.MyFieldType == FieldType.PKEnum)
                        continue;
                    if (ds.Tables.Contains(attr.UIBindKey) == true)
                        continue;
                    dtt = BP.Pub.PubClass.GetDataTableByUIBineKey(attr.UIBindKey);

                    dtt.TableName = attr.UIBindKey;
                    ds.Tables.Add(dtt);
                    if (DataType.IsNullOrEmpty(treeKey) == false && attr.Key.Equals(treeKey))
                        attrKey = attr;
                }
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    foreach (BP.En.Attr attr in attrsExp)
                    {
                        ...
                        if (val == null)
                            continue;
                        val = val.Trim();
                        if (attrKey != null && attr.Key.Equals(treeKey))
                            treeNames += val + ",";
                        switch (attr.MyFieldType)
                        {
                            case FieldType.Enum:
                            case FieldType.PKEnum:
                                SysEnums ses = enums.GetEntitiesByKey(SysEnumAttr.EnumKey, attr.UIBindKey) as SysEnums;
                                bool isHavel = false;
                                ...
                                if (isHavel == false)
                                {
                                    rowMsg += "@数据格式不规范,第(" + i + ")行，列(" + attr.Desc + ")，数据(" + val + ")不符合格式,改值没有在枚举列表里.";
                                    val = attr.DefaultVal.ToString();
                                }
                                break;
                            case FieldType.FK:
                            case FieldType.PKFK:
                                dtt = ds.Tables[attr.UIBindKey];
                                bool isHavelIt = false;
                                foreach (DataRow drr in dtt.Rows)
                                {
                                    if (val.Equals(drr["Name"].ToString()))
                                    {
                                        val = drr["No"].ToString();
                                        isHavelIt = true;
                                        break;
                                ...
                                if (isHavelIt == false)
                                    rowMsg += "@数据格式不规范,第(" + i + ")行，列(" + attr.Desc + ")，数据(" + val + ")不符合格式,改值没有在外键数据列表里.";
                                break;
                            default:
                                if (attr.UIContralType == UIContralType.DDL)
                                {
                                    ...
                                    foreach (DataRow drr in dtt.Rows)
                                    {
                                        if (val.Equals(drr["Name"].ToString()))
                                        {
                                            val = drr["No"].ToString();
                                            isHaveIt = true;
                                            dtlEn.SetValByKey(attr.Key + "T", drr["Name"].ToString());
                                            break;
                                        }
                                    }
                                    if (isHaveIt == false)
                ...
                {
                    dtt = ds.Tables[attrKey.UIBindKey];
                    foreach (DataRow dr in dtt.Rows)
                    {
                        if (treeNames.Contains("," + dr[1].ToString() + ","))
                            continue;
                        GEDtl dtlEn = dtls.GetNewEntity as GEDtl;
                        dtlEn.ResetDefaultVal();
                        dtlEn.SetValByKey(attrKey.Key, dr[0]);
                        if (attrs.Contains(attrKey.Key + "T"))
                            dtlEn.SetValByKey(attrKey.Key + "T", dr[1]);
                        dtlEn.RefPK = this.RefPKVal;
                        switch (dtl.DtlOpenType)
                        {
                            case DtlOpenType.ForEmp:  // 按人员来控制.
                            ...
                            case DtlOpenType.ForWorkID: // 按工作ID来控制
                                dtlEn.RefPKInt = (int)this.WorkID;
                                dtlEn.SetValByKey("FID", this.WorkID);
                                break;
                            case DtlOpenType.ForFID: // 按流程ID来控制.
                                dtlEn.RefPKInt = (int)this.WorkID;
                                dtlEn.SetValByKey("FID", this.FID);
                                break;
                        }
                        dtlEn.SetValByKey("RDT", rdt);
                        dtlEn.SetValByKey("Rec", WebUser.No);
        ...
        public string AttachmentUpload_Down()
        {
            ...
            {
                //根据流生成初试源文件
                string orignalFile = downDB.GenerTempFile(dbAtt.AthSaveWay);

                if (fileEncrypt == true && isEncrypt == true)
                {
                    string temFilePath = orignalFile + ".decryptTemp";

                    if (File.Exists(temFilePath) == true)
                        File.Delete(temFilePath);
                    //根据初试源文件生成解密后的文件
                    EncHelper.DecryptDES(orignalFile, temFilePath);
                    if (File.Exists(orignalFile) == true)
                        File.Delete(orignalFile);

                    filepath = temFilePath;
                }
                else
                    filepath = orignalFile;
            }
        ...
        public void AttachmentDownFromByte()
        {
            ...
            byte[] byteList = downDB.GetFileFromDB("DBFile", null);
            if (byteList != null)
            {
        ...
        public string ChapterFrm_Init()
        {
            ...
            GEEntityOID en = new GEEntityOID(this.FrmID);
            en.OID = this.OID;
            if (en.OID == 0)
                en.ResetDefaultVal();
            if (en.RetrieveFromDBSources() == 0)
            ...
            foreach (MapAttr attr in attrs)
            {
                ...
                me = mes.GetEntityByKey(MapExtAttr.ExtType, MapExtXmlList.AutoFullDLL, MapExtAttr.AttrOfOper, keyOfEn) as MapExt;
                // 过滤掉空sql
                if (me != null && !DataType.IsNullOrEmpty(me.Doc))
                {
                    string fullSQL = me.Doc.Clone() as string;
                    fullSQL = fullSQL.Replace("~", ",");
                    fullSQL = BP.WF.Glo.DealExp(fullSQL, en, null);
            ...
            if (DataType.IsNullOrEmpty(fileds) == false)
            {
                ...
                foreach (GroupField gf in gfs)
                {
                    ...
                    {
                        GEEntityOID ge = new GEEntityOID(gf.CtrlID);
                        ge.OID = this.WorkID;
