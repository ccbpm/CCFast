...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_Rpt : DirectoryPageBase
    {
        ...
        public string SearchFlow_Init()
        {
            ...
            Attrs attrs = ges.GetNewEntity.EnMap.Attrs;
            ...
            if (this.WFSta == 3)
                qo.AddWhere(NDXRptBaseAttr.WFState, ">=", 1);
            #endregion 控制状态.
        ...
        public string ContrastDtlFlow_Init()
        {
            ...
            #region 其他条件
            List<string> strs = HttpContextHelper.RequestParamKeys;
            foreach (string key in strs)
            {
                if (key.Equals("FlowNo") || key.Equals("DTFrom") || key.Equals("DTTo")
                    ...
                    continue;
                if (key.Equals("DoType") || key.Equals("DoMethod") || key.Equals("HttpHandlerName") || key.Equals("t")
                   || key.Equals("Token"))
                    continue;
