            ...
            try
            {
                ...
                switch (nd.HisDeliveryWay)
                {
                    ...
                    case DeliveryWay.ByStationOnly:
                        ...
                        {
                            // 查询当前组织下所有的该岗位的人员.
                            sql += " A.UserID as No, A.Name, D.Name AS FK_DeptText FROM Port_Emp A, Port_DeptEmpStation B, WF_NodeStation C,Port_Dept D ";
                            sql += " WHERE A.OrgNo='" + WebUser.OrgNo + "' AND C.FK_Node=" + nd.NodeID;
                    ...
                    case DeliveryWay.ByTeamDeptOnly: //按照组织智能计算。
                        ...
                        sql += " WHERE A." + BP.Sys.Base.Glo.UserNoWhitOutAS + "=C.FK_Emp AND B.FK_Team=C.FK_Team AND B.FK_Node=" + nd.NodeID + " AND A.OrgNo='" + WebUser.OrgNo + "' AND A.FK_Dept=D.No";
                        break;
                    ...
                    case DeliveryWay.ByDept:
                        sql = " SELECT a.No,a.Name, C.Name FROM Port_Emp A, Port_DeptEmp B, Port_Dept C WHERE C.No=B.FK_Dept AND  B.FK_Dept IN (select fk_dept from WF_NodeDept WHERE FK_Node = "+nd.NodeID+") AND a.no = b.FK_Emp";
                        sql += "  UNION ";
                        sql += "SELECT a.No,a.Name, C.Name FROM Port_Emp A, WF_NodeDept B, Port_Dept C WHERE C.No=A.FK_Dept AND  A.FK_Dept = B.FK_Dept AND B.FK_Node=" + nd.NodeID;
                    ...
                    case DeliveryWay.ByBindEmp:
                        if (SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
                            sql += "  A." + BP.Sys.Base.Glo.UserNo + ",A.Name,B.Name AS FK_DeptText FROM Port_Emp A,Port_Dept B,WF_NodeEmp C WHERE A.FK_Dept=B.No AND A.UserID=C.FK_Emp AND C.FK_Node=" + nodeid + "  AND B.OrgNo='" + WebUser.OrgNo + "'";
                        else
                    ...
                    case DeliveryWay.ByDeptAndStation:
                        sql += "  pdes.FK_Emp AS No,A.Name AS Name,B.Name AS FK_DeptText  FROM  Port_DeptEmpStation pdes, WF_NodeDept wnd,WF_NodeStation wns,Port_Emp A,Port_Dept B WHERE pdes.FK_Emp=A.No AND pdes.FK_Dept=B.No AND A.FK_Dept=B.NO AND  wnd.FK_Dept = pdes.FK_Dept  AND wnd.NodeID = " + nodeid + " AND  wns.FK_Station = pdes.StationNo  AND wnd.NodeID =" + nodeid + " ORDER BY" + "  pdes.EmpNo";
                        break;
                    case DeliveryWay.BySelected: //所有的人员多可以启动, 2016年11月开始约定此规则.
                        ...
                        {
                            sql += "  A." + BP.Sys.Base.Glo.UserNo + ", A.Name, B.Name as FK_DeptText FROM  Port_Emp A, Port_Dept B WHERE A.FK_Dept=B.No ";
