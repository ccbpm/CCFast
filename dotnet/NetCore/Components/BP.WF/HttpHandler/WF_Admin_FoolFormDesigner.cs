...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_Admin_FoolFormDesigner : DirectoryPageBase
    {
        ...
        public string Designer_Move()
        {
            ...
            if (gf.CtrlType.Equals("") == false && (gf.CtrlType.Equals("Attr") || gf.CtrlType.Equals("Dir")) == false)
        ...
        {
            //删除垃圾数据.
            DBAccess.RunSQL("DELETE FROM Sys_GroupField where oid not in (select groupID from Sys_MapAttr ) and ctrlType='' and lab=''");
            ...
            mapDtl.No = this.FrmID;
            int count = mapDtl.RetrieveFromDBSources();
            bool isDtlMapData = count == 0 ? false : true;
            if (i == 0)
            {
                if (count == 1)
                {
            ...
            if (this.NodeID != 0)
            {
                ...
                if (dtl.RetrieveFromDBSources() == 0)
                {
                    ...
                    foreach (MapAttr item in mattrs)
                    {
                        ...
                        item.setFrmID(this.MapDtlNo + "_" + this.NodeID);
        ...
        public string ParseStringToPinyin()
        {
            ...
            string name = GetRequestFormVal("name");
            string flag = GetRequestFormVal("flag");
