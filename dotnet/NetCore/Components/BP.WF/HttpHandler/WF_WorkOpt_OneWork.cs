...
namespace BP.WF.HttpHandler
{
    ...
    public class WF_WorkOpt_OneWork : DirectoryPageBase
    {
        ...
        public string OP_GetStatus()
        {
            ...
            switch (gwf.WFState)
            {
                case WFState.Runing: /* 运行时*/
                    ...
                    break;
                case WFState.CompleteEnd: // 完成.
                case WFState.CompleteByNode: // 完成.
        ...
        private bool IsCanPrintSpecForTianYe(GenerWorkFlow gwf)
        {
            ...
            // 判断是否可以打印.
            string sql = "SELECT Distinct as NDFrom, EmpFrom FROM ND" + int.Parse(this.FlowNo) + "Track WHERE WorkID=" + this.WorkID;
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
            foreach (DataRow dr in dt.Rows)
            {
                //判断节点是否启用了按钮?
                ...
                if (btn.PrintPDFEnable == true || btn.PrintZipEnable == true)
                {
                    ...
                    if (gwf.IsOver && (BP.Web.WebUser.No == empFrom || gwf.Starter == WebUser.No))
                        return true;
                }
            }
            return false;
        ...
        public string Chart_Init2020()
        {
            ...
            try
            {
                ...
                sql = "SELECT NodeID as \"ID\", Name as \"Name\", ICON as \"Icon\", X as \"X\", Y as \"Y\", NodePosType as \"NodePosType\",RunModel as \"RunModel\",HisToNDs as \"HisToNDs\",TodolistModel as \"TodolistModel\" FROM WF_Node WHERE FK_Flow='" +
                ...
                sql = "SELECT FID as \"FID\",NDFrom as \"NDFrom\",NDFromT as \"NDFromT\",NDTo as  \"NDTo\", NDToT as \"NDToT\", ActionType as \"ActionType\",ActionTypeText as \"ActionTypeText\",Msg AS  \"Msg\",RDT AS \"RDT\",EmpFrom \"EmpFrom\",EmpFromT AS \"EmpFromT\", EmpToT AS \"EmpToT\",EmpTo AS \"EmpTo\" FROM " + trackTable +
                      " WHERE WorkID=" +
        ...
        public string Chart_Init()
        {
            ...
            try
            {
                ...
                //获取流程中的节点信息
                sql = "SELECT NodeID AS \"ID\", Name AS \"Name\", ICON AS \"Icon\", X AS \"X\", Y AS \"Y\", NodePosType AS \"NodePosType\",RunModel AS \"RunModel\",HisToNDs AS \"HisToNDs\",TodolistModel AS \"TodolistModel\" ,NodeType AS \"NodeType\",Step AS \"Step\" FROM WF_Node WHERE FK_Flow='" +
                    fk_flow + "' ORDER BY Step";
                dt = DBAccess.RunSQLReturnTable(sql);
                dt.TableName = "WF_Node";
                ds.Tables.Add(dt);
                ...
                if (workid != 0)
                {
                    ...
                    //获取工作轨迹信息
                    string trackTable = "ND" + int.Parse(fk_flow) + "Track";
                    sql = "SELECT FID AS \"FID\",NDFrom AS \"NDFrom\",NDFromT AS \"NDFromT\",NDTo AS \"NDTo\", NDToT AS \"NDToT\", ActionType AS \"ActionType\",ActionTypeText AS \"ActionTypeText\",Msg AS \"Msg\",RDT AS \"RDT\",EmpFrom AS \"EmpFrom\",EmpFromT AS \"EmpFromT\", EmpToT AS \"EmpToT\",EmpTo AS \"EmpTo\",NodeData AS \"NodeData\" FROM " + trackTable +
                          " WHERE WorkID=" +
                          workid + (fid == 0 ? (" OR FID=" + workid) : (" OR WorkID=" + fid + " OR FID=" + fid)) + " ORDER BY RDT DESC";
                ...
                {
                    string trackTable = "ND" + int.Parse(fk_flow) + "Track";
                    sql = "SELECT NDFrom AS \"NDFrom\", NDTo AS \"NDTo\",ActionType AS \"ActionType\",ActionTypeText AS \"ActionTypeText\", Msg AS \"Msg\",RDT AS \"RDT\",EmpFrom AS \"EmpFrom\",EmpFromT AS \"EmpFromT\",EmpToT AS \"EmpToT\",EmpTo AS \"EmpTo\",NodeData AS \"NodeData\" FROM " + trackTable +
        ...
        private string GetFutureCCNodes(Node nd, GERpt rptGe, String nodeIDs)
        {
            string nodes = "";
            //查询出来到达的抄送节点.
            ...
            foreach (Direction dir in dirs)
            {
                ...
                int i = conds.Retrieve(CondAttr.FK_Node, nd.NodeID, CondAttr.ToNodeID, dir.ToNode, CondAttr.CondType, (int)CondType.Dir, CondAttr.Idx);

                //判断是否通过.
                if (i == 0 || conds.GenerResult(rptGe, this.WebUser) == true)
                {
                    if (nodeIDs.Contains(dir.ToNode + ",") == false)
                        nodes += dir.ToNode + ",";
                }
            }
            return nodes;
        ...
        public string GetFlowTrackJsonData()
        {
            ...
            try
            {
                ...
                if (workid != 0)
                {
                    ...
                    //获取工作轨迹信息
                    string trackTable = "ND" + int.Parse(fk_flow) + "Track";
                    sql = "SELECT NDFrom AS \"NDFrom\",NDFromT AS \"NDFromT\",NDTo  AS \"NDTo\", NDToT AS \"NDToT\", ActionType AS \"ActionType\",ActionTypeText AS \"ActionTypeText\",Msg AS \"Msg\",RDT AS \"RDT\",EmpFrom AS \"EmpFrom\",EmpFromT AS \"EmpFromT\", EmpToT AS \"EmpToT\",EmpTo AS \"EmpTo\" FROM " + trackTable +
                          " WHERE WorkID=" +
                ...
                {
                    string trackTable = "ND" + int.Parse(fk_flow) + "Track";
                    sql = "SELECT NDFrom AS \"NDFrom\", NDTo AS \"NDTo\",ActionType AS \"ActionType\",ActionTypeText AS \"ActionTypeText\",Msg AS \"Msg\",RDT AS \"RDT\",EmpFrom AS \"EmpFrom\",EmpFromT AS \"EmpFromT\",EmpToT AS \"EmpToT\",EmpTo AS \"EmpTo\" FROM " + trackTable +
                          " WHERE WorkID=0 ORDER BY RDT ASC";
                    dt = DBAccess.RunSQLReturnTable(sql);
                    dt.TableName = "TRACK";
                    ds.Tables.Add(dt);
