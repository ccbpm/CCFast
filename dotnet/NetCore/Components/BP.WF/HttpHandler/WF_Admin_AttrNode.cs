...
namespace BP.WF.HttpHandler
{
    public class WF_Admin_AttrNode : BP.WF.HttpHandler.DirectoryPageBase
    {
        ...
        /// 获取eventbase类
        /// </summary>
        public string ActionEventBase_Init()
        {
            //业务单元集合.
            DataTable dtBuess = new DataTable();
            dtBuess.Columns.Add("No", typeof(string));
            ...
            ArrayList al = BP.En.ClassFactory.GetObjects("BP.Sys.Base.EventBase");
            foreach (BP.Sys.Base.EventBase en in al)
            {
                DataRow dr = dtBuess.NewRow();
                dr["No"] = en.ToString();
                dr["Name"] = en.Title;
        ...
        public string PushMsg_Save()
        {
            ...
            //推送方式。
            //    msg.PushWayNo = Convert.ToInt32(HttpContextHelper.RequestParams("RB_SMS").Replace("RB_SMS_", ""));
            ...
            //节点预警
            if (this.EventNo.Equals(BP.Sys.EventListNode.NodeWarning) == true)
            {
            ...
            //节点逾期
            if (this.EventNo.Equals(BP.Sys.EventListNode.NodeOverDue))
            {
                int noticeType = Convert.ToInt32(HttpContextHelper.RequestParams("RB_NoticeType").Replace("RB_NoticeType", ""));
                msg.SetPara("NoticeType", noticeType);
