...
namespace BP.Web
{
    ...
    public class WebUser
    {
        ...
        public static Emp ChangeMainDept(string empNo, string deptNo)
        {
            ...
            string authNo = null, string authName = null, bool isSSO = false)
        {
            ...
            if (BP.Difference.SystemConfig.isBSsystem)
            {
                ...
                cookieValues.Add("AppCenterDBType", SystemConfig.AppCenterDBType.ToString());
        ...
        public static string GetValFromCookie(string valKey, string isNullAsVal, bool isChinese)
        {
            ...
            {
                //先从session里面取.
                //string v = System.Web.HttpContextHelper.Current.Session[valKey] as string;
                //2019-07-25 zyt改造
            ...
            get
            {
                ...
                if (SystemConfig.CCBPMRunModel == CCBPMRunModel.Single)
                {
                    if (WebUser.IsAdmin)
                    {
                        ...
                        gloVar.No = WebUser.No + "_Adminer";
                        if (gloVar.RetrieveFromDBSources() == 1)
                            return gloVar.Note; //单机版.
                    }
                    return "0";   
                }
        ...
        /// 设置临时变量.
        /// </summary>
        /// <param name="key">不重复的标记</param>
        /// <param name="vals">值</param>
        public static void TempValSaveByKey(string key, string vals)
        {
            UserRegedit en = new UserRegedit();
            string mypk = WebUser.UserID + "_" + key;
            en.setMyPK(mypk);
        ...
        /// 获得临时变量
        /// </summary>
        /// <param name="key">不重复的值</param>
        /// <param name="isNullValsAs">如果没有就默认返回值.</param>
        /// <returns></returns>
        public static string TempValGetByKey(string key, string isNullValsAs = "")
        {
            UserRegedit en = new UserRegedit();
            string mypk = WebUser.UserID + "_" + key;
            en.setMyPK(mypk);
        ...
        public static bool IsFirstLogin
        {
            get
            {
                string val = GetValFromCookie("IsFirstLogin", null, false);
                if (val == null)
                    val = GetSessionByKey("IsFirstLogin", null);
            ...
            {
                if (value == true)
                    SetSessionByKey("IsFirstLogin", "1");
                else
                    SetSessionByKey("IsFirstLogin", "0");
            }
        }
        public static string Roles
        {
            ...
            {
                string list = "";
                string sql = "select FK_Station from Port_DeptEmpStation where FK_Emp='" + No + "'";
                DataTable rlist = DBAccess.RunSQLReturnTable(sql);
                foreach (DataRow row in rlist.Rows)
                {
                    string FK_Station = row["FK_Station"].ToString();
                    list += "," + FK_Station;
                }
                return list;
            }
