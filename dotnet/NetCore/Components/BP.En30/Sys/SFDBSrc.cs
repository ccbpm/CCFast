...
namespace BP.Sys
{
    ...
    public class SFDBSrc : EntityNoName
    {
        ...
        public FieldCaseModel FieldCaseModel
        {
            get
            {
                switch (this.DBSrcType)
                {
                    ...
                    case BP.Sys.DBSrcType.GBASE8CByOracle:
            ...
            get
            {
                switch (this.DBSrcType)
                {
                    ...
                    case Sys.DBSrcType.HGDB:
                        return DBType.HGDB;
                    case Sys.DBSrcType.DM:
                        return DBType.DM;
                    case Sys.DBSrcType.GBASE8CByOracle:
                        return DBType.GBASE8CByOracle;
                    case Sys.DBSrcType.GBASE8CByMySQL:
        ...
        {
            if (this.No.ToLower().Equals("local") == true)
            ...
            try
            {
                ...
                switch (this.DBSrcType)
                {
                    ...
                    case Sys.DBSrcType.WebApi:
                        //执行Url
                        try
                        {
                            string json = BP.Tools.PubGlo.HttpGet(runObj);
                        ...
                        break;
                    default:
                        throw new Exception("err@不支持的数据库类型:" + this.DBSrcType);
                }
                return null;
            }
            catch (Exception ex)
        ...
        public string IsExistsObj(string objName)
        {
            ...
            switch (this.DBSrcType)
            {
                ...
                case Sys.DBSrcType.Oracle:
                    ...
                    break;
                case Sys.DBSrcType.DM:
                    sql = GetIsExitsSQL(DBType.DM, objName, this.DBName);
                    dt = RunSQLReturnTable(sql);
                    break;
                case Sys.DBSrcType.KingBaseR3:
                case Sys.DBSrcType.KingBaseR6:
        ...
        public string GetIsExitsSQL(DBType dbType, string objName, string dbName)
        {
            switch (dbType)
            {
                ...
                case DBType.MySQL:
                    return string.Format("SELECT (CASE t.TABLE_TYPE WHEN 'BASE TABLE' THEN 'TABLE' ELSE 'VIEW' END) OTYPE FROM information_schema.tables t WHERE t.TABLE_SCHEMA = '{1}' AND t.TABLE_NAME = '{0}'", objName, GetTableSchema());
        ...
        public override Map EnMap
        {
            get
            {
                ...
                string cfg1 = "@local=应用系统数据库(默认)@MSSQL=SQLServer数据库@Oracle=Oracle数据库@MySQL=MySQL数据库@Informix=Informix数据库@KindingBase3=人大金仓库R3@KindingBase6=人大金仓库R6@UX=优漩@Dubbo=Dubbo服务@WS=WebService数据源@WebApi=WebApi";
        ...
        public Hashtable getPairs()
        {
            ...
            if (BP.Sys.DBSrcType.local.Equals(this.DBSrcType))
            {
                return ht;
            }
        ...
        public DataTable GetAllTablesWithoutViews()
        {
            ...
            switch (dbType)
            {
                ...
                case Sys.DBSrcType.MySQL:
                    ...
                    sql.AppendLine(string.Format("    table_schema = '{0}'", GetTableSchema()));
        ...
        public string GetTableSchema()
        {
            ...
            if (BP.Sys.DBSrcType.local.Equals(dbType))
            {
                ...
                schema = BP.Difference.SystemConfig.AppCenterDBDatabase;
            }
            else
            {
                //如果非本地数据库，需要解析连接字符串获取Initial Catalog
                Hashtable ht = getPairs();
                //C#的连接字符串与Java不同
                schema = ht["Initial Catalog"].ToString();
            }
            return schema;
        }
        ...
        /// <returns></returns>
        public string GetTableFieldsJson(string tableName)
        {
            ...
            dt.Columns[0].ColumnName = "No";  //字段名.
            dt.Columns[1].ColumnName = "FType"; //类型.
            dt.Columns[2].ColumnName = "FLen"; //长度.
            dt.Columns[3].ColumnName = "Name"; //描述.
        ...
        public DataTable GetTables(bool isCutFlowTables = false)
        {
            ...
            foreach (DataRow dr in allTables.Rows)
            {
                ...
                mydr["No"] = dr["No"];
                mydr["Name"] = dr["No"].ToString() + ";" + dr["Name"].ToString();
                dt.Rows.Add(mydr);
            }
            return dt;
        }
        ...
        public void Rename(string objType, string oldName, string newName, string tableName = null)
        {
            ...
            switch (dbType)
            {
                ...
                case Sys.DBSrcType.MySQL:
                    if (objType.ToLower() == "column")
                    {
                        string sql = string.Format("SELECT c.COLUMN_TYPE FROM information_schema.columns c WHERE c.TABLE_SCHEMA = '{0}' AND c.TABLE_NAME = '{1}' AND c.COLUMN_NAME = '{2}'", GetTableSchema(), tableName, oldName);
