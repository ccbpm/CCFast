...
namespace BP.Sys
{
    ...
    public class CCFormAPI
    {
        ...
        public static string fieldNameKeys = "[{\"regName\":\"费用|费|计|金额|额|值|价|金\",\"dataType\":\"8\"}," +
                                              "{\"regName\":\"日期\",\"dataType\":\"6\"}," +
                                              "{\"regName\":\"时间\",\"dataType\":\"7\"}," +
                                              "{\"regName\":\"长|宽|高|公斤|斤|率|重|公里\",\"dataType\":\"3\"}," +
        ...
        public static string GenerDataTypeByCHName(List<string> arrayListName)
        {
            ...
            if (!DataType.IsNullOrEmpty(result))
            {
                ...
                qo.AddWhereIn("Name", "(" + Glo.GenerWhereInSQL(String.Join(",", result)) + ")");
            ...
            try
            {
                ...
                {
                    string name = arrayListName[i].ToString();

                    if (DataType.IsNullOrEmpty(name))
                    {
                        continue;
                    }
                    var datas = dt.Select(string.Format("Name='{0}'", name));
                    if (datas.Length > 0)
                    {
                        Hashtable hashtable = new Hashtable();
                        hashtable.Add("name", name);
                        hashtable.Add("dataType", datas[0]["MyDataType"].ToString());
                        list.Add(hashtable);
                    }
                    else
                    {
                        foreach (DataRow drKey in dtFieldNameKeys.Rows)
                        {
                            string regex = "";
                            try
                            {
                                regex = drKey["regName"].ToString();
                            }
                            catch (Exception ex)
                            {
                                Log.DebugWriteError(ex);
                            }
                            if (Regex.IsMatch(arrayListName[i].ToString(), regex))
                            {
                                Hashtable hashtable = new Hashtable();
                                hashtable.Add("name", name);
                                hashtable.Add("dataType", drKey["dataType"]);
                                list.Add(hashtable);
                                break;
                            }
                        }
        ...
        /// </summary>
        /// <param name="name">名称</param>
        public static string GenerFildEndInfoByCHName(ArrayList arrayListName)
        {
            ...
            try
            {
                for (int i = 0; i < arrayListName.Count; i++)
                {
                    ...
                    foreach (DataRow dr in dtFieldEndLabs.Rows)
                    {
                        string regex = "";
                        try
                        {
                            regex = dr["regName"].ToString();
                        }
                        catch (Exception ex)
                        {
                            Log.DebugWriteError(ex);
                        ...
                        if (Regex.IsMatch(arrayListName[i].ToString(), regex))
                        {
                            Hashtable hashtable = new Hashtable();
                            hashtable.Add("name", name);
                            hashtable.Add("unitEndLabName", dr["unitEndLabName"]);
                            hashtable.Add("icon", dr["icon"]);
                            hashtable.Add("tip", dr["tip"]);
                            list.Add(hashtable);
                            break;
                        }
                    }
        ...
        /// </summary>
        /// <param name="frmID"></param>
        public static void RepareCCForm(MapData md)
        {
            if (md.EntityType == EntityType.FrmEntityNoName || md.EntityType == EntityType.EntityTree)
                return;
