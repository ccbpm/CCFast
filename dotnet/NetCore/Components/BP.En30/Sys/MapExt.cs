...
namespace BP.Sys
{
    ...
    public class MapExt : EntityMyPK
    {
        ...
        public bool ItisLoadFull

        {
            get
            {
                return this.GetValBooleanByKey("IsLoadFull");
            }
            set
            {
                this.SetValByKey("IsLoadFull", value);
            }
        }
        /// <summary>
        /// 数据格式
        ...
        public string AutoFullDLL_SQL_ForDtl(Hashtable htMainEn, Hashtable htDtlEn)
        {
            ...
            fullSQL = fullSQL.Replace("@WebUser.DeptNo", WebUser.DeptNo);
...

        {
            get
            {
                string s = this.GetValStrByKey("RefPKVal");
                return s;
            }
            set
            {
                this.SetValByKey("RefPKVal", value);
            }
        }
        public string Tag
        {
        ...
        public string Tag5
        {
            get
            {
                string str = this.GetValStrByKey("Tag5");
                str = str.Replace("~~", "\"");
                str = str.Replace("~", "'");
                str = str.Replace("‘", "'");
                str = str.Replace("’", "'");
        ...
        public string Tag6
        {
            get
            {
                string str = this.GetValStrByKey("Tag6");
                str = str.Replace("~~", "\"");
                str = str.Replace("~", "'");
                str = str.Replace("‘", "'");
                str = str.Replace("’", "'");
        ...
        protected override void afterInsertUpdateAction()
        {
            if (this.ExtType.Equals("MultipleChoiceSmall") == true || this.ExtType.Equals("SingleChoiceSmall") == true)
            {
                ...
                if (attrH.RetrieveFromDBSources() == 0)
                {
                    ...
                    attr.setMyPK(this.FrmID + "_" + this.AttrOfOper);
        ...
        public string GetFullDataDtl(string paras, string oid)
        {
            ...
            if (this.DoWay.Equals("Self"))
            {
                ...
                string sql = DealExp(this.Doc, paras, null);
                SFDBSrc src = new SFDBSrc(this.DBSrcNo);
        ...
        public string GetDataTableByField(string field, string paras, string sqlWhere, string oid, string type)
        {
            ...
            if (DataType.IsNullOrEmpty(sql) == true)
                return "err@【"+this.AttrOfOper+"】配置的扩展属性【"+this.ExtModel+"】的字段" + field + "执行的内容为空,或者没有配置字典.";
        ...
        /// <returns></returns>
        public String GetDataTableByTableSimple(String paras, String field)
        {
        ...
        public string GetDataTableByTableSearch(string paras,string paras1)
        {
            ...
            //如果数据源不包含PageIdx
            bool isHavePageIdx = sql.IndexOf("PageIdx") == -1 ? false : true;
            if (sql.ToLower().IndexOf("where") == -1)
                sql += " WHERE 1=1";
            if (DataType.IsNullOrEmpty(paras) == false)
            {
                ...
                foreach (var item in json)
                {
                    if(item.Key.Equals("PageSize") || item.Key.Equals("PageIdx"))
                    {
                        ...
                        if (item.Key.Equals("PageIdx"))
                            pageIdx = Int32.Parse(item.Value.ToString());
                        if (item.Key.Equals("PageSize"))
                            pageSize = Int32.Parse(item.Value.ToString());
                        continue;
                    }
                    string val = item.Value != null ? item.Value.ToString() : "";
                    if (item.Key.Equals("Key") || item.Key.Equals("key") || item.Key.Equals("KEY"))
                    {
                        sql = sql.Replace("@Key", val).Replace("@key",val).Replace("@KEY", val);
                        continue;
                    }
            ...
            if (DataType.IsNullOrEmpty(this.DBSrcNo) == false && this.DBSrcNo.Equals("local") == false)
            {
                ...
                if (sfdb.HisDBType.Equals(BP.Sys.DBSrcType.MySQL))
                {
                ...
                if (isHavePageIdx == false)
                {
                    ...
                    if (sfdb.HisDBType.Equals(BP.Sys.DBSrcType.Oracle) || sfdb.HisDBType.Equals(BP.Sys.DBSrcType.KingBaseR3) || sfdb.HisDBType.Equals(BP.Sys.DBSrcType.KingBaseR6) || sfdb.HisDBType.Equals(BP.Sys.DBSrcType.GBASE8CByOracle))
                    {
            ...
            else
            {
                ...
                if (isHavePageIdx == false)
                {
                    ...
                    if (SystemConfig.AppCenterDBType == BP.DA.DBType.Oracle || SystemConfig.AppCenterDBType == BP.DA.DBType.KingBaseR3 || SystemConfig.AppCenterDBType == BP.DA.DBType.KingBaseR6 || SystemConfig.AppCenterDBType == BP.DA.DBType.DM || SystemConfig.AppCenterDBType == BP.DA.DBType.GBASE8CByOracle)
                    {
                        int beginIndex = (pageIdx - 1) * pageSize + 1;
                        int endIndex = pageIdx * pageSize;
                        sql = "SELECT * FROM ( SELECT A.*, ROWNUM RN " + "FROM (" + sql + ") A WHERE ROWNUM <= " + endIndex + " ) WHERE RN >=" + beginIndex;
            ...
            if (DataType.IsNullOrEmpty(paras) == false && paras.Equals("undefined") == false)
            {
                if (paras.Contains("@") == true)
                {
                    ...
                    foreach (string key in strs)
                    {
                        ...
                        if (attrKeyOfEn.ToLower().Equals("key"))
                        {
