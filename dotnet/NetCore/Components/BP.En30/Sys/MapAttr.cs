...
namespace BP.Sys
{
    ...
    public class MapAttr : EntityMyPK
    {
        ...
        {
            this.SetValByKey(MapAttrAttr.FK_MapData, frmID);
        }
        /// <summary>
        /// 字段名
        /// </summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(MapAttrAttr.Tag1, null, "标识1", true, false, 0, 2000, 20);
        ...
        public string DoTurnQZFieldType(int fieldConvertType)
        {
            try
            {
                int oldMyDataType = this.MyDataType;
                ...
                if (DBAccess.IsExitsTableCol(md.PTable, this.Field) && (oldMyDataType == (DataType.AppString) || oldMyDataType == (DataType.AppDate) || oldMyDataType == (DataType.AppDateTime))
                  ...
                  fieldConvertType == (DataType.AppInt) || fieldConvertType == (DataType.AppMoney)))
                {
                    ...
                    string fieldBak = this.Field + "Bak";

                    //把字段名更新.
                    if (DBAccess.IsExitsTableCol(md.PTable, fieldBak) == true)
                        DBAccess.DropTableColumn(md.PTable, fieldBak);

                    DBAccess.RenameTableField(md.PTable, this.Field, fieldBak);

                    //创建列
                    DBAccess.CreateTableColumn(md.PTable, this.Field, fieldConvertType, this.Name, this.DefVal);

                    //设置为0.
                    DBAccess.RunSQL("UPDATE " + md.PTable + " SET " + this.Field + "=0 ");

                    string sql = "SELECT  OID," + fieldBak + " FROM " + md.PTable + " WHERE 1=1";
                    string ptable = md.PTable;
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        if (fieldConvertType == (DataType.AppFloat))
                        {
                            ...
                            {
                                sqls += "UPDATE " + ptable + " SET " + this.Field + "=" + result + " WHERE OID=" + oid + ";";
                            }
                        }
        ...
        public string DoTurnFieldType(int fieldType)
        {
            try
            {
                ...
                {
                    //检查是否符合数据规范.
                    string sql = "SELECT  " + this.Field + " FROM " + md.PTable + " WHERE 1=1";
                    DataTable dt = DBAccess.RunSQLReturnTable(sql);
                    foreach (DataRow dr in dt.Rows)
                    {
                    ...
                    {
                        string updataSql = "update " + md.PTable + " set " + this.Field + "=0 where ISNULL(" + this.Field + ") or " + this.Field + "=''";
        ...
        {
            int oldMyDataType = this.MyDataType;
            if (fieldType == DataType.AppDate)
            {
                this.MyDataType = DataType.AppDate;
            }
            else if (fieldType == (DataType.AppDateTime))
            {
                this.MyDataType = DataType.AppDateTime;
            }
            else if (fieldType == (DataType.AppInt))
            {
                this.MyDataType = DataType.AppInt;
            }
            else if (fieldType == (DataType.AppBoolean))
            {
                this.MyDataType = DataType.AppBoolean;
            }
            else if (fieldType == (DataType.AppFloat))
            {
                this.MyDataType = DataType.AppFloat;
            }
            else if (fieldType == (DataType.AppMoney))
            {
                this.MyDataType = DataType.AppMoney;
            }
            else
            {
                return "err@字段类型不存在";
            }
            if (oldMyDataType == this.MyDataType)
            {
                return "err@此字段已是此类型";
            }
            return "";
                            ...
                            switch (BP.Difference.SystemConfig.AppCenterDBType)
                            {
                                ...
                                case DBType.MySQL:
                                    if (this.MaxLen >= 4000)
                                        ...
                                        sql = "ALTER table " + md.PTable + " modify " + this.Field + " NVARCHAR(" + this.MaxLen + ") COMMENT '" + attr.Name + "'";
                                    break;
                                case DBType.Oracle:
                                case DBType.DM:
        ...
        protected override bool beforeInsert()
        {
            ...
            this.setIdx(int.Parse(DateTime.Now.ToString("MMddHHmmss")));
