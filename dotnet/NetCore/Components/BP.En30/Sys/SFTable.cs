...
namespace BP.Sys
{
    ...
    public class SFTable : EntityNoName
    {
        ...
        public DataTable GenerHisDataTable(Hashtable ht = null)
        {
                ...
                foreach (string p in ps)
                {
                    ...
                    try
                    {
                        ...
                        if (pa[1].Contains("@WebUser.DeptNo"))
                            pa[1] = pa[1].Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
                    }
                    catch
                    {
                    }
            ...
            {
                string postData = BP.En.OverrideFile.Data_WebApi(ht, this.GetValStringByKey("RequestMethod"), this.SFDBSrcNo, this.SelectStatement, this.GetValStringByKey("PostDoc"), this.GetValStringByKey("Remark"));
            ...
            if (this.SrcType == DictSrcType.SysDict || this.SrcType == DictSrcType.CreateTable)
            {
                ...
                if (this.CodeStruct == CodeStruct.NoName)
                    sql = "SELECT  BH AS No, Name FROM Sys_SFTableDtl WHERE FK_SFTable='" + this.No + "'";
                else
                    sql = "SELECT  BH AS No, Name,ParentNo FROM Sys_SFTableDtl WHERE FK_SFTable='" + this.No + "'";
                DataTable dt = src.RunSQLReturnTable(sql);
        ...
        public string SaveData(string no, string name, string fk_SFTable)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public string InsertData(string no, string name, string fk_SFTable)
        {
        ...
        public string TS_YuanShi_Data_WebApi_Test()
        {
            SFDBSrc mysrc = new SFDBSrc(this.SFDBSrcNo);
            ...
            paramData = paramData.Replace("~", "\"");
            return BP.Tools.PubGlo.HttpPostConnect(tsUrl, paramData, requestMethod, true);

        }
        public string TS_YuanShi_Data_WebApi_CodeStruct_Test()
        {
            string fieldNo = this.GetValStringByKey("FieldNo");
            if (DataType.IsNullOrEmpty(fieldNo))
                fieldNo = "No";
            ...
            // 根节点
            string jsonNode = this.GetValStringByKey("JsonNode");
            string postData = this.TS_YuanShi_Data_WebApi_Test();
            JToken jToken = JToken.Parse(postData);
            // 如果是JSON数组
            if (jToken.Type == JTokenType.Array)
            {
                ...
                return BP.Tools.Json.ToJson(BP.Tools.Json.ConvertToDataTable(newJsonArr));
            }
            // 如果是JSON对象
            if (jToken.Type == JTokenType.Object)
            {
                ...
                if (!DataType.IsNullOrEmpty(jsonNode) && jsonItem.ContainsKey(jsonNode))
                {
                    ...
                    if (jToken1.Type == JTokenType.Array)
                    {
                        ...
                        return BP.Tools.Json.ToJson(BP.Tools.Json.ToDataTable(newJsonArr.ToString()));
                    }
                    if (jToken1.Type == JTokenType.Object)
                    {
                        JObject targetObj = (JObject)jToken;
                        checkJsonField(targetObj, fieldNo, fieldName, fieldParentNo);
                        JObject itemOfRootNode = getJSONByTargetName(targetObj, fieldNo, fieldName, fieldParentNo);
                        return BP.Tools.Json.ToJson(BP.Tools.Json.ToDataTable(itemOfRootNode.ToString()));
                    }
                    throw new Exception("指定的RootNode下不是JSON数组 或 JSON对象");
                ...
                checkJsonField(currObj, fieldNo, fieldName, fieldParentNo);
                JObject itemOfResponse = getJSONByTargetName(currObj, fieldNo, fieldName, fieldParentNo);
                return BP.Tools.Json.ToJson(BP.Tools.Json.ToDataTable(itemOfResponse.ToString()));
            }
            return "";
        }
        /// <summary>
        ...
        {
            return BP.En.OverrideFile.Data_WebApi(null, this.GetValStringByKey("RequestMethod"), this.SFDBSrcNo, this.SelectStatement, this.GetValStringByKey("PostDoc"), this.GetValStringByKey("Remark"));
        }
        /// <summary>
        /// 获得原始的数据
        /// </summary>
        ...
        public string TS_YuanShi_Data_WebApi_Para(string paras)
        {
            ...
            return BP.En.OverrideFile.Data_WebApi(ht, this.GetValStringByKey("RequestMethod"), this.SFDBSrcNo, this.SelectStatement, this.GetValStringByKey("PostDoc"), this.GetValStringByKey("Remark"));
        }
        /// <summary>
        /// 返回json.
        /// </summary>
