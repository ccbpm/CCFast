...
namespace BP.Sys
{
    ...
    public class MapDataAttr : EntityNoNameAttr
    {
        ...
        /// VSTO模板文件
        /// </summary>
        public const string VSTOExcelFile = "VSTOExcelFile";
        #endregion 参数属性.
    }
    /// <summary>
    /// 映射基础
    ...
    public class MapData : EntityNoName
    {
        ...
        public SysEnums SysEnums
        {
            get
            {
                SysEnums ens = this.GetRefObject("SysEnums") as SysEnums;
                ens = null;
                if (ens == null)
                {
                    ens = new SysEnums();
       ...
       FrmAttachmentAttr.FK_MapData, this.No);
        ...
        public void Check()
        {
            if (this.EntityType == EntityType.FrmEntityNoName)
            {
                GEEntityNoName entity = new GEEntityNoName(this.No);
                entity.CheckPhysicsTable();
            }
            else
            {
                GEEntity entity = new GEEntity(this.No);
                entity.CheckPhysicsTable();
            }
        ...
        public Map GenerHisMap()
        {
            ...
            foreach (MapAttr mapAttr in mapAttrs)
            {
                ...
                ////如果是strin类型的枚举.
                //if (mapAttr.LGType == FieldTypeS.Enum && mapAttr.MyDataType == DataType.AppString
                //    && DataType.IsNullOrEmpty(mapAttr.UIBindKey) == false && mapAttr.UIContralType== UIContralType.RadioBtn)
                //{
                //    Attr attr = mapAttr.HisAttr;
                //    attr.UIVisible = false;
                //    attr.Key = attr.Key + "Text";
                //    attr.Field = attr.Field + "Text";
                //    map.AddAttr(attr);
                //}
            }
        ...
        public GEEntityOID HisGEEn
        {
            get
            {
                if (this._HisEn == null)
                    _HisEn = new GEEntityOID(this.No);
                return _HisEn;
            }
        }
        /// <summary>
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(MapDataAttr.OrgNo, null, "OrgNo", true, false, 0, 34, 20);
        ...
        public static MapData ImpMapData(string specFrmID, DataSet ds)
        {
            ...
            foreach (DataTable dt in ds.Tables)
            {
                ...
                switch (dt.TableName)
                {
                    ...
                    case "Sys_MapData":
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            if (count == 1)
                            {
                                ...
                                md.HisEntityType = mdOld.HisEntityType;
                            }
                    ...
                    case "Sys_MapExt":
                        ...
                        break;
                    case "Sys_MapAttr":
                        string systemFields = ",OID,FID,EnOID,EntityState,BillState,RecNo,RecName,No,Name,BillNo,DeptNo,Starter,StarterName,";
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            if (systemFields.Contains("," + en.KeyOfEn + ","))
                                continue;
                    ...
                    case "Sys_Enums":
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            // 首先处理公共的逻辑部分  
                            string baseKey = se.EnumKey + "_" + se.Lang;
                            ...
                            // 根据不同的模式设置不同的主键  
                            if (BP.Difference.SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS || BP.Difference.SystemConfig.CCBPMRunModel == CCBPMRunModel.GroupInc)
                            {
                                se.OrgNo = BP.Web.WebUser.OrgNo;
                                se.setMyPK(baseKey + "_" + se.OrgNo);
                            }
                            else
                            {
                                se.setMyPK(baseKey);
            ...
            if (mdNew.EntityType == EntityType.FrmEntityNoName)
                mdNew.EnPK = "No";
            else
                mdNew.EnPK = "OID";
            mdNew.RepairMap();
        ...
        /// 修复map.
        /// </summary>
        public void RepairMap(bool isDtl = false)
        {
...
#warning 这些sql 对于Oracle 有问题，但是不影响使用.
            ...
            if (this.EntityType != EntityType.FrmEntityNoName && this.EnPK.Equals("OID"))
            {
            ...
            if (this.EnPK.Equals("No") || this.EnPK.Equals("MyPK"))
            {
                if (attr.IsExit(MapAttrAttr.KeyOfEn, this.EnPK, MapAttrAttr.FK_MapData, this.No) == false)
                {
                    ...
                    attr.Name = this.EnPK.Equals("No") ? "编号" : this.EnPK;
                    attr.setMyDataType(DataType.AppInt);
                    attr.setUIContralType(UIContralType.TB);
                    attr.setLGType(FieldTypeS.Normal);
                    attr.SetValByKey(MapAttrAttr.UIVisible, this.EnPK.Equals("No") ? true : false);
                    attr.SetValByKey(MapAttrAttr.UIIsEnable, false);
                    attr.DefVal = "0";
                    attr.setEditType(EditType.Readonly);
                    attr.Insert();
        ...
        protected override bool beforeInsert()
        {
            ...
            if (this.No.IndexOf("ND") != 0 && DataType.IsNullOrEmpty(this.FormTreeNo) == true)
            {
                string treeNo = "";
                ...
                {
                    string sql = "SELECT No FROM Sys_FormTree WHERE OrgNo='" + WebUser.No + "'";
            ...
            #endregion 给表单找一个目录.
            if (DataType.IsNullOrEmpty(this.Icon) == true)
        ...
        /// <returns></returns>
        public bool ExcelGenerFile(ref byte[] bytes)
        {
            try
            {
                byte[] by = BP.DA.DBAccess.GetByteFromDB(this.EnMap.PhysicsTable, MapDataAttr.No, this.No, MapDataAttr.VSTOExcelFile);
                if (by != null)
                {
                    bytes = by;
                    return true;
                }
                else //数据库中没有就找FrmVSTOTemplate目录里的
                {
                    ...
                    {
                        throw new Exception("err@没有找到模版文件." + tempExcel + " 请确认表单配置.");
                    }
                    return false;
                }
            }
            ...
            {
                Log.DebugWriteError("err@[ExcelGenerFile]读取excel失败：" + ex.Message);
                throw ex;
                return false;
            }
        }
        ...
        public bool ExcelGenerFile(string pkValue, ref byte[] bytes, string saveTo)
        {
            ...
            {
                BP.DA.Log.DebugWriteError("err@ExcelGenerFile读取excel失败：" + ex.Message);
                throw ex;
            }
        }
        /// <summary>
        ...
        public void ExcelSaveFile(byte[] bty)
        {
            BP.DA.DBAccess.SaveBytesToDB(bty, this.EnMap.PhysicsTable, MapDataAttr.No, this.No, MapDataAttr.VSTOExcelFile);
        }
        /// <summary>
        /// 保存excel文件
        /// </summary>
        ...
        public string CreateMapDataVer()
        {
            ...
            if (vers.Count == 0)
            {
                ...
                //把当前表单对应数据改成当前的版本
                DBAccess.RunSQL("UPDATE " + this.PTable + " SET AtPara=CONCAT(AtPara,'@FrmVer=" + this.Ver2022 + "') WHERE AtPara NOT LIKE '%@FrmVer=%'");
            ...
            {
                mattr.setFrmID(this.No);
                mattr.setEditType(EditType.UnDel);
        ...
        /// </summary>
        /// <returns></returns>
        public string Turn2EntityNoName_To_FrmBill()
        {
            if (this.EntityType != EntityType.FrmEntityNoName)
                return "当前不是 FrmEntityNoName 不能转换.";

            string mypk = this.No + "_No";
            ...
            {
                mapAttr.SetValByKey("KeyOfEn", "OID");
                mapAttr.SetValByKey("FK_MapData", this.No);
                mapAttr.SetValByKey("Name", "OID");
                mapAttr.setUIVisible(false);
            ...
            //删除相关字段.
            DBAccess.RunSQL("DELETE FROM Sys_MapAttr WHERE KeyOfEn IN ('Name','No','RecNo','RecName','EntityState','EnOID') AND FK_MapData='" + this.No + "'");
            ...
            //做字段检查.
            DBAccess.RenameTableField(this.PTable, "No", "BillNo");
            //   DBAccess.RenameTableField(this.PTable, "FK_Dept", "DeptNo");
            DBAccess.RenameTableField(this.PTable, "Name", "Title");
            DBAccess.RenameTableField(this.PTable, "RecNo", "Starter");
            DBAccess.RenameTableField(this.PTable, "RecName", "StarterName");
            DBAccess.RenameTableField(this.PTable, "EntityState", "BillState");

            if (DBAccess.IsExitsTableCol(this.PTable, "EnOID") == true)
                DBAccess.RenameTableField(this.PTable, "EnOID", "OID");
            else
                DBAccess.CreateTableColumn(this.PTable, "OID", DataType.AppInt, "OID", "0");
            //删除主键.
            DBAccess.DropTablePK(this.PTable);
            // DBAccess.IsExitsTableIndex(this.PTable,"BillNo");

            //更新从表附件的主键.
            string sql = " SELECT BillNo,OID FROM " + this.PTable + " WHERE OID >0 ";
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
            MapDtls dtls = new MapDtls(this.No);
            FrmAttachments aths = new FrmAttachments(this.No);
            foreach (DataRow dr in dt.Rows)
            {
                string billNo = dr[0].ToString();
                string oid = dr[1].ToString();
                foreach (MapDtl dtl in dtls)
                {
                    string sql1 = "UPDATE " + dtl.PTable + " SET RefPK='" + oid + "' WHERE RefPK='" + billNo + "'";
                    DBAccess.RunSQL(sql1);
                }
                foreach (FrmAttachment ath in aths)
                {
                    string sql1 = "UPDATE  Sys_FrmAttachmentDB  SET RefPKVal='" + oid + "' WHERE RefPKVal='" + billNo + "' AND FK_FrmAttachment='" + ath.MyPK + "' AND FK_MapData='" + this.No + "'";
                    DBAccess.RunSQL(sql1);
                }
            }
            ...
            //更新EnOID= 0 的数据.
            sql = " SELECT BillNo FROM " + this.PTable + " WHERE OID IS NULL OR OID=0";
            dt = DBAccess.RunSQLReturnTable(sql);
            foreach (DataRow dr in dt.Rows)
            {
                string billNo = dr[0].ToString();
                string oid = DBAccess.GenerOID().ToString();
                DBAccess.RunSQL("UPDATE " + this.PTable + " SET OID=" + oid + " WHERE BillNo='" + billNo + "'");
            ...
            DBAccess.DropTablePK(this.PTable);
            //创建主键.
            DBAccess.CreatePK(this.PTable, "OID", SystemConfig.AppCenterDBType);

            DBAccess.RunSQL("UPDATE " + this.PTable + " SET BillState=2 WHERE BillState IS NULL");
            return "执行成功.";
        }
        /// <summary>
        /// 翻译
        ...
        public string Turn2EntityNoName()
        {
            ...
            //删除主键.
            DBAccess.RunSQL("DELETE FROM  Sys_MapAttr WHERE KeyOfEn IN ('Title','BillNo','OID','FID') AND FK_MapData='" + this.No + "'");
            ...
            //做字段检查.
            FrmEntityNoName frm = new FrmEntityNoName(this.No);
            frm.CheckEnityTypeAttrsFor_EntityNoName();

            DBAccess.RenameTableField(this.PTable, "BillNo", "No");
            DBAccess.RenameTableField(this.PTable, "FK_Dept", "DeptNo");
            DBAccess.RenameTableField(this.PTable, "Title", "Name");
            DBAccess.RenameTableField(this.PTable, "Starter", "RecNo");
            DBAccess.RenameTableField(this.PTable, "StarterName", "RecName");
            DBAccess.RenameTableField(this.PTable, "BillState", "EntityState");
            if (DBAccess.IsExitsObject(this.PTable) == false)
            {
                GEEntityNoName en = new GEEntityNoName(this.No);
                en.CheckPhysicsTable();
            }
            if (DBAccess.IsExitsTableCol(this.PTable, "EnOID") == false)
            {
                DBAccess.CreateTableColumn(this.PTable, "EnOID", DataType.AppInt, "EnOID", "0");
                DBAccess.RunSQL("UPDATE " + frm.PTable + " SET EnOID=OID");
            }


            //修改菜单.
            string sql = "UPDATE GPM_Menu SET MenuModel='EntityNoName', AtPara='@EnName=TS.CCBill.FrmEntityNoName@EnPKVal=" + this.No + "', UrlPath ='/src/CCFast/CCBill/SearchEntityNoName.vue' WHERE FrmID='" + this.No + "'";
            DBAccess.RunSQL(sql);

            //删除PK.
            DBAccess.DropTablePK(this.PTable);
            //创建主键.
            DBAccess.CreateTableColumn(this.PTable, "No", DataType.AppString, "编号", "");
            //创建主键.
            DBAccess.CreatePK(this.PTable, "No", SystemConfig.AppCenterDBType);

            //删除主键.
            DBAccess.DropTablePK(frm.PTable);
            ////删除OID列.
            ...
            GEEntityNoName rpt1 = new GEEntityNoName(this.No);
            ...
            //删除描述字段.
            DBAccess.RunSQL("DELETE FROM  Sys_MapAttr WHERE KeyOfEn IN ('BillState','Starter','StarterName') AND FK_MapData='" + this.No + "'");
            return "执行成功.";
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string FrmBillTurn2EntityNoName()
        {
            if (this.EntityType != EntityType.FrmBill)
