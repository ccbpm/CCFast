...
namespace BP.Sys
{
    ...
    public class GEEntity : Entity
    {
        ...
        {
            //先求出来旧的OID.
            object oldOID = this.PKVal;

            //复制主表数据.
            this.Copy(en);
            this.Save();
            this.SetValByKey(this.PK,  oldOID);
            ...
            {
                //删除旧的数据.
                DBAccess.RunSQL("DELETE FROM " + dtl.PTable + " WHERE RefPK='" + this.PKVal.ToString() + "'");
                ...
                foreach (GEDtl enDtlFrom in ensDtlFrom)
                {
                    ...
                    dtlEnBlank.RefPK = this.PKVal.ToString();
            ...
            {
                //删除数据,防止copy重复
                DBAccess.RunSQL("DELETE FROM Sys_FrmAttachmentDB WHERE frmID='" + this.FrmID + "' AND RefPKVal='" + this.PKVal.ToString() + "'");
                ...
                foreach (FrmAttachment athFrom in athsFrom)
                {
                    ...
                    athDBsFrom.Retrieve(FrmAttachmentDBAttr.FK_FrmAttachment, athFrom.MyPK, FrmAttachmentDBAttr.RefPKVal, en.PKVal.ToString());
                    foreach (FrmAttachmentDB athDBFrom in athDBsFrom)
                    {
                        ...
                        athDBFrom.RefPKVal = this.PKVal.ToString();
                        athDBFrom.Insert();
                    }
        ...
        {
            //实例化历史数据表单entity.
            object oidOID = this.PKVal;
            this.SetValByKey(this.PK, oid);
            this.Save();
            ...
            foreach (MapDtl dtl in dtls)
            {
                ...
                {
                    enDtl.RefPK = this.PKVal.ToString();
                    enDtl.InsertAsNew();
                }
            }
            ...
            foreach (FrmAttachment ath in aths)
            {
                ...
                foreach (FrmAttachmentDB athDB in athDBs)
                {
                    ...
                    athDB_N.RefPKVal = this.PKVal.ToString();

                    if (athDB_N.HisAttachmentUploadType == AttachmentUploadType.Single)
                    {
                        /*如果是单附件.*/
                        athDB_N.setMyPK(athDB_N.FK_FrmAttachment + "_" + this.PKVal.ToString());
        ...
        public void SaveAsNew()
        {
            try
            {
                string oidOID = this.PKVal.ToString();

                this.SetValByKey( this.PK, DBAccess.GenerOIDByKey32(this.SerialKey).ToString());
                ...
                {
                    //获取旧数据bye.
                    byte[] val = DBAccess.GetByteFromDB(ptable, "OID", oidOID.ToString(), "DBFile");

                    //保存到新记录里面.
                    DBAccess.SaveBytesToDB(val, ptable, "OID", this.PKVal.ToString(), "DBFile");
                }
        ...
        public static implicit operator GEEntity(GEEntityOID v)
        {
    ...
    public class GEEntitys : EntitiesOID
    {
        ...
        {
            //if (this.FrmID == null)
            //    throw new Exception("@没有能 frmID 给值。");
            return this.FrmID;
        }
        /// <summary>
        /// 主键
