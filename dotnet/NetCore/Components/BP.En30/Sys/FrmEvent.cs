...
namespace BP.Sys
{
    ...
    public class FrmEvent : EntityMyPK
    {
        ...
        public string EventName
        {
            get
            {
                return this.GetValStringByKey(FrmEventAttr.EventName);
            }
            set
            {
                this.SetValByKey(FrmEventAttr.EventName, value);
            }
        }
        ...
        public string EventID
        {
            get
            {
                return this.GetValStringByKey(FrmEventAttr.EventID);
            }
            set
            {
                this.SetValByKey(FrmEventAttr.EventID, value);
            }
        }
        public string EventDoType
        {
            get
            {
                return this.GetValStringByKey(FrmEventAttr.EventDoType);
            }
            set
            {
        ...
        public string MailTitle
        {
            get
            {
                ...
                switch (this.EventID)
                {
                    ...
                    default:
                        throw new Exception("@该事件类型没有定义默认的消息模版:" + this.EventID);
                        break;
                }
                return str;
            }
        ...
        {
            this.EventID = fk_Event;
            this.setFrmID(fk_mapdata);
            this.setMyPK(this.FrmID + "_" + this.EventID);
            this.RetrieveFromDBSources();
        }
        /// <summary>
        /// 重写基类方法
        ...
        public override Map EnMap
        {
            get
            {
                ...
                map.AddTBString(FrmEventAttr.EventID, null, "事件标记", true, true, 0, 400, 10);
                map.AddTBString(FrmEventAttr.EventName, null, "事件名称", true, true, 0, 400, 10);
    ...
    public class FrmEvents : EntitiesMyPK
    {
        ...
        public string DoEventNode(string dotype, Entity en, string atPara)
        {
            ...
            if (en != null)
            {
                //en.RetrieveFromDBSources(); //变量污染的问题. @WebUser.No 是当前人的信息了.,yln注释原因部分数据没有保存就进行查询
            }
        ...
        private string _DoEventNode(string dotype, Entity en, string atPara)
        {
            ...
            FrmEvent nev = this.GetEntityByKey(FrmEventAttr.EventID, dotype) as FrmEvent;
            ...
            doc = doc.Replace("@WebUser.DeptNo", BP.Web.WebUser.DeptNo);
            ...
            switch (nev.HisDoType)
            {
                ...
                case EventDoType.EventBase: //执行事件类.
                    ...
                    try
                    {
                        ...
                        try
                        {
                            r.Add("EventSource", nev.EventID);
                        }
                        catch
                        {
                            r["EventSource"] = nev.EventID;
                        }
                        ...
                        if (atPara != null)
                        {
                            ...
                            foreach (string s in ap.HisHT.Keys)
                            {
                                if (ht.ContainsKey(s) == false)
                                    ht[s]= ap.GetValStrByKey(s);
                            }
                        }
                        return pro.Exec(ht);
                    }
                    throw new Exception("@自定义过程[" + doDoc + "]信息丢失,请联系管理员");
                    #endregion 自定义存储过程
                    break;
                default:
