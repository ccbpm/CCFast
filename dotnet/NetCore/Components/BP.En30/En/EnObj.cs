    ...
    abstract public class EnObj
    {
        ...
        public void ResetDefaultValRowValues()
        {
            ...
            foreach (Attr attr in this._enMap.Attrs)
            {
                ...
                if (v.Equals("@WebUser.DeptNo"))
                {
                    this.SetValByKey(attr.Key, WebUser.DeptNo);
                    continue;
                }

                if (v.Equals("@WebUser.DeptLeader"))
                {
                    this.SetValByKey(attr.Key, WebUser.DeptLeader);
                    continue;
                }
                if (v.Equals("@WebUser.EmpLeader"))
                {
                ...
                if (v.Equals("@WebUser.FK_DeptNameOfFull")
        ...
        public void ResetDefaultVal(string fk_mapdata = null, string fk_flow = null, int fk_node = 0)
        {
            ...
            {
                Paras ps = new Paras();
                ps.SQL = "SELECT MyPK,DefVal,UIIsEnable FROM Sys_FrmSln WHERE FK_MapData =" + ps.DBStr + "FK_MapData  AND FK_Node =" + ps.DBStr + "FK_Node";
                ...
                switch (v)
                {
                    ...
                    case "@RDT":
                        ...
                        if (isReadonly == true)
                        {
                            /// if (myval == v)
                            //如果是流程第一个节点，每次发起取新数据 
                            string fkNode = fk_node.ToString();
                            if (DataType.IsNullOrEmpty(fkNode) == false)
                            {
                                if (fkNode.EndsWith("01") == true)
                                {
                                    this.SetValByKey(attr.Key, DataType.CurrentDateByFormart(dataFormat));
                                    continue;
                                }
