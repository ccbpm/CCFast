...
namespace BP.En
{
    ...
    abstract public class Entity : EnObj
    {
        ...
        /// 
        private bool checkTable = false; // 是否检查表
            ...
            catch (Exception ex)
            {
                    ...
                    || ex.Message.Contains("field list"))
                {
                    if (!checkTable)
                    {
                        this.CheckPhysicsTable();
                        checkTable = true;
                        if (this.EnMap.EnDBUrl.DBUrlType == DBUrlType.AppCenterDSN && DBAccess.IsView(this.EnMap.PhysicsTable, BP.Difference.SystemConfig.AppCenterDBType) == false)
        ...
        /// 避免inset死循环.
        /// </summary>
        protected virtual int InsertExt()
        {
...
#warning zhoupeng 打开如下注释代码？没有考虑到为什么要改变PK.
        ...
        public int Update(string[] keys)
        {
            ...
            catch (System.Exception ex)
            {
                ...
                if (msg.Contains("列名") || msg.Contains("将截断字符串") || msg.Contains("缺少") || msg.Contains("的值太大") || msg.Contains("too long") == true)
                {
                    ...
                    // 输出友好提示信息.
                    throw new Exception(this.CheckInfo());
                }
        ...
        public void SaveFileToDB(string saveToField, string fileFullName)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (DBAccess.IsExitsTableCol(this.EnMap.PhysicsTable, saveToField) == false)
                {
                    ...
                    if (DBAccess.AppCenterDBType == DBType.Oracle || DBAccess.AppCenterDBType == DBType.DM || DBAccess.AppCenterDBType == DBType.GBASE8CByOracle)
                        sql = "ALTER TABLE " + this.EnMap.PhysicsTable + " ADD " + saveToField + " Blob NULL ";
                    if (DBAccess.AppCenterDBType == DBType.KingBaseR3 || DBAccess.AppCenterDBType == DBType.KingBaseR6 || DBAccess.AppCenterDBType == DBType.KingBaseR8)
                        sql = "ALTER TABLE " + this.EnMap.PhysicsTable + " ADD   " + saveToField + " Blob NULL ";
        ...
        public void SaveBytesToDB(string saveToField, byte[] bytes)
        {
            try
            {
                DBAccess.SaveBytesToDB(bytes, this.EnMap.PhysicsTable, this.PK, this.PKVal.ToString(), saveToField);
            }
            catch (Exception ex)
            {
                /* 为了防止任何可能出现的数据丢失问题，您应该先仔细检查此脚本，然后再在数据库设计器的上下文之外运行此脚本。*/
                if (DBAccess.IsExitsTableCol(this.EnMap.PhysicsTable, saveToField) == false)
                {
                    ...
                    if (DBAccess.AppCenterDBType == DBType.Oracle || DBAccess.AppCenterDBType == DBType.DM || DBAccess.AppCenterDBType == DBType.GBASE8CByOracle)
                        sql = "ALTER TABLE " + this.EnMap.PhysicsTable + " ADD " + saveToField + " Blob NULL ";
                    if (DBAccess.AppCenterDBType == DBType.KingBaseR3
                         || DBAccess.AppCenterDBType == DBType.KingBaseR8
                        || DBAccess.AppCenterDBType == DBType.KingBaseR6)
                        sql = "ALTER TABLE " + this.EnMap.PhysicsTable + " ADD   " + saveToField + " Blob NULL ";
                ...
                switch (attr.MyDataType)
                {
                    ...
                    case DataType.AppDateTime:
                        ...
                        else
                        {
                            ...
                            DBAccess.RunSQL("ALTER TABLE " + this.EnMap.PhysicsTable + " ALTER   COLUMN " + attr.Field + " SET DEFAULT '" + attr.DefaultVal + "' ");
                            continue;
                        }
                        break;
                    case DataType.AppInt:
        ...
        public void CheckPhysicsTable()
        {
            try
            {
                ...
                //如果是主键,判断是否MyPK.
                if (pk.Equals("MyPK") == true)
                {
                    if (DBAccess.IsExitsTableCol(this.EnMap.PhysicsTable, "MyPK") == false)
                    {
                        string sql = "SELECT COUNT(*) as Num FROM " + this.EnMap.PhysicsTable;
                        if (DBAccess.RunSQLReturnValInt(sql) == 0)
                        {
                            DBAccess.RunSQLDropTable(this.EnMap.PhysicsTable);
                            this.CreatePhysicsTable(); //创建.
                            return;
                        }
                    }
                }
...
#warning 没有处理好。
        ...
        private void CheckPhysicsTable_MySQL()
        {
            ...
            DataTable dt = DBAccess.RunSQLReturnTable(sql);
            DataTable dc = DBAccess.getTableFieldType(this._enMap.PhysicsTable);
            ...
            foreach (Attr attr in this._enMap.Attrs)
            {
                ...
                if (dt.Columns.Contains(attr.Field) == true)
                    continue; //不存在此列.
                ...
                if (DBAccess.IsExitsTableCol(this._enMap.PhysicsTable, attr.Field) == true)
                    continue;
            ...
            foreach (Attr attr in this._enMap.Attrs)
            {
                ...
                DataRow[] typeRow = dc.Select("Field='" + attr.Key + "'");
                if (typeRow.Length != 1)
                {
                    continue;
                }
                string type = typeRow[0][1].ToString().ToUpper();

                if (attr.MyDataType == DataType.AppString)
                {
                    if (type.Equals(DataBaseFieldType.INT.ToString()) || type.Equals(DataBaseFieldType.FLOAT.ToString())
                        || type.Equals(DataBaseFieldType.DOUBLE.ToString()))
                    {
                        int len = attr.MaxLength;
                        if (len == 0)
                        {
                        ...
                        if (len > 3000)
                        {
                            DBAccess.RunSQL("ALTER TABLE " + table + " MODIFY " + attr.Field + " TEXT COMMENT '" + attr.Desc + "'");
                        }
                        else
                        {
                            DBAccess.RunSQL("ALTER TABLE " + table + " MODIFY " + attr.Field + " NVARCHAR(" + len + ") DEFAULT '" + attr.DefaultVal + "' NULL COMMENT '" + attr.Desc + "'");
                        }
                    }
                    continue;
                ...
                {
                    if (!type.Equals(DataBaseFieldType.INT.ToString()))
                    {

                        DBAccess.RunSQL("ALTER TABLE " + table + " MODIFY " + attr.Field + " INT DEFAULT '" + attr.DefaultVal + "' NULL COMMENT '" + attr.Desc + "'");
                ...
                {
                    if (!type.Equals(DataBaseFieldType.FLOAT.ToString()))
                    {
                        ////对于varchar类型字段转float null和空值会引起错误
                        String updataSql = "update " + table + " set " + attr.Field + "=0 where ISNULL(" + attr.Field + ") or " + attr.Field + "=''";
                        DBAccess.RunSQL(updataSql);
                        DBAccess.RunSQL("ALTER TABLE " + table + " MODIFY " + attr.Field + " FLOAT (11,2) DEFAULT '" + attr.DefaultVal + "' NULL COMMENT '" + attr.Desc + "'");
                    }
                    continue;
                }
                if (attr.MyDataType == DataType.AppDouble)
                {
                    if (!type.Equals(DataBaseFieldType.DOUBLE.ToString()))
                    {
                        DBAccess.RunSQL("ALTER TABLE " + table + " MODIFY " + attr.Field + " DOUBLE DEFAULT '" + attr.DefaultVal + "' NULL COMMENT '" + attr.Desc + "'");
                    }
                    continue;
                ...
                {
                    if (!type.Equals(DataBaseFieldType.DECIMAL.ToString()))
                    {
                        DBAccess.RunSQL("ALTER TABLE " + table + " MODIFY " + attr.Field + " DECIMAL (20,4) DEFAULT '" + attr.DefaultVal + "' NULL COMMENT '" + attr.Desc + "'");
                    }
                    continue;
                }
            }
        ...
        private void CheckPhysicsTable_Ora()
        {
            ...
            foreach (Attr attr in this._enMap.Attrs)
            {
                if (attr.MyFieldType != FieldType.FK)
                    continue;
                attr.HisFKEn.CheckPhysicsTable();
            }
            #endregion 检查外键表是否存在.
        ...
        private void DTSMapToSys_MapData_InitMapAttr(Attrs attrs, string frmID)
        {
            foreach (Attr attr in attrs)
            {
                if (attr.ItIsRefAttr)
