        ...
        public static string GenerFormWhereOfOra(Entity en)
        {
            ...
            foreach (Attr attr in fkAttrs)
            {
                ...
                if (attr.UIBindKey.Contains(","))
                {
                    ...
                    {
                        if (attr.MyDataType == DataType.AppString)
                            where += "(  " + mytable + "." + attr.Key + "=" + fktable + "." + noCol + "  (+) )";
                        else
                            where += "(  " + mytable + "." + attr.Key + "=" + fktable + "." + noCol + "  (+) )";
                    ...
                    {
                        if (attr.MyDataType == DataType.AppString)
                            where += " AND (  " + mytable + "." + attr.Key + "=" + fktable + "." + noCol + "  (+) )";
                        else
                            where += " AND (  " + mytable + "." + attr.Key + "=" + fktable + "." + noCol + "  (+) )";
                    }
                }
...
#warning 20111-12-03 不应出现异常。
        ...
        public static string SelectSQLOfMySQL(Entity en, int topNum)
        {
            ...
            //if (en.ClassID == "TS.CCBill.MethodDictRefBill")
            //{
            //    var i = 0;
            //}
            ...
            //if (map.HisFKAttrs.Count != 0)
            mainTable = en.EnMap.PhysicsTable + ".";
            string[] paraFields = "".Split(',');
            if (map.ParaFields != null) paraFields = map.ParaFields.Split(',');

            foreach (Attr attr in attrs)
            {
                if (attr.MyFieldType == FieldType.RefText)
                    continue;
                if (map.ParaFields != null && paraFields.Contains(attr.Key) == true)
                    continue;
...
#warning 2011-12-03 不应出现异常。
                    ...
                    case DataType.AppInt:
                        ...
                        {
                            if (DataType.IsNullOrEmpty(attr.DefaultVal.ToString()))
                                val = val + ", IFNULL( " + mainTable + attr.Field + " ,'') AS  " + attr.Key;
                            else
...
#warning 替换掉',是因为在直接使用SQL中查询条件是该值的时候会出现SQL错误
                            ...
                            else
                            {
                                if (en.Row[attr.Key] == DBNull.Value)
                                {
                                    if (DataType.IsNullOrEmpty(attr.DefaultValOfReal))
                                        ...
                                        ps.Add(attr.Key, (int)double.Parse(attr.DefaultValOfReal));
                                    continue;
                                }
                                ...
                                if (strInt == null || DataType.IsNullOrEmpty(strInt.ToString()))
                                {
                                    ...
                                    ps.Add(attr.Key, (int)double.Parse(strInt.ToString()));
                            }
                            break;
        ...
        public static string UpdateForPara(Entity en, string[] keys)
        {
            ...
            foreach (Attr attr in map.Attrs)
            {
                ...
                if (parasFields != null)
                {
                    if (parasFields.Contains("," + attr.Key + ","))
                        continue;
                }
