...
namespace BP.En
{
        ...
        public bool ItIsNull
        {
            get
            {
                ...
                else
                {
                    if (this.ItIsNum)
                    {
                        if (this.DefaultVal.Equals("") == true)
                            return true;
                        else 
                            return false;
                    }
            ...
            get
            {
                if (_HisFKEns == null)
                {
                    ...
                    else if (this.MyFieldType == FieldType.FK || this.MyFieldType == FieldType.PKFK)
                    {
                        ...
                        else if (this.UIBindKey.Contains("."))
                    ...
                    else if (this.MyFieldType == FieldType.Normal && this.UIContralType == UIContralType.DDL && DataType.IsNullOrEmpty(this.UIBindKey) == false && this.UIBindKey.ToUpper().StartsWith("SELECT"))
                    {
                        String sqlBindKey = BP.Tools.PubGlo.DealExp(this.UIBindKey, null);
                        ...
                        try
                        {
                            ...
                            {
                                noName = new GENoName(this.Field, this.Desc);
                                noName.No = dr[0].ToString();
                                noName.Name = dr[1].ToString();
                                noNames.AddEntity(noName);
                            }
                            _HisFKEns = noNames;
                        }
                        catch (Exception e)
 ...
 @Key=MyFKField;Name=我的外键字段;Type=String;DefVal=01;AppType=FK;IsLine=false;BindKey=BP.Port.Depts
        ...
        {
            if (attr.MyFieldType == FieldType.Enum && attr.MyDataType == DataType.AppString && attr.UIContralType == UIContralType.CheckBok)
                return;
