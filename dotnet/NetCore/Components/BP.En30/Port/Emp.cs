...
namespace BP.Port
{
    ...
    public class Emp : EntityNoName
    {
        ...
        {
            this.SetValByKey(EmpAttr.UserID, userNo);

            if (BP.Difference.SystemConfig.CCBPMRunModel == CCBPMRunModel.SAAS)
            {
                if (userNo.StartsWith(BP.Web.WebUser.OrgNo + "_") == true)
                {
                    this.SetValByKey(EmpAttr.UserID, userNo.Replace(BP.Web.WebUser.OrgNo + "_", ""));
                    this.SetValByKey(EmpAttr.No, userNo);
                }
                else
                    this.SetValByKey(EmpAttr.No, BP.Web.WebUser.OrgNo + "_" + userNo);
            }
            else
                this.SetValByKey(EmpAttr.No, userNo);
        }
        ...
        public bool CheckPass(string pass)
        {
            ...
            string sqlPass = this.Pass;
            pass = BP.Tools.AESHelper.AESDecrypt(pass, key);

            //BCrypt加密类型 
            if (type != null && type.ToString().Equals("2"))
            {
                return BP.Tools.Cryptography.VerifyPassword(pass, sqlPass);
        ...
        public string DoResetpassword(string pass1, string pass2)
        {
            ...
            if (type != null && type.ToString().Equals("2"))
            {
                pass1 = BP.Tools.Cryptography.HashPassword(pass1);
            }
            //MD5加密类型
            if (type != null && type.ToString().Equals("1"))
            {
        ...
        {
            //用户默认密码
            string defaultPass = SystemConfig.UserDefaultPass;
            //加密类型
            string type = SystemConfig.GetIsPasswordEncryptionType();
            ...
            if (type != null && type.ToString().Equals("2"))
            {
                encodPass = BP.Tools.Cryptography.HashPassword(defaultPass);
            }
            //MD5加密类型
            if (type != null && type.ToString().Equals("1"))
            {
                encodPass = BP.Tools.Cryptography.MD5_Encrypt(defaultPass);
            }
            this.Pass = encodPass;
            return "执行成功.";
        }
        ...
        public string ChangePass(string oldpass, string pass1, string pass2)
        {
            ...
            if (type != null && type.ToString().Equals("2"))
            {
                pass2 = BP.Tools.Cryptography.HashPassword(pass2);
            }
            //MD5加密类型
            if (type != null && type.ToString().Equals("1"))
            {
                pass2 = BP.Tools.Cryptography.MD5_Encrypt(pass2);
            }
