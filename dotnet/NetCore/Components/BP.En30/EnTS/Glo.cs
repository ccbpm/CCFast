...
namespace BP.EnTS
{
    public class Glo
    {
        ...
        public static void SetMap(string classID, string mapData)
        {
            ...
            string isAutoGenerNo = Convert.ToString(json.GetValue("IsAutoGenerNo"));
            if (isAutoGenerNo.ToUpper().Equals("FALSE") == true)
                myMap.ItIsAutoGenerNo = false;
            else
                myMap.ItIsAutoGenerNo = true;
            if (DataType.IsNullOrEmpty(myMap.ParaFields) == true)
                myMap.ParaFields = null; //参数字段.
            myMap.FJSavePath = json.GetValue("FJSavePath").ToString();
            #endregion 主表信息.

            #region 字段集合
            JArray arr = (JArray)json.GetValue("attrs");

            // 移除前端专用key
            foreach(JObject obj in arr)
            {
                obj.Remove("DateConfig");
                obj.Remove("DateTimeConfig");
            }
            // end
            ...
            foreach (System.Data.DataRow dr in dtAttrs.Rows)
            {
                ...
                attr.UIContralType = (BP.En.UIContralType)int.Parse(dr["UIContralType"].ToString());
                myMap.AddAttr(attr);
            }
            #endregion 字段集合
            ...
            string fields = json.GetValue("searchFields")==null?"": json.GetValue("searchFields").ToString();
            string searchFields = "";
            System.Data.DataTable dt = BP.Tools.Json.ToDataTable(fields);
            foreach (System.Data.DataRow dr in dt.Rows)
            {
                searchFields += "@" + dr["label"].ToString() + "=" + dr["searchKey"].ToString();
            }
            myMap.SearchFields = searchFields;
            //查询条件 - 枚举外键的集合.
            string ass = json.GetValue("searchFKEnums")==null?"": json.GetValue("searchFKEnums").ToString();
