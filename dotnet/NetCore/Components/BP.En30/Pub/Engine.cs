...
\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx1065\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx2126\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx3187\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx4248\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx5309\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx6370\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx7431\clbrdrl\brdrw10\brdrs\clbrdrt\brdrw10\brdrs\clbrdrr\brdrw10\brdrs\clbrdrb\brdrw10\brdrs \cellx8492\pard\intbl\qj\f2 <Idx>\f0\cell\f2 <NDFromT>    \f0\cell\f2 <State>        \f0\cell\f2 <ActionTypeTex>            \f0\cell\f2 <EmpFromT>                \f0\cell\f2 <StartTime>                    \f0\cell\f2                     <EndTime>                        \f0\cell\f2 <PassTime>    \f0\cell\row\";
        ...
        public string BKDDtls(string key)
        {
            string oid = this.HisGEEntity.GetValStringByKey("OID");

            string strs = "";
            strs += GenerDtlString("xxxx", "SELECT XX FROM XXX WHERE RefPK=" + oid);
            return strs;
        }
        private string GenerDtlString(string colName, string sql)
        {
            ...
            try
            {
                ...
                foreach (string para in paras)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (para.Contains("BKDDtls"))
                            {
                                str = str.Replace("<" + para + ">", this.BKDDtls(para));
                                continue;
                            }
                            ...
                            foreach (Attr attr in map.Attrs)
                            {
                                switch (attr.MyDataType)
                                {
                                    ...
                                    case DataType.AppString:
                                        ...
                                        string valp = dtl.GetValStrByKey(attr.Key + "Text");
                                        if (DataType.IsNullOrEmpty(valp))
                                            valp = dtl.GetValStrByKey(attr.Key + "T");
                                        if (DataType.IsNullOrEmpty(valp))
                                            valp = dtl.GetValStrByKey(attr.Key);

                                        rowData = rowData.Replace("<" + shortName + "." + attr.Key + ">", GetCode(valp));
                                        break;
                                    default:
                ...
                //节点单个审核人——相关信息按配置标签输出
                if (dtTrack != null && str.ToString().Contains("<WorkCheckBegin>") == false && str.ToString().Contains("<WorkCheckEnd>") == false && SystemConfig.WorkCheckShow == 1)
                {
                    string wkKey = "";
                    string nodeID = "";
                    string empNo = "";
                    string wkVal = "";
                    StringBuilder mypict = null;
                    foreach (DataRow row in dtTrack.Rows) //此处的22是ActionType.WorkCheck的值，此枚举位于BP.WF项目中，此处暂写死此值
                    {
                        int acType = Int32.Parse(row["ActionType"].ToString());
                        if (acType != 22)
                            continue;
                        //节点从.
                        empNo = row["EmpFrom"] == null ? "" : row["EmpFrom"].ToString();
                        nodeID = row["NDFrom"].ToString();
                        wkKey = "<WorkCheck.Note." + nodeID + ">";
                    ...
                    foreach (DataRow row in dtTrack.Rows)
                    {
                        ...
                        if (acType != 22)
                            continue;
                        nodeID = row["NDFrom"].ToString();
                        empNo = row["EmpFrom"].ToString();
                        wkKey = "<WorkCheck.Msg." + nodeID + "." + empNo + ">";
                        if (str.ToString().Contains(wkKey) == true)
                        {
                            str = str.Replace(wkKey, this.GetValueCheckWorkByKey(row, "Msg"));
                        }
                        ...
                        if (str.ToString().Contains(wkKey) == true)
                        {
                            str = str.Replace(wkKey, this.GetValueCheckWorkByKey(row, "EmpFromT"));
                        }
                        wkKey = "<WorkCheck.RDT." + nodeID + "." + empNo + ">";
                        if (str.ToString().Contains(wkKey) == true)
                        {
                            str = str.Replace(wkKey, this.GetValueCheckWorkByKey(row, "RDT"));
                        }
                        wkKey = "<WorkCheck.Siganture." + nodeID + "." + empNo + ">";
                        if (str.ToString().Contains(wkKey) == true)
                        {
                            String filePath = BP.Difference.SystemConfig.PathOfDataUser + "Siganture/" + empNo + ".jpg";
                            if (File.Exists(filePath) == true)
                            {
                                // 定义rtf中图片字符串
                                mypict = new StringBuilder();
        ...
        public RTFEngine(string rtfFile)
        {
            this._EnsDataDtls = null;
            this._HisEns = null;
            this._rtfStr = Cache.GetBillStr(rtfFile, true).Substring(0);
        }
