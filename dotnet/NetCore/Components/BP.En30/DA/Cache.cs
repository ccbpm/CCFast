...
namespace BP.DA
{
    ...
    public class Cache
    {
        ...
        /// 读取单据RTF模板文件
        /// </summary>
        /// <param name="cfile"> 模板文件全路径</param>
        /// <param name="isCheckCache"> 是否从缓存中获取</param>
        /// <returns>RTF内容字符</returns>
        ...
        public static string GetBillStr(string cfile, bool isCheckCache)
        {
            ...
            if (val == null)
            {
                ...
                {
                    file = SystemConfig.PathOfCyclostyleFile + cfile;
                }
        ...
        public static object GetObj(string key, Depositary where)
        {
            ...
            if (BP.Difference.SystemConfig.isBSsystem)
            {
                if (where == Depositary.Application)
                    // return  System.Web.HttpContextHelper.Current.Cache[key];
                    return BS_Cache[key]; //  System.Web.HttpContextHelper.Current.Cache[key];
                else
        ...
        public static object GetObjFormApplication(string key, object isNullAsVal)
        {
            if (BP.Difference.SystemConfig.isBSsystem)
            {
                object obj = BS_Cache[key]; // System.Web.HttpContextHelper.Current.Cache[key];
                if (obj == null)
                    return isNullAsVal;
                else
                    return obj;
