    ...
    public class DBAccess
    {
        ...
        public static void SaveBytesToDB(byte[] bytes, string tableName, string tablePK, object pkVal, string saveToFileField)
        {
            ...
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.DM)
            {
                ...
                DmParameter spFile = new DmParameter("FlowJsonFile", OracleDbType.Blob);
                spFile.Value = bytes;
                cm.Parameters.Add(spFile);

                DmParameter spPK = new DmParameter("PKVal", OracleDbType.NVarchar2);
                spPK.Value = pkVal;
                cm.Parameters.Add(spPK);
            ...
            if (BP.Difference.SystemConfig.AppCenterDBType == DBType.KingBaseR3
            ...
            get
            {
                if (_connectionUserID == null)
                {
                    ...
                    foreach (string str in strs)
                    {
                        //if (BP.Difference.SystemConfig.AppCenterDBType == DBType.DM)
                        //{
                        //    if (str.ToLower().Contains("Initial Catalog") == true)
                        //    {
        ...
        /// <param name="attrMyDataType"></param>
        /// <param name="desc"></param>
        /// <param name="defaultVal"></param>
        public static void CreateTableColumn(string tableName, string columnName, int attrMyDataType, string desc, string defaultVal)
        {
            ...
            switch (attrMyDataType)
            {
                ...
                case DataType.AppBoolean:
                    DBAccess.RunSQL("ALTER TABLE " + tableName + " ADD " + columnName + " INT DEFAULT '" + defaultVal + "' NULL ");
                    break;
        ...
        public static DataTable getTableFieldType(string tableName)
        {
            try
            {
                string sql = "";
                switch (BP.Difference.SystemConfig.AppCenterDBType)
                {
                    ...
                    case DBType.UX:
                    case DBType.GBASE8CByOracle:
                        sql = "	SELECT COLUMN_NAME Field,DATA_TYPE Type FROM INFORMATION_SCHEMA.COLUMNS  WHERE TABLE_NAME = '" + tableName + "'";
                        //Field 字段 Type 类型
                        break;
                    case DBType.MSSQL:
                        sql = "SELECT COLUMN_NAME Field, DATA_TYPE Type FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '" + tableName + "';";
                        break;
                    default:
                        throw new Exception("err@不支持的数据库类型." + BP.Difference.SystemConfig.AppCenterDBType);
        ...
        {
            if (DBAccess.IsExitsTableCol(table, columnName) == false)
                return;
        ...
        public static void AlterColumnToBigInt(String table, String fieldName)
        {
            ...
            switch (SystemConfig.AppCenterDBType)
            {
                ...
                case DBType.KingBaseR6:
                case DBType.GBASE8CByOracle:
                    sql = "ALTER TABLE " + table + " MODIFY " + fieldName + " NUMBER";
                    break;
                case DBType.PostgreSQL:
                case DBType.HGDB:
                    sql = "ALTER TABLE " + table.ToLower() + " ALTER COLUMN " + fieldName.ToLower() + " SET DATA TYPE BIGINT";
                    break;
                case DBType.MySQL:
                case DBType.GBASE8CByMySQL:
                    sql = "ALTER TABLE " + table + " MODIFY " + fieldName + " bigint(64)";
                    break;

                default:
                    Log.DebugWriteError("DBAccess DropTablePK " + "@不支持的数据库类型." + SystemConfig.AppCenterDBType);
        ...
        #region 在当前的 Connection执行 SQL 语句，返回受影响的行数
        /// <summary>
        /// 是否去掉 -- 
        /// </summary>
        ...
        public static void RunSQLScript(string sqlOfScriptFilePath, bool isCutDoubleJianHao = true)
        {
            ...
            if (SystemConfig.AppCenterDBType == DBType.KingBaseR3
                || SystemConfig.AppCenterDBType == DBType.KingBaseR8
        ...
        {
            //String connstr = "Server=146.56.231.175;UserId=SYSDBA;PWD=DAmeng!2#4mN09@$lkjh;Database=dm_jflow;Port=5237";
            //DmConnection conn = new DmConnection(connstr);
            DmConnection conn = new DmConnection(BP.Difference.SystemConfig.AppCenterDSN);
            try
            {
                 if (conn.State != ConnectionState.Open)
        ...
        public static DataTable _delRunProcReturnTable(string sql, Paras paras)
        {
            if (DataType.IsNullOrEmpty(sql))
                throw new Exception("要执行的 sql = null ");
        ...
        public static int RunSQLReturnValIntPara(string sql, string val)
        {
            Paras ps = new Paras();
            ps.SQL = sql;
            ps.Add("V",val);
        ...
        public static int RunSQLReturnValIntPara(string sql, string val1, string val2)
        {
            Paras ps = new Paras();
            ps.SQL = sql;
            ps.Add("V1", val1);
        ...
        {
            object val = DBAccess.RunSQLReturnVal(sql, paras);
        ...
        public static bool IsExitsObject(DBUrl dburl, string obj)
        {
            ...
            {
                throw new Exception("err@判断对象是否存在出现错误,请检查数据库设置不能区分大消息,参考:https://gitee.com/opencc/JFlow/wikis/pages/preview?sort_id=9400758&doc_id=31094, 详细信息:" + ex.Message);
            }
        }
        /// <summary>
        ///是否存在索引？
        ...
        public static DataTable GetTableSchema(string tableName, bool isUpper = true)
        {
            ...
            if (isUpper == false)
            {
                dt.Columns["FNAME"].ColumnName = "FName";  //字段名.
                dt.Columns["FTYPE"].ColumnName = "FType"; //类型.
                dt.Columns["FLEN"].ColumnName = "FLen"; //长度.
                dt.Columns["FDESC"].ColumnName = "FDesc"; //描述.
            }
            return dt;
        }
    }
