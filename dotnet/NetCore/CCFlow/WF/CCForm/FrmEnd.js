                ...
                case "MultipleChoiceSearch"://搜索多选
                    if (mapAttr.UIIsEnable == 0 || isReadonly == true) {
                        ...
                        if (!!val) {
                            ...
                            selects.Retrieve("EleID", mapAttr.KeyOfEn, "RefPKVal", pageData.WorkID || pageData.OID);
                            var vals = [];
                            selects.TurnToArry().forEach(item => {
                                if (val.indexOf(item.Tag1 + ",") != -1)
                                    vals.push(item.Tag2);
                            })
                            if (vals.length != 0)
                                $("#TB_" + mapAttr.KeyOfEn).val(vals.join(","));
                        }
                        break;
                    }
...
function OpenAth(title, keyOfEn, athMyPK, atPara, FK_MapData, frmType, isRead) {
    ...
    var url = url = basePath + "/WF/CCForm/Ath.htm?1=1&AthPK=" + athMyPK + "&OID=" + pageData.WorkID + "&FK_MapData=" + FK_MapData + "&FK_Node=" + pageData.FK_Node + "&IsReadonly=" + isRead + "&Token=" + token;
    if (isRead == 1) {
