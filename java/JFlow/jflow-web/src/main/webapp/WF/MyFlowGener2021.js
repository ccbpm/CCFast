...
const msgTypeKey = 'iframe_msg_type';
...
function GenerWorkNode() {
    ...
    if (data.indexOf("url@") == 0) {
        ...
        if (result.indexOf('err@') != 0) {
            ...
            if (!isEmbedded) ShowWorkReturnTip();
        }
        isChartFrm = true;
...
function Save(saveType) {
    ...
    if (isChartFrm == true && $("#ChapterIFrame").length > 0) {
        ...
        if (frame && frame.contentWindow.Save != undefined && typeof (frame.contentWindow.Save) == "function") {
            ...
            if (!isEmbedded) layer.close(index);
    ...
    layui.form.on('submit(Save)', function (data) {
        ...
        if (isEmbedded) {
            const msgType = localStorage.getItem(msgTypeKey);
...
window.addEventListener("message", (ev) => {
    ...
    if (msgType == 'save-dev-form') {
        localStorage.setItem(msgTypeKey, 'save')
        const saveBtn = document.getElementById("ToolBar").querySelector("button[lay-filter='Save']")
        if (saveBtn) {
            saveBtn.click();
        }
    }
    if (msgType == 'send-dev-form') {
        localStorage.setItem(msgTypeKey, 'send')
        const saveBtn = document.getElementById("ToolBar").querySelector("button[lay-filter='Save']")
        if (saveBtn) {
            saveBtn.click();
        }
